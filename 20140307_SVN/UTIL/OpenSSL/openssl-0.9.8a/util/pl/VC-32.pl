...
if ($FLAVOR =~ /WIN64/)
    ...
    SWITCH: for($wcetgt) {
	/^X86/		&& do {	$wcecdefs.=" -Dx86 -D_X86_ -D_i386_ -Di_386_";
				$wcelflag.=' /LIBPATH:$(PORTSDK_DIR)/lib/x86';
				$wcelflag.=" /machine:IX86";	last; };
	/^ARMV4[IT]/	&& do { $wcecdefs.=" -DARM -D_ARM_ -D$wcetgt";
				$wcecdefs.=" -DTHUMB -D_THUMB_" if($wcetgt=~/T$/);
				$wcecdefs.=" -QRarch4T -QRinterwork-return";
				$wcelflag.=' /LIBPATH:$(PORTSDK_DIR)/lib/ARM';
				$wcelflag.=" /machine:THUMB";	last; };
	/^ARM/		&& do {	$wcecdefs.=" -DARM -D_ARM_ -D$wcetgt";
				$wcelflag.=' /LIBPATH:$(PORTSDK_DIR)/lib/ARM';
				$wcelflag.=" /machine:ARM";	last; };
	/^MIPSIV/	&& do {	$wcecdefs.=" -DMIPS -D_MIPS_ -DR4000 -D$wcetgt";
				$wcecdefs.=" -D_MIPS64 -QMmips4 -QMn32";
				$wcelflag.=' /LIBPATH:$(PORTSDK_DIR)/lib/MIPS';
				$wcelflag.=" /machine:MIPSFPU";	last; };
	/^MIPS16/	&& do {	$wcecdefs.=" -DMIPS -D_MIPS_ -DR4000 -D$wcetgt";
				$wcecdefs.=" -DMIPSII -QMmips16";
				$wcelflag.=' /LIBPATH:$(PORTSDK_DIR)/lib/MIPS';
	...
	/^MIPSII/	&& do {	$wcecdefs.=" -DMIPS -D_MIPS_ -DR4000 -D$wcetgt";
				...
				$wcelflag.=" /machine:MIPS";	last; };
	/^R4[0-9]{3}/	&& do {	$wcecdefs.=" -DMIPS -D_MIPS_ -DR4000";
				$wcelflag.=" /LIBPATH:$(PORTSDK_DIR)/lib/MIPS /machine:MIPS";	last; };
	/^SH[0-9]/	&& do {	$wcecdefs.=" -D$wcetgt -D_$wcetgt_ -DSHx";
				$wcecdefs.=" -Qsh4" if ($wcetgt =~ /^SH4/);
				$wcelflag.=" /LIBPATH:$(PORTSDK_DIR)/lib/SH3 /machine:$wcetgt";	last; };
	{ $wcecdefs.=" -D$wcetgt -D_$wcetgt_";
	  $wcelflag.=" /machine:$wcetgt";			last; };
    }

    $cc='$(CC)';
    $base_cflags=' /W3 /GF /Gy /nologo -DUNICODE -D_UNICODE -DOPENSSL_SYSNAME_WINCE -DWIN32_LEAN_AND_MEAN -DL_ENDIAN -DDSO_WIN32 -DNO_CHMOD -DWCE_ON_DEVICE_CONSOLE -DBUFSIZ=512 -I$(WCE_LITE_COMPAT)/include -I$(PORTSDK_DIR)/include';
    $base_cflags.=" $wcecdefs";
    $opt_cflags=' /MC /O1i';	# optimize for space, but with intrinsics...
    $dbg_clfags=' /MC /Od -DDEBUG -D_DEBUG';
    $lflags=' /nologo /opt:ref';
    $lflags.=' /LIBPATH:$(WCE_LITE_COMPAT)\lib';
    $lflags.=" $wcelflag ";
    }
else	# Win32
    {
...
if ($FLAVOR eq "CE")
	...
	{
	$out_def.='_$(TARGETCPU)_pda_console';
	}
$tmp_def="tmp32";

if ($FLAVOR =~ /CE_PDA_CONSOLE/)
	{
	$tmp_def.='_$(TARGETCPU)_pda_console';
	}
...
if ($FLAVOR eq "CE")
	...
	{
	$ex_libs.=' portlib.lib portlibd.lib wce_lite_compat.lib corelibc.lib ntcompat.lib ws2.lib winsock.lib';
	$ex_libs.=' /nodefaultlib:oldnames.lib' if ($ENV{'TARGETCPU'} eq "X86");
	}
else
	{
...
#		$ret.="\t\$(RM) \$(O_$Name)\n";
		...
		if ($FLAVOR eq "CE")
			...
			{
			$ex.=' portlib.lib wce_lite_compat.lib ws2.lib';
			}
		else
			{
