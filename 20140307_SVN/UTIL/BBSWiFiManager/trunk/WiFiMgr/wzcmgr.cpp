...
void CWZCManagerWireless::OnWZCMessage(HWND hwndSink, const LOG_ENTRY *ple)
{
	...
	case WZC_STARTED:
		...
		if(defPrefNetInfo.bEnable == TRUE)
		{
			...
			// Hidden
			if(defPrefNetInfo.bHidden == TRUE)
			{
...
ConnectError:
    ...
    case WZC_CANCEL_AUTH:
        {
			...
			if( GetRSSI((LPTSTR)ple->ptcAdapterName, &nRSSI) == TRUE )
			{
				wsprintf(tszLogMsg, L"RSSI : %d \r\n", nRSSI );
				AddLogData(1, tszLogMsg);
			}
		...
		NKDbgPrintfW( (ple->dwLogId == WZC_CANCEL_AUTH)? _T("[WiFi]: WZC_CANCEL_AUTH \r\n") : _T("[WiFi]: WZC_FAILED_ASSOCIATION \r\n") );
		
		wsprintf(tszLogMsg, (ple->dwLogId == WZC_CANCEL_AUTH)? _T("WZC_CANCEL_AUTH \r\n") : _T("WZC_FAILED_ASSOCIATION \r\n"));
		AddLogData(1, tszLogMsg);

		// 주변 SSID의 RSSI 를 Log로 기록한다.
		{
			// 주변 SSID 정보
			...
			if( GetBBSIDListInfo((LPTSTR)ple->ptcAdapterName, ndisWlanBssid, &dwNumofItems) == TRUE )
			{
				TCHAR tszTemp[32] = L"";
				...
				if( dwNumofItems > 0 )
				{
					for(DWORD dwIdx = 0; dwIdx<dwNumofItems; dwIdx++)
					{
						...
						wsprintf(tszLogMsg, L"_____%s_%d_", tszTemp, ndisWlanBssid[dwIdx].Rssi );
						AddLogData(1, tszLogMsg);
	...
	// 연결 완료시에 UNATTENDEDMODE 설정
	// 연결 실패, 취소, Close 시에는 UNATTENDEDMODE 해제
	...
	if(!nAllowValue)
	{
		if(ple->dwLogId == WZC_AUTHENTICATED || ple->dwLogId == WZC_ASSOCIATED_NO_8021X || ple->dwLogId == WZC_ASSOCIATED_REQUIRES_8021X)
		{
			SetUnattendedMode(TRUE);
		}
		else if(ple->dwLogId == WZC_CLOSE_EAPOL_SESSION || ple->dwLogId == WZC_FAILED_AUTH_NO_RETRY || ple->dwLogId == WZC_FAILED_ASSOCIATION || ple->dwLogId == WZC_CANCEL_AUTH)
		{
