...
void UpdateWZCLogView()
{
	...
	ptcLog = (TCHAR*)LocalAlloc(0, (WZCLOG_ENTRIES * LOG_STATUS_SIZE * sizeof(TCHAR)));
	if(! ptcLog)
	{
	...
	// Set new text
	::SetWindowText((HWND)hTextBox, ptcLog);
...
DWORD WINAPI WzcLoggingThread(LPVOID pv)
{
	...
	if(g_LogMsgQueue.dwState == WZCLOG_INACTIVE)
	{
		...
		// Create Dialog
		g_hWndLog = CreateDialog(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDD_QC_WZCLOG_N), NULL, WzcLoggingDialogProc);
		if(g_hWndLog == NULL)
		{
...
void LoadStateFromRegistry()
{
	...
	{
		//DEBUGMSG(ZONE_ERROR, 			(TEXT("NETUIQC: Unable to open key [%s]"),			REG_WZCLOG_KEY));
		goto exit1;
	}
...
void SaveStateToRegistry()
{
    ...
    if (dwErr != ERROR_SUCCESS)
    {
        //DEBUGMSG (ZONE_ERROR,            (TEXT("NETUIQC: Unable to open/create key [%s]\r\n"),            REG_WZCLOG_KEY));
        goto exit;
    }
