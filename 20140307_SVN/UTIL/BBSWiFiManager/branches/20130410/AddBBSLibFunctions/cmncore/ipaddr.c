                          ...
                          UINT idMsg, ...)
{
    ...
    pszTitle = (LPCTSTR)LoadString(HINST_RESDLL, idTitle ? idTitle : IDS_NETUI_ERR_TITLE, NULL, 0);
    CBRA(pszTitle);

    pszFormat = (LPCTSTR)LoadString(hMod, idMsg, NULL, 0);
    CBRA(pszFormat);

    // Get the real message.  Allowed to fail...user will get a truncated error message, but it's better than nothing
    VHR(StringCchVPrintf(szMsg, ARRAYSIZE(szMsg), pszFormat, (va_list)(&idMsg + 1)));

    iRet = MessageBox(hWnd, szMsg, pszTitle, fuStyle | MB_SETFOREGROUND);
...
int My_atoi (LPTSTR szBuf)
{
    ...
    // This purposely doesn't check for valid digits
    while (*szBuf)
    {
...
#undef LPCS
        ...
        case WM_PAINT: /* paint control window */
        {
            ...
            //$OZUP: Create pen using the border pen width
            hPen = CreatePen(PS_SOLID, AM_SM_CXBORDER,((CLR_BLACK==GetSysColor(COLOR_WINDOW)) ? CLR_WHITE : CLR_BLACK));
            hOldPen = (HPEN)SelectObject(Ps.hdc, hPen);

            //$OZUP: Rectangle drawing should adjust for thicker pens
            SHRectangle(Ps.hdc, 0, 0, rect.right, rect.bottom);
                                 ...
                                 LPARAM lParam)
{
    ...
    switch (wMsg)
    {
        ...
        case WM_GETDLGCODE :
            {
                LONG nCode = DLGC_WANTCHARS | DLGC_WANTARROWS;
                ...
                if (0 != lParam)
                {
                    ...
                    if (WM_KEYDOWN == pMsg->message)
                    {
                        switch(pMsg->wParam)
                        {
                            case VK_UP:
                            case VK_DOWN:
                                // Let dialog manager cause navigation outside of control with up/down arrows
                                nCode &= ~DLGC_WANTARROWS;
                                break;

                            case VK_LEFT:
                            case VK_RIGHT:
                                if (((VK_LEFT == pMsg->wParam) && (dwChildID == 0)) ||
                                    ((VK_RIGHT == pMsg->wParam) && (dwChildID == (NUM_FIELDS-1))))
                                {
                                    // If left is pressed on the first field or right on the last field
                                    // then navigation might be external. Let edit control decide this.
                                    nCode &= ~DLGC_WANTARROWS;
                                    nCode |= (DLGC_WANTARROWS &
                                             CallWindowProc((WNDPROC)pControl->Children[dwChildID].lpfnWndProc,
                                                              hWnd, 
                                                              wMsg, 
                                                              wParam, 
                                                              lParam));
...
/* spaces and periods fills out the current field and then if possible,
   goes to the next field.*/
            ...
            {
                DWORD dwRet;
                dwRet = Edit_GetSel(hWnd);
                if (dwRet != 0L && HIWORD(dwRet) == LOWORD(dwRet)
                    && ExitField(pControl, dwChildID))
                {
        ...
        if (i < (int)(UINT)pField->byLow || i > (int)(UINT)pField->byHigh)
        {
                ...
                && (hDialog = GetParent(hControlWnd)) != NULL)
            {
                ...
                RuiUserMessage(hDialog, NULL, MB_ICONEXCLAMATION, 0,
                               IDS_NETUI_IPBAD_FIELD_VALUE, i,
                               pField->byLow, pField->byHigh);
                pControl->fInMessageBox = FALSE;
                Edit_SetSel(pField->hWnd, 0, CHARS_PER_FIELD);
                return FALSE;
            }
            // couldn't show warning, just set to max value
            wsprintf(szBuf, TEXT("%d"), pField->byHigh);
