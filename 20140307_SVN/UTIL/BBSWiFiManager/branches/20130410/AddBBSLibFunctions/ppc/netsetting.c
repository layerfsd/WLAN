...
HRESULT WINAPI NetCardsSettingDialog(LPCTSTR pszClass)
{
    ...
    // There are two pages "Wireless" and "Network adapters"
    psp[0].pszTemplate           = MAKEINTRESOURCE(IDD_WZC_WLAN_NETWORK);
    psp[cnPages - 1].pszTemplate = MAKEINTRESOURCE(IDD_NETUI_ADAPTERS);
    
    psp[0].pfnDlgProc            = WZCWLanNetworkDlgProc;
    psp[cnPages - 1].pfnDlgProc  = NetCardAdaptersDlgProc;

    psp[0].pszTitle              = (LPCTSTR)LoadString(HINST_RESDLL, IDS_WZC_WLAN_NETWORK_PAGE_TITLE, NULL, 0);
    psp[cnPages - 1].pszTitle    = (LPCTSTR)LoadString(HINST_RESDLL, IDS_NETUI_ADAPTERS_PAGE_TITLE, NULL, 0);

    psp[0].dwSize                = psp[cnPages - 1].dwSize      = sizeof(PROPSHEETPAGE);
    psp[0].hInstance             = psp[cnPages - 1].hInstance   = HINST_RESDLL;
    psp[0].dwFlags               = psp[cnPages - 1].dwFlags     = PSP_USETITLE;

    psp[0].lParam                = (LPARAM)&sWLanInfo; 
