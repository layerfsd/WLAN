...
static BOOL OnNetworkContextMenu(PWLAN_INFO psInfo, POINT pt, INT iItem)
{
	...
	//TrackPopupMenu(hMenuTrack, TPM_LEFTALIGN | TPM_TOPALIGN, pt.x, pt.y, 0, psInfo->hDlg, NULL);
	SHTrackPopupMenu(hMenuTrack, TPM_LEFTALIGN | TPM_TOPALIGN, pt.x, pt.y, 0, psInfo->hDlg, NULL);
...
IMPLEMENT_DYNAMIC(CWifiPage, CPropertyPage)

CWifiPage::CWifiPage()
	: CPropertyPage(CWifiPage::IDD)
{
	m_psWLanInfo = NULL;
	m_psWLanInfo = new WLAN_INFO;
}
...
BEGIN_MESSAGE_MAP(CWifiPage, CPropertyPage)
	ON_WM_ACTIVATE()
	ON_WM_DESTROY()
	ON_WM_SIZE()
...
BOOL CWifiPage::OnInitDialog()
{
    ...
    SHCreateMenuBar(&mbi);
    m_psWLanInfo->hwndNetListMB = mbi.hwndMB;
	...
	hMenu = (HMENU)::SendMessage(m_psWLanInfo->hwndNetListMB, SHCMBM_GETSUBMENU, 0, tbbi.idCommand);
    VWR(hMenu);

    m_psWLanInfo->pNetworkObject = new CNetworkObject;
    VPR(m_psWLanInfo->pNetworkObject);
    SHLoadMenuExtensions((IUnknown*)m_psWLanInfo->pNetworkObject,
                         gc_szMenuNamespace,
                         gc_szSofkeyExtensions,
                         &m_psWLanInfo->hSoftKeyExt);


    // Initialize the dialog contents
    //InitNetworkListView(m_psWLanInfo);
    

    
    // Override notification UI
    SetCallbackRegistration(TRUE, m_psWLanInfo);
    ...
    InitFilterCombo(m_psWLanInfo);

    m_psWLanInfo->dwHWFlags = SHWZCGetHardwareFlags();

    // Populate the network list
    PopulateNetworkList(m_psWLanInfo);



    // We assume that we have hardware if we're here
    m_psWLanInfo->fWirelessCardPresent = TRUE;

    ListView_SetItemState(m_psWLanInfo->hwndNetList, 0, LVIS_SELECTED|LVIS_FOCUSED, LVIS_SELECTED|LVIS_FOCUSED);



	if (m_psWLanInfo->fWirelessCardPresent)
...
BOOL CWifiPage::OnCommand(WPARAM wParam, LPARAM lParam)
{
	// TODO: 여기에 특수화된 코드를 추가 및/또는 기본 클래스를 호출합니다.
    INT  nItemSelected;

    nItemSelected = ListView_GetNextItem(m_psWLanInfo->hwndNetList, -1, LVNI_SELECTED);

    switch(LOWORD(wParam))
    {
    case IDM_WZC_NETWORK_CONNECT:
        OnConnect(m_psWLanInfo, nItemSelected);
        break;

    case IDS_WZC_ADD_NEW_SETTINGS:
        OnAddNewSetting(m_psWLanInfo);
        break;

    case IDM_WZC_NETWORK_REMOVE:
        OnRemoveSetting(m_psWLanInfo, nItemSelected);
        break;

    case IDM_WZC_NETWORK_PROPERTIES:
        OnNetworkProperties(m_psWLanInfo, nItemSelected);
        break;

    case IDM_WZC_ADVANCED:
        DialogBox(HINST_RESDLL, MAKEINTRESOURCE(IDD_WZC_WLAN_ADVANCED), m_psWLanInfo->hDlg, WZCWLanAdvancedDlgProc);
        break;

    case IDC_WZC_COMBO_FILTER:
        if (HIWORD(wParam) == CBN_SELENDOK)
        {
            OnChangeNetFilter(m_psWLanInfo);
        }
        break;

    case IDM_WZC_WIFI_POWERCMD:
        OnChangePowerState(m_psWLanInfo);
        break;

    default:
        if (LOWORD(wParam) >= IDM_WZC_MENU_EXT_FIRST && LOWORD(wParam) <= IDM_WZC_MENU_EXT_LAST)
        {
            SHInvokeContextMenuCommand(m_psWLanInfo->hwndNetList,
                                       LOWORD(wParam),
                                       m_psWLanInfo->hContextExt ? m_psWLanInfo->hContextExt : m_psWLanInfo->hSoftKeyExt);
        }
    }
...
void CWifiPage::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
    ...
    m_psWLanInfo->bActivate = (nState != WA_INACTIVE);
	...
	else if (NULL == pWndOther || pWndOther->GetParent() != this->GetParent()) 
    {
        SetCallbackRegistration(FALSE, m_psWLanInfo);
    }

}

void CWifiPage::OnDestroy()
{
	CPropertyPage::OnDestroy();

	// TODO: 여기에 메시지 처리기 코드를 추가합니다.

	DeleteAllItems(m_psWLanInfo);
    if (NULL != m_psWLanInfo->hdsaNetworks)
    {
        DSA_Destroy(m_psWLanInfo->hdsaNetworks);
    }

    // Destroy the callback
    VHR(SetCallbackRegistration(FALSE, m_psWLanInfo));

    // Free the image list
    if (m_psWLanInfo->hImgs)
    {
        ImageList_Destroy(m_psWLanInfo->hImgs);
    }

    SHFreeContextMenuExtensions(m_psWLanInfo->hSoftKeyExt);
    ((IUnknown*)m_psWLanInfo->pNetworkObject)->Release();
}

BOOL CWifiPage::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	// TODO: 여기에 특수화된 코드를 추가 및/또는 기본 클래스를 호출합니다.
	...
	if (NULL != pnmh)
	{
		...
		case IDC_WZC_LIST_NETWORKS:
			...
			{
			case GN_CONTEXTMENU:
				OnNetworkContextMenu(m_psWLanInfo, ((NMRGINFO *)lParam)->ptAction, nItemSelected);
				break;
			...
			case LVN_ITEMACTIVATE:
				if (nItemSelected == 0)
				{
					...
					OnFirstSettingActivate(m_psWLanInfo);
				}
				else if (nItemSelected > 0)
				{
					// User hit a wireless network, bring up the config properties dialog
					::SendMessage(m_psWLanInfo->hDlg, WM_COMMAND, MAKEWPARAM(IDM_WZC_NETWORK_PROPERTIES, 0), 0L);
				}                       
				break;
			...
			case LVN_ITEMCHANGED:
				{
					if (pnmlv->uChanged & LVIF_STATE && pnmlv->uNewState & LVIS_SELECTED)
					{
						...
						if(fEnable)
						{
							PCSHWZCNET pwzcnet = (PCSHWZCNET)GetListItemLPARAM(m_psWLanInfo->hwndNetList, nItemSelected);
							ASSERT(pwzcnet);
						...
						SHEnableSoftkey(m_psWLanInfo->hwndNetListMB, 0, TRUE, fEnable);
					}
					break;
				}
			}
		...
		default:        
			...
			{
			case PSN_APPLY:
				::SendMessage(m_psWLanInfo->hDlg, WM_COMMAND, IDOK, 0L);
				break;
			case PSN_KILLACTIVE:
			case PSN_SETACTIVE:
				::ShowWindow(m_psWLanInfo->hwndNetListMB, (pnmh->code == PSN_SETACTIVE ? SW_SHOW : SW_HIDE));
				break;
			}
			break;
		}
...
void CWifiPage::OnSize(UINT nType, int cx, int cy)
{
	CPropertyPage::OnSize(nType, cx, cy);

	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
	
	if (NULL != m_psWLanInfo)
    {
		HWND hwndStatic = ::GetDlgItem(m_psWLanInfo->hDlg, IDC_WZC_STATIC_NO_WIRELESS_CARD_PRESEND);
        BOOL fShowList  = m_psWLanInfo->fWirelessCardPresent;
        RECT rect;

        GetWindowRect(&rect);
		::MapWindowPoints(NULL, m_psWLanInfo->hDlg, (LPPOINT)&rect, 2 );
		...
		::SetWindowPos(m_psWLanInfo->hwndNetList, hwndStatic, 0, 0, 0, 0,
		...
		::EnableWindow(m_psWLanInfo->hwndNetList, fShowList);
    }
	

}

HRESULT CWifiPage::RefreshNetListView()
{
	HRESULT hr = S_OK;
    CNetID nidOrig;
    ...
    INT nItemSelected = ListView_GetNextItem(m_psWLanInfo->hwndNetList, -1, LVNI_SELECTED);
	...
	::SendMessage(m_psWLanInfo->hwndNetList, WM_SETREDRAW, (WPARAM)FALSE, 0L);
    ...
    if (m_psWLanInfo->fWirelessCardPresent)
    {
        hr = PopulateNetworkList(m_psWLanInfo);
        CHR(hr);
        ...
        if (nidOrig.IsValid())
        {
            INT nItems = ListView_GetItemCount(m_psWLanInfo->hwndNetList);
            INT i;
            CNetID  nidTemp;
            // Don't bother with index 0, it is "Add New..."
            for (i = 1; i < nItems; i++)
            {
                PCSHWZCNET pwzcnetTemp = (PCSHWZCNET) GetListItemLPARAM(m_psWLanInfo->hwndNetList, i);
        ...
        ListView_SetItemState(m_psWLanInfo->hwndNetList, nItemSelected, LVIS_SELECTED|LVIS_FOCUSED, LVIS_SELECTED|LVIS_FOCUSED);
    }

Error:
	::SendMessage(m_psWLanInfo->hwndNetList, WM_SETREDRAW, (WPARAM)TRUE, 0L);
	::RedrawWindow(m_psWLanInfo->hwndNetList, NULL, NULL, RDW_INVALIDATE | RDW_NOERASE | RDW_UPDATENOW);

    return hr;
}


HRESULT CWifiPage::OnActivateHome(WPARAM wParam, LPARAM lParam)
{
    HRESULT hr = S_OK;
    ...
    // closing the dialog.
    hr = SetCallbackRegistration(FALSE, m_psWLanInfo);
    CHR(hr);
...
void CWifiPage::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
	...
	HRESULT    hr = S_OK;
    PCSHWZCNET pwzcnet;
	INT	iItem = ListView_GetNextItem(m_psWLanInfo->hwndNetList, -1, LVNI_SELECTED);
    ...
    pPopupMenu->InsertMenuW((UINT)-1, MF_BYPOSITION|MF_STRING, IDM_WZC_WIFI_POWERCMD,
               LoadStringEtc(HINST_RESDLL, IDS_WZC_WIFI_POWERON + ((m_psWLanInfo->dwHWFlags & SHWZCF_HW_ON) ? 1:0), NULL));

    pwzcnet = (PCSHWZCNET)GetListItemLPARAM(m_psWLanInfo->hwndNetList, iItem);
    CBR(pwzcnet != NULL);
    ...
    ((CNetworkObject*)(m_psWLanInfo->pNetworkObject))->SetData(pNetwork, CF_UNICODETEXT);
    SHQueryMenuExtensions(m_psWLanInfo->hContextExt ? m_psWLanInfo->hContextExt : m_psWLanInfo->hSoftKeyExt,
                          pPopupMenu->m_hMenu, 0xFFFFFFFF, IDM_WZC_MENU_EXT_FIRST, IDM_WZC_MENU_EXT_LAST,
                          CMF_NORMAL);
	
    // Get the current state
    SHWZCGetState(m_psWLanInfo->hshwzc, &dwState);
