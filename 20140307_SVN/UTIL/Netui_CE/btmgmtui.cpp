...
static BOOL CALLBACK DlgProcScanDC (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch (uMsg) 
	{
		...
		{	
			...
			HIMAGELIST hIconList = LoadIcons();
			ListView_SetImageList(ghUntrustedList, hIconList, LVSIL_SMALL);
			ListView_SetImageList(ghTrustedList, hIconList, LVSIL_SMALL);			
...
static DWORD WINAPI DoInquiry (LPVOID lpUnused) {
	...
	if ((iErr == SOCKET_ERROR) && ((iErr = GetLastError ()) != WSA_E_NO_MORE) && ghInquiryHandle) {
		...
		WCHAR szString[MAX_LOC_STRING + 20];
		WCHAR *szErr = L"";
		switch (iErr) {
...
static void ActivateBthDevice(BTDEVICE* pbtDevice) {
	...
	if (! BthEnumActivate (pbtDevice, TRUE)) {
		WCHAR szString[256];
		StringCchPrintfW (szString, ARRAYSIZE(szString), L"%s %d", gszErrFailComm, GetLastError ());
