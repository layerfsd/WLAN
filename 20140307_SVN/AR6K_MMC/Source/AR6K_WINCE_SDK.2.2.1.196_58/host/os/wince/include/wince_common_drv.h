...
{
    ULONG	dbgSetOid;			//buildercheck: for debug
    ULONG	dbgQueryOid;		//buildercheck: for debug
    ULONG	dbgBSSEvent;		//buildercheck: for debug
    ULONG	dbgBSSList;			//buildercheck: for debug
    ULONG	dbgErrorLog;		//buildercheck: for file log	
    ULONG	dbgFileLog;		//buildercheck: for file log
...
//buildercheck 
    ...
    ULONG RoamThreshold;
    ULONG RoamFloorThre;
	A_UINT16 channelList[64]; // 64 channel max list;	
	A_INT8 channel_cnt;	
	A_INT8 ChannelMode;	
	wchar_t dbgLogDir[128];	
	char dbgLogFName[512];
	char probeSSID[128];
	char probeSSIDa[128];
    ULONG   probeEnable;               // Enable/Disable probe hidden ssid
    ULONG   tp;               // set thread priority in start endpoint: 100~248 100~150
    ULONG   DwellTime;  //scan param
    ULONG   suspending;
	char lockSSID[128];
	 ULONG   lockEnable;               // Enable/Disable ssid lock
}WLAN_STA_CONFIG;
