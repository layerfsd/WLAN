                      ...
                      AR6K_BIN_CACHE_INFO *pRomCache)
{
    ...
    do
    {
        if (pRomCache->Valid) {
            ...
            //-builder ATHR_DISPLAY_MSG (L"file %s \n",filePath);
            //Read the patch distribution file
            if ( (fd = CreateFile (filePath, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL)) == INVALID_HANDLE_VALUE)
            {
                //-builder ATHR_DISPLAY_MSG (L"CAR6K:: No Patch File :: ====> %s", filePath);
                break;
            }
            ...
            if (pRomCache->Static)
            {
            	RETAILMSG(1, (TEXT("builder:ar6k2:apply_patch() what?\r\n")));
...
RETAILMSG(1, (TEXT("builder:ar6k2:%i wlan on start  firm down\r\n"), GetTickCount()));
