...
//builderrssi:ar6000_get_target_stats() RSSI
A_STATUS
CAR6KMini::ar6000_get_target_stats()
{
	...
	logPrintf( debugFileLog, "AR6K: ERROR - No TGT_GETSTATS event after %u ms\n", AR6K_DEFAULT_MS_TO_WAIT_FOR_TGT_GETSTATS);
	if( targetfailEvent )
         ...
         )
{
 ...
 RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() start ==>\r\n"), GetTickCount()));
...
//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() 0\r\n")));
...
#else /*!UNDER_CE*/
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() 0.0\r\n"), GetTickCount()));
...
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() 0\r\n"), GetTickCount()));
...
// RETAILMSG(1, (TEXT("builder:ar6k2:sysPwrState dec=%d, hex=%x\n"), sysPwrState, sysPwrState));
	
    if (*pDevPowerState == NdisDeviceStateD3)
    {
    	m_Config.suspending = 1;
        ...
        else
        {
			...
			//NdisMSleep(100000); //++builder: need flushing time?
                }

//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() 1.1\r\n")));
                if ((m_ConnectInProgress) || (m_Connected)) {
                    NdisResetEvent (&m_SuspendEvent);
                    m_WantToBeConnected = FALSE;
                    SendWMIDisconnectCommand ();
//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() 1.2\r\n")));					
                    NdisWaitEvent (&m_SuspendEvent, NDIS_SUSPEND_TIMEOUT_INTERVAL);
                }
//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() 1.3\r\n")));

                if (pwrMgmtMode == POWER_MANAGEMENT_MODE_WLAN_DEEP_SLEEP) {
			RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan deep sleep\r\n")));
		...
		if( sysPwrState == POWER_STATE_RESET )
			{
			RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan halt\r\n")));
			logPrintf( debugFileLog, "builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan halt\r\n");
			}
		else
			{
//			wmi_scanparams_cmd((wmi_t *)m_pWMI,0xFFFF,0,0,0,0,0,0,0,0,0);
//			NdisMSleep(100000); //++builder: need flushing time?		
			//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan reset 0\r\n")));
			StopEndpoints (TRUE);
			//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan reset 1\r\n")));
//                    if (pwrMgmtMode == POWER_MANAGEMENT_MODE_CUT_PWR) {
			...
			//RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan reset 2\r\n")));
                        ...
                        //NdisMSleep(100000); //++builder: need sdio flushing time?
                        //plat_suspend ();
				RETAILMSG(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan off\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan off\r\n");
...
//                    } 
//			else {
//                    		RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD3, wlan warm reset\r\n")));
...
//                    }
    ...
    m_Config.suspending = 0;
    	//RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:SetPnpPower() NdisDeviceStateD0\r\n")));
    	RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() NdisDeviceStateD0, wlan on\r\n"), GetTickCount()));
	logPrintf( debugFileLog, "builder:ar6k2:SetPnpPower() NdisDeviceStateD0\r\n" );
        if ((m_Config.wowEnable) &&
            (m_bIsSuspend))
        {
            /*
...
//                if (pwrMgmtMode == POWER_MANAGEMENT_MODE_CUT_PWR) {
		   ...
		   //ar6000_reset_device (m_pHIFDevice, m_TargetType, FALSE);
//               }
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() wlan on 1\r\n"), GetTickCount()));
#endif
#if 1
                Init();
...
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() wlan on 2\r\n"), GetTickCount()));
               if (StartEndpoints(isColdBoot) != A_OK)
                {
                    Status = NDIS_STATUS_FAILURE;
                    logPrintf (ATH_LOG_INF, "WiFi ReEnabled and StartEndpont failed\n");
                }
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() wlan on 3\r\n"), GetTickCount()));
                if (TRUE == m_bIsSuspend &&
                    POWER_MANAGEMENT_MODE_CUT_PWR == pwrMgmtMode)
                {
                    if (preBss != NULL)
...
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() wlan on 4\r\n"), GetTickCount()));
        if (m_bIsSuspend) {
            m_bIsSuspend = FALSE;
        } else {
...
//RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() wlan on 5\r\n"), GetTickCount()));
    if (Status == NDIS_STATUS_SUCCESS)
    {
...
RETAILMSG(1, (TEXT("builder:ar6k2:%i SetPnpPower() <<end\r\n"), GetTickCount()));	
...
//
{
...

	{
		...
		RETAIL_DEBUG_PRINTF(m_Config.dbgBSSList, (TEXT("builder:ar6k2: bss list: SSID  = %s, RSSI = %d\r\n"), b, pBSSID->Rssi));
		//RETAILMSG(1, (TEXT("builder:ar6k2: bss list: SSID  = %s, RSSI = %d\r\n"), b, pBSSID->Rssi));
	}
...
//
{
	...
	A_UINT16 ch;
        ...
        this->Get802_11BSSIDList(pBSSIDListEx,dwSize,&pcbNeeded);
			...
			{
	//this->Get802_11BSSIDList(pBSSIDListEx,dwSize,&pcbNeeded);
		RETAILMSG(1, (TEXT("builder:ar6k2:SendWMIConnectCommand() pBSSIDListEx alloc failed\r\n")));
			}
        if(pBSSIDListEx) {
		...
		*/				
                if( (pBSSID->Ssid.SsidLength == SSID->SsidLength) &&
		     (memcmp(pBSSID->Ssid.Ssid, SSID->Ssid, SSID->SsidLength) == 0) )
                {
                    memcpy( sSSID, SSID->Ssid, SSID->SsidLength );
		    sSSID[SSID->SsidLength] = '\0';
		    RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: connect find RSSI[%d] SSID[%S], BSSID[%02X:%02X:%02X:%02X:%02X:%02X] CH[%d]\r\n"), pBSSID->Rssi, sSSID,
				pBSSID->MacAddress[0], pBSSID->MacAddress[1], pBSSID->MacAddress[2], 
				pBSSID->MacAddress[3], pBSSID->MacAddress[4], pBSSID->MacAddress[5], (pBSSID->Configuration.DSConfig - 2407000) / 5000));
		    logPrintf( debugFileLog, "builder:ar6k2: connect find RSSI[%d] SSID[%s], BSSID[%02X:%02X:%02X:%02X:%02X:%02X], CH[%d]\r\n", pBSSID->Rssi, sSSID,
				pBSSID->MacAddress[0], pBSSID->MacAddress[1], pBSSID->MacAddress[2], 
				pBSSID->MacAddress[3], pBSSID->MacAddress[4], pBSSID->MacAddress[5], (pBSSID->Configuration.DSConfig - 2407000) / 5000 );
		    ch = A_UINT16((pBSSID->Configuration.DSConfig - 2407000) / 5000);
		    if( m_ChannelLast != 0 && m_ChannelLast == ch )
		    	{
                    	if (pBSSID->Rssi > rssi) 
				{
                        	rssi = pBSSID->Rssi;
				memcpy( m_DesiredAPBSSID, pBSSID->MacAddress, sizeof(NDIS_802_11_MAC_ADDRESS) );
				m_ChannelHint = A_UINT16((pBSSID->Configuration.DSConfig - 2407000) / 5000);
			    	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: found AP saved channel\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2: found AP saved channel\r\n" );
                    		}
                        }
		    if( m_ChannelLast == 0 )
		    	{
    ...
    //
    //-builder GetChannelHint (SSID);
    //m_ChannelHint = 0; //builderrssi

#ifndef MMC_PATCH_CHANNEL_ASSOC
	m_ChannelHint = 0;
#endif

#ifndef MMC_PATCH_BSSID_ASSOC
	memset(m_DesiredAPBSSID, 0xFF, sizeof(m_DesiredAPBSSID)); //+builderrssi
#endif
    ...
    //builderrssi:set BSSID to connect
    if (!IsBroadcastAddress (m_DesiredAPBSSID))
    {
        pAPBSSID  = m_DesiredAPBSSID;
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: set BSSID[%02X:%02X:%02X:%02X:%02X:%02X]\r\n"),
				m_DesiredAPBSSID[0], m_DesiredAPBSSID[1], m_DesiredAPBSSID[2],
				m_DesiredAPBSSID[3], m_DesiredAPBSSID[4], m_DesiredAPBSSID[5]));
    logPrintf( debugFileLog, "builder:ar6k2: set BSSID[%02X:%02X:%02X:%02X:%02X:%02X]\r\n",
				m_DesiredAPBSSID[0], m_DesiredAPBSSID[1], m_DesiredAPBSSID[2], 
				m_DesiredAPBSSID[3], m_DesiredAPBSSID[4], m_DesiredAPBSSID[5] );
    }
...
if( (m_Config.probeEnable == 0) && (SSID->SsidLength > 0) )
	{
	wmi_probedSsid_cmd((struct wmi_t *)m_pWMI, 1, SPECIFIC_SSID_FLAG, (A_UINT8)SSID->SsidLength, SSID->Ssid);
	}
#endif
    ...
    if (wmiStatus != A_OK) {
        NDIS_DEBUG_PRINTF(ATH_LOG_ERR,"AR6K: ERROR - wmi_connect_cmd failed = %u\n", wmiStatus);
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: wmi_connect_cmd failed = %u\r\n"), wmiStatus));
	logPrintf( debugFileLog, "builder:ar6k2: wmi_connect_cmd failed = %u\r\n", wmiStatus);
        Status = NDIS_STATUS_FAILURE;
    } else {
    	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: wmi_connect_cmd success\r\n")));
	logPrintf( debugFileLog, "builder:ar6k2: wmi_connect_cmd success\r\n" );
...
//
{
	...
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: wmi_disconnect_cmd failed = %u\r\n"), wmiStatus));
	logPrintf( debugFileLog, "builder:ar6k2: wmi_disconnect_cmd failed = %u\r\n", wmiStatus);
        Status = NDIS_STATUS_FAILURE;
    }
...
//
{
    ...
    NDIS_802_11_BSSID_LIST_EX *pBSSIDListEx = NULL;
#ifdef WAPI_ENABLE
    NDIS_802_11_BSSID_LIST_EX *pBSSIDListExCheck = NULL;
#endif
	char sSSID[512];
    ...
    if (m_CounterMeasureOn)
    	{
    	RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2: m_CounterMeasureOn, SSID set ignore!!\r\n")) );
	logPrintf( debugFileLog, "builder:ar6k2: m_CounterMeasureOn, SSID set ignore!!\r\n" );
        goto done;
    	}
	...
	RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2: Bad SSID, ignore\r\n")) );
	logPrintf( debugFileLog, "builder:ar6k2: Bad SSID, ignore\r\n" );
        goto done;
    }
	...
	RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2: SSID length is 0, connect to any available AP\r\n")) );
	logPrintf( debugFileLog, "builder:ar6k2: SSID length is 0, connect to any available AP\r\n" );
...
/*		
		char a[128];
		wchar_t b[128] = {0};
		memcpy( a, SSID->Ssid, SSID->SsidLength );
		a[SSID->SsidLength] = '\0';
		MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, a, 32, b, 64 );
*/		
	if( m_WantToBeConnected )
		...
		memcpy( sSSID, SSID->Ssid, SSID->SsidLength );
		sSSID[SSID->SsidLength] = '\0';
		RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: set SSID = %S\r\n"), sSSID));
		logPrintf( debugFileLog, "builder:ar6k2: set SSID = %s\r\n", sSSID );
		}

    if (FALSE == m_WantToBeConnected)
    {
#ifdef WAPI_ENABLE
	//IvanChen added for WIFI/WAPI coexist
	//WAPI connect will protected when WZC try to clear the connect status
	//but open connect which even from WAPI configuration UI will be cleared.
	if(1)
	{
	        ULONG dwSize = 2048;
	        ULONG NoOfItems;
	        ULONG pcbNeeded=0;
		 NDIS_802_11_SSID current_SSID;
		 NDIS_802_11_SSID *pcuSSID;
		 pcuSSID = &current_SSID;
    		 this->Get802_11SSID(pcuSSID);

		 if(pcuSSID->SsidLength)
		 {
		        pBSSIDListExCheck = (NDIS_802_11_BSSID_LIST_EX *)A_MALLOC(dwSize);
		        this->Get802_11BSSIDList(pBSSIDListExCheck,dwSize,&pcbNeeded);
			 if((pcbNeeded==0)||(pcbNeeded>dwSize))
			 {
			 	A_FREE(pBSSIDListExCheck);
				if (0 == pcbNeeded)
				{
					dwSize = 20480;
				}
				...
				{
					dwSize = 4096 * ((pcbNeeded-1)/4096 + 1);
				}
				 pBSSIDListExCheck = (NDIS_802_11_BSSID_LIST_EX *)A_MALLOC(dwSize);
				 this->Get802_11BSSIDList(pBSSIDListExCheck,dwSize,&pcbNeeded);
			 }

		        if(pBSSIDListExCheck) {
		            NoOfItems = pBSSIDListExCheck->NumberOfItems;
		            NDIS_WLAN_BSSID_EX *pBSSID = (NDIS_WLAN_BSSID_EX *) &pBSSIDListExCheck->Bssid[0];
			     ULONG check_first=NoOfItems;
			     NDIS_WLAN_BSSID_EX *check_pBSSID=pBSSID;
			     while(check_first)
			     {
		                if (check_pBSSID->Ssid.SsidLength)
		                {
		                	if((pcuSSID->SsidLength==check_pBSSID->Ssid.SsidLength)&&
						(!memcmp(pcuSSID->Ssid,check_pBSSID->Ssid.Ssid,pcuSSID->SsidLength)))
		                	{
						break;
		                	}
		                }
		                check_pBSSID = (NDIS_WLAN_BSSID_EX *)((PBYTE)check_pBSSID + check_pBSSID->Length);
		                check_first--;
			     }
			     if(!check_first)
			     {
			         RETAILMSG(1,(L"List the current associated AP failed, try again......\r\n"));
			     	  this->Set802_11BSSIDListScan();
		        	  this->Get802_11BSSIDList(pBSSIDListExCheck,dwSize,&pcbNeeded);
				 if((pcbNeeded==0)||(pcbNeeded>dwSize))
				 {
				 	A_FREE(pBSSIDListExCheck);
				  ...
				  NoOfItems = 0;
		        	  if(pBSSIDListExCheck) {
		            		NoOfItems = pBSSIDListExCheck->NumberOfItems;
		            		pBSSID = (NDIS_WLAN_BSSID_EX *) &pBSSIDListExCheck->Bssid[0];
		        	  }
			     }
		            while (NoOfItems) {
		                // check all the ap with security
		                if ((pBSSID->Privacy) &&
		                    (pBSSID->Ssid.SsidLength))
		                {
					int l = pBSSID->IELength;
					BYTE* pe = pBSSID->IEs + l;
					BYTE* p = pBSSID->IEs + sizeof(NDIS_802_11_FIXED_IEs);
					while (p < pe-3)
					{
						if (68==p[0])//id
						{
							//RETAILMSG(1,(L"=bb==%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c===\r\n",
							//	pBSSID->Ssid.Ssid[0],pBSSID->Ssid.Ssid[1],pBSSID->Ssid.Ssid[2],pBSSID->Ssid.Ssid[3],
							//	pBSSID->Ssid.Ssid[4],pBSSID->Ssid.Ssid[5],pBSSID->Ssid.Ssid[6],pBSSID->Ssid.Ssid[7],
							//	pBSSID->Ssid.Ssid[8],pBSSID->Ssid.Ssid[9],pBSSID->Ssid.Ssid[10],pBSSID->Ssid.Ssid[11],
							//	pBSSID->Ssid.Ssid[12],pBSSID->Ssid.Ssid[13],pBSSID->Ssid.Ssid[14],pBSSID->Ssid.Ssid[15]));
							if((pcuSSID->SsidLength==pBSSID->Ssid.SsidLength)&&
								(!memcmp(pcuSSID->Ssid,pBSSID->Ssid.Ssid,pcuSSID->SsidLength)))
							{
								RETAILMSG(1,(L"WAPI connected, WZC disconnect ignore!\r\n"));
								goto done;
							}
						}	
						p += p[1] + 2;
        ...
        if (m_ConnectInProgress || m_Connected)
        {
        	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:SET_SSID:send disconnect cmd\r\n")));
		logPrintf( debugFileLog, "builder:ar6k2:SET_SSID:send disconnect cmd\r\n" );
            Status = SendWMIDisconnectCommand();
        }
    }
    else
    {
        ...
        if ((m_ConnectInProgress) || (m_Connected))
        {
        	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:SET_SSID: set %S, go to reconnect\r\n"), lSSID->Ssid));
		logPrintf( debugFileLog, "builder:ar6k2:SET_SSID: set %s, go to reconnect\r\n", lSSID->Ssid );
            SendWMIDisconnectCommand ();
        } else {
        	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:SET_SSID: set %S, send connect cmd\r\n"), lSSID->Ssid));
		logPrintf( debugFileLog, "builder:ar6k2:SET_SSID: set %s, send connect cmd\r\n", lSSID->Ssid );
...
//
{
    ...
    {
    	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: Set802_11BSSID Long Scan skip, now connection stats %d %d\r\n"), m_ConnectInProgress,m_Connected ));
	logPrintf( debugFileLog, "builder:ar6k2: Set802_11BSSID Long Scan skip, now connection stats %d %d\r\n", m_ConnectInProgress,m_Connected );
        goto done;
    }
#ifdef NO_BCAST_PROBE_IN_CONNECT
    wmi_probedSsid_cmd((struct wmi_t *)m_pWMI, 0, ANY_SSID_FLAG, 0,NULL);
	...
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2: Set802_11BSSID Long Scan Start\r\n")));
...
{
	char auths[9][24] = { "open", "Shared", "AutoSwitch", "WPA", "WPAPSK", "WPANone", "WPA2", "WPA2PSK", "Max" };
    if ( m_AuthenticationMode == Ndis802_11AuthModeAutoSwitch ) {
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:set Authentication Mode: Ndis802_11AuthModeAutoSwitch not supported!!! \r\n")));
        return NDIS_STATUS_NOT_SUPPORTED;
    }
    if ((Mode == Ndis802_11AuthModeOpen) || (Mode == Ndis802_11AuthModeShared) ||
	    ...
	    RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:set Authentication Mode = %S\r\n"), auths[Mode]));
	    logPrintf( debugFileLog, "builder:ar6k2:set Authentication Mode = %s\r\n", auths[Mode] );
            return NDIS_STATUS_SUCCESS;
    } else {
        return NDIS_STATUS_INVALID_DATA;
    }
                          ...
                          IN  ULONG              cbBuffer)
{
	...
	RETAILMSG(1, (TEXT("builder:ar6k2:Set802_11PMKID()\r\n")));
	logPrintf( debugFileLog, "builder:ar6k2:Set802_11PMKID()\r\n" );
...
RETAILMSG(1, (TEXT("builder:ar6k2:Set802_11RoamControl() +++\r\n")));
    ...
    if (Ndis802_11NicRoamControlOff == *pCtl)
    {
		...
		RETAILMSG(1, (TEXT("builder:ar6k2:Set802_11RoamControl() osroam diabled\r\n")));
	...
	RETAILMSG(1, (TEXT("builder:ar6k2:Set802_11RoamControl() osroam enabled\r\n")));
