...
//
{
    ...
    if (NDIS_STATUS_SUCCESS == Status)
    {
        ...
        if(m_networkType != AP_NETWORK) {
		    ...
		    //builder:debug print eapol
		    for( i=0; i<32; i++ )
		    	{
		    	sprintf( buf+(i*2), "%02x", epKeyWpa->keyNonce[i] );
		    	}
		    RETAILMSG(1, (TEXT("builderd:ar6k2:TX eapol: WPA Key Nonce= %S\r\n"), buf));
	   	    logPrintf( debugFileLog, "builder:ar6k2:TX eapol: WPA Key Nonce= %s\r\n", buf );
		    for( i=0; i<16; i++ )
		    	{
		    	sprintf( buf+(i*2), "%02x", epKeyWpa->keyIV[i] );
		    	}
		    RETAILMSG(1, (TEXT("builderd:ar6k2:TX eapol: WPA Key IV= %S\r\n"), buf));
    		    logPrintf( debugFileLog, "builder:ar6k2:TX eapol: WPA Key IV= %s\r\n", buf );
		    for( i=0; i<16; i++ )
		    	{
		    	sprintf( buf+(i*2), "%02x", epKeyWpa->keyMIC[i] );
		    	}
		    RETAILMSG(1, (TEXT("builderd:ar6k2:TX eapol: WPA Key MIC= %S\r\n"), buf));
    		    logPrintf( debugFileLog, "builder:ar6k2:TX eapol: WPA Key MIC= %s\r\n", buf );
		...
		//RETAILMSG(1, (TEXT("builderd:ar6k2:TXPacket = %x\r\n"), (A_UINT32)pPacket));
        }
	else
		{
		//RETAILMSG(1, (TEXT("builderd:ar6k2:TXPacket = %x\r failed\n"), (A_UINT32)pPacket));
		}
    ...
    if (NDIS_STATUS_PENDING != Status)
    {
		...
		//RETAILMSG(1, (TEXT("builderd:ar6k2:TXCompletePacket = %x\r\n"), (A_UINT32)pPacket));
            }
...
//
{
	...
	//RETAILMSG(1, (TEXT("builderd:ar6k2:HTCDataSendCompleted() return to tx buffer list\r\n")));
        TransmitBufferListInsertHead(pTxBuffer);
    ...
    if (pNdisPacket)
    {
    	//RETAILMSG(1, (TEXT("builderd:ar6k2:HTCDataSendCompleted() send another\r\n")));
...
//
{
	...
	//RETAILMSG(1, (TEXT("builderd:ar6k2:PendingTXPacket = %x\r\n"), (A_UINT32)pNdisPacket));
    }
}
