...
CAR6KMini::Init()
{
	...
	m_RoamTimeout = FALSE; //builderrssi
...
//
{
    ...
    if (pPacket->Status == A_OK) {
        ...
        //-builderorg wmi_control_rx((wmi_t *)m_pWMI, pb);
        //builderbb:add ssid filter
        wmi_control_rx((wmi_t *)m_pWMI, pb, WMI_11G_MODE, m_Config.lockSSID); //+buildertemp
...
//
{
	//buildercheck:control tx buffer free
//    RETAILMSG(1, (TEXT("builderd:ar6k2:free = %x\r\n"), (A_UINT32)cookie));
...
//
{
	...
	if( m_Config.tp > 0 )
		...
		RETAILMSG( 1, (TEXT("builder:ar6k2:set thread priority = %d, org priority = %d\r\n"), m_Config.tp, priority) );
		logPrintf( debugFileLog, "builder:ar6k2:set thread priority = %d, org priority = %d\r\n", m_Config.tp, priority );
		CeSetThreadPriority(GetCurrentThread(),m_Config.tp);
		}
...
//	RETAILMSG(1, (TEXT("builderd:ar6k2:%d %d %d %d\r\n"), m_ControlEp, arAc2EndpointID(WMM_AC_BE),
//					arAc2EndpointID(WMM_AC_BK), arAc2EndpointID(WMM_AC_VO)));
...
//		RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2:configTargetParams() power save mode disabled\r\n")) );
//		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() power save mode disabled\r\n" );
                break;
            case Ndis802_11PowerModeMAX_PSP:

                m_powerMode = REC_POWER;
//		RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2:configTargetParams() max power save mode\r\n")) );
//		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() max power save mode\r\n" );
                break;
            case Ndis802_11PowerModeFast_PSP:

                m_powerMode = REC_POWER;
//		RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2:configTargetParams() normal power save mode\r\n")) );
//		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() normal power save mode\r\n" );
                break;
            default:
 ...
 //   status = configTargetParams();  //-builder
...
*/
//RETAILMSG(1, (TEXT("builder:ar6k2:StopEndpoints() start>>\r\n")));
...
//RETAILMSG(1, (TEXT("builder:ar6k2:StopEndpoints() 0>>\r\n")));
    if (!m_Config.byPassWmi)
    {
        if (m_pWMI)
        {
...
RETAILMSG(1, (TEXT("builder:ar6k2:StopEndpoints() <<end\r\n")));
...
void CAR6KMini::configChannelParams()
{
	A_STATUS   wmiStatus = A_OK;

	wmiStatus = wmi_set_channelParams_cmd((wmi_t *)m_pWMI, 1, WMI_11G_MODE, m_Config.channel_cnt, m_Config.channelList);
	//wmiStatus = wmi_set_channelParams_cmd((wmi_t *)m_pWMI, 1, WMI_11GONLY_MODE, m_Config.channel_cnt, m_Config.channelList);

	if( wmiStatus != A_OK )
		{
		RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() channel setting failed!!!\r\n")));
		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() channel setting failed!!! \r\n" );
		}
#if 0
	if( m_Config.channel_cnt > 0 )
	{
	if( wmiStatus == A_OK )
		{
		int chno, freq, i;
		char ieee;
		RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configChannelParams() Setting channels =======> [%2d]\r\n"), m_Config.channel_cnt));
		logPrintf( debugFileLog, "builder:ar6k2:configChannelParams() Setting channels =======> [%2d]\r\n", m_Config.channel_cnt );
		for( i=0; i<m_Config.channel_cnt; i++ )
			{
			freq = m_Config.channelList[i];
			if( freq > 5000 )
				{
				ieee = 'A';
				chno = (freq - 5000) / 5;
			...
			RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:[%2d][11%c] %2d (%4dMhz)\r\n"), i+1, ieee, chno, freq ));
			logPrintf( debugFileLog, "builder:ar6k2:[%2d][11%c] %2d (%4dMhz)\r\n", i+1, ieee, chno, freq );
			}
		}
	else
		{
	...
	{
		RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() use default channel setting\r\n")));
		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() use default channel setting\r\n" );
	}
#endif	
}


//buildercheck:configTargetParams()
...
CAR6KMini::configTargetParams()
{
	...
	debugBssInfo = m_Config.dbgBSSEvent; //buildercheck
	debugFileLog = m_Config.dbgFileLog; //buildercheck
	debugErrorLog = m_Config.dbgErrorLog; //buildercheck

	m_Config.suspending = 0;
	...
	if( nodeage < (m_Config.bkScanEnable*1000) )
		nodeage = m_Config.bkScanEnable*1000 + 1000;
	
        wmi_set_nodeage((wmi_t *)m_pWMI, nodeage);
		
        if (m_Config.bkScanEnable) {
            bkenable = (A_UINT16)m_Config.bkScanPeriod;
//		RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2:configTargetParams() background scan enabled, period = %dsecs\r\n"), bkenable) );
//		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() background scan enabled, period = %dsecs\r\n", bkenable );
        } else {
            bkenable = 0xFFFF;
//		RETAIL_DEBUG_PRINTF( 1 , (TEXT("builder:ar6k2:configTargetParams() background scan disabled \r\n")) );
//		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() background scan disabled \r\n" );
        }
...
#define CHDWELL 100
	...
	chdwell = (A_UINT16)m_Config.DwellTime;
        wmiStatus = wmi_scanparams_cmd((wmi_t *)m_pWMI,fgenable,0,bkenable,
                                        chdwell,chdwell,chdwell,WMI_SHORTSCANRATIO_DEFAULT, DEFAULT_SCAN_CTRL_FLAGS, 0, 0);
	...
	wmi_bmisstime_cmd((wmi_t *)m_pWMI, 0, (A_UINT16)m_Config.BmissCount);  //+builder

	configChannelParams();

	//builder: for hidden ssid
	if( m_Config.probeEnable == 1 )
		{
		wmi_probedSsid_cmd((struct wmi_t *)m_pWMI, 1, SPECIFIC_SSID_FLAG,strlen(m_Config.probeSSID), (A_UCHAR*)m_Config.probeSSID);
		wmi_probedSsid_cmd((struct wmi_t *)m_pWMI, 2, SPECIFIC_SSID_FLAG,strlen(m_Config.probeSSIDa), (A_UCHAR*)m_Config.probeSSIDa);
		//RETAILMSG(1, (TEXT("builder:ar6k2: probe hidden SSID enabled probeSSID=%S, length=%d\r\n"), m_Config.probeSSID, strlen(m_Config.probeSSID)));
		}
/*
	if( m_Config.dbgFileLog == 1 )
		{
		RETAILMSG(1, (TEXT("builder:ar6k2: dbgFileLog enabled, dbgLogPath=%S\r\n"), m_Config.dbgLogFName));
		}
*/

	if( m_Config.lockEnable == 0 )
		m_Config.lockSSID[0] = '\0';
	
m_powerMode = MAX_PERF_POWER;
wmi_powermode_cmd((struct wmi_t *)m_pWMI, m_powerMode);
...
//		logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() normal power save mode\r\n" );
		...
		if (A_OK == wmiStatus) {
			if(m_80211_PowerMode == Ndis802_11PowerModeMAX_PSP) {
				wmi_pmparams_cmd((struct wmi_t *)m_pWMI, 100, 0, 3); /* 3:stick, 2:normal, 1:ignore */
				m_ListenInterval = NDIS_MAXPSP_LISTEN_INTERVAL;
				wmi_listeninterval_cmd((struct wmi_t *)m_pWMI, m_ListenInterval, 0);
			}
			else {
				wmi_pmparams_cmd((struct wmi_t *)m_pWMI, 100, 0, 2); /* 3:stick, 2:normal, 1:ignore */
				m_ListenInterval = NDIS_FASTPSP_LISTEN_INTERVAL;
				wmi_listeninterval_cmd((struct wmi_t *)m_pWMI, m_ListenInterval, 0);
			}
		}
			...
			{
			RETAILMSG(1, (TEXT("builder:ar6k2:configTargetParams():wmi_powermode_cmd fail !!! \r\n")));
			logPrintf( debugFileLog, "builder:ar6k2:configTargetParams():wmi_powermode_cmd fail !!! \r\n");
			}
        }
#endif
...
#if 1
		//buildercheck
		if( m_Config.RoamEnable && (m_Config.hostAssistedRoaming == 0))
			{
			WMI_SET_ROAM_CTRL_CMD cmd;
			int size = sizeof(WMI_SET_ROAM_CTRL_CMD);
			int roamfloor;

			memset(&cmd, 0x00, sizeof(WMI_SET_ROAM_CTRL_CMD));
			cmd.roamCtrlType = WMI_SET_LOWRSSI_SCAN_PARAMS;			
			// roam scan period = %d, roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d;
			cmd.info.lrScanParams.lowrssi_scan_period = 1; /* may be sec */
			cmd.info.lrScanParams.lowrssi_scan_threshold = -((A_INT16)m_Config.ScanThreshold);
			cmd.info.lrScanParams.lowrssi_roam_threshold = -((A_INT16)m_Config.RoamThreshold);
			cmd.info.lrScanParams.roam_rssi_floor = -((A_INT8)m_Config.RoamFloorThre);
			roamfloor = -((A_INT16)m_Config.RoamFloorThre);
		
//			RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() firmware roam enabled\r\n")));
//			logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() firmware roam enabled\r\n" );
				
//			RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams(): roam scan period = %d, roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n"), 
//					cmd.info.lrScanParams.lowrssi_scan_period, cmd.info.lrScanParams.lowrssi_scan_threshold,
//					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor));

//			logPrintf( debugFileLog, "builder:ar6k2:configTargetParams(): roam scan period = %d, roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n", 
//					cmd.info.lrScanParams.lowrssi_scan_period, cmd.info.lrScanParams.lowrssi_scan_threshold,
//					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor );

			wmiStatus = wmi_set_roam_ctrl_cmd((struct wmi_t *)m_pWMI, &cmd, size);


			if (A_OK != wmiStatus)
				{
				RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() roam parameter set failed\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() roam parameter set failed\r\n" );
				}

			memset(&cmd, 0x00, sizeof(WMI_SET_ROAM_CTRL_CMD));
			cmd.roamCtrlType = WMI_SET_ROAM_MODE;
			cmd.info.roamMode = WMI_DEFAULT_ROAM_MODE;
			cmd.info.bssBiasInfo.numBss = 0;

			wmiStatus = wmi_set_roam_ctrl_cmd((struct wmi_t *)m_pWMI, &cmd, size);

			if (A_OK != wmiStatus)
				{
				RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() roam mode set failed\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() roam mode set failed\r\n" );
				}
			}
		else if( ((m_Config.hostAssistedRoaming==1) || (m_Config.hostAssistedRoaming==2)) && (m_Config.RoamEnable == 0) )
			{
			WMI_SET_ROAM_CTRL_CMD cmd;
			int size = sizeof(WMI_SET_ROAM_CTRL_CMD);
			int roamfloor;
			...
			if( m_Config.hostAssistedRoaming == 1 )
				{
				RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() RoamFloorThre driver roam enabled\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() RoamFloorThre driver roam enabled\r\n" );
				}
			else
				{
				RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() bestBSS driver roam enabled\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() bestBSS driver roam enabled\r\n" );
				}

			RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams(): roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n"), 
					cmd.info.lrScanParams.lowrssi_scan_threshold,
					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor));

			logPrintf( debugFileLog, "builder:ar6k2:configTargetParams(): roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n", 
					cmd.info.lrScanParams.lowrssi_scan_threshold,
					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor );
#endif			
			}
			...
			{
			RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:configTargetParams() firmware, driver roam disabled\r\n")));
			logPrintf( debugFileLog, "builder:ar6k2:configTargetParams() firmware, driver roam disabled\r\n" );
			}
#endif		
#endif		
...
CAR6KMini::ResetPowerWorkItem()
{
	...
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k:ResetPowerWorkItem() - m_Halting = %d, powerState = %d\r\n"), m_Halting, powerState));
#ifdef WINDOWS_MOBILE
        if (m_Halting == FALSE) {
...
CAR6KMini::TargetInitialize(void)
{
    ...
    RETAIL_DEBUG_PRINTF( 1, (TEXT("builder:ar6k2: FIRMWARE VERSION ==> %X\r\n"), m_TargetId ));
...
//
{
	...
	if( m_Config.dbgFileLog )
		{
		logInit( m_Config.dbgLogFName );  //+builder
		}
	...
	RETAILMSG( 1, (TEXT("\ntest==========================================COMMON MODE\r\nbuilder:ar6k2:Initialize() mac address = [%02X:%02X:%02X:%02X:%02X:%02X]\r\n"), 
		m_PermanentAddress[0], m_PermanentAddress[1], m_PermanentAddress[2],
		m_PermanentAddress[3], m_PermanentAddress[4], m_PermanentAddress[5]) );
	logPrintf( debugFileLog, "builder:ar6k2:Initialize() mac address = [%02X:%02X:%02X:%02X:%02X:%02X]\r\n", 
		m_PermanentAddress[0], m_PermanentAddress[1], m_PermanentAddress[2],
		m_PermanentAddress[3], m_PermanentAddress[4], m_PermanentAddress[5] );

#ifdef MMC_PATCH_BLOCK_OTHER_MAC
	if( m_PermanentAddress[0] != 0x00 || m_PermanentAddress[1] != 0x30 || m_PermanentAddress[2] != 0x0d )
		{
		Status = NDIS_STATUS_FAILURE;
		RETAILMSG( 1, (TEXT("builder:ar6k2:other's WLAN device!!!, this driver for MMC WLAN only!!!\r\n")) );
		logPrintf( debugFileLog, "builder:ar6k2:other's WLAN device!!!, this driver for MMC WLAN only!!!\r\n" );
		goto done;
		}

	RETAILMSG( 1, (TEXT("builder:ar6k2:Initialize() driver ver = %d.%d.%d.%d(b%d), date = %s \r\n"),
		__VER_MAJOR_, __VER_MINOR_, __VER_PATCH_, __BUILD_NUMBER_, MMC_BUILD_NUM, MMC_BUILD_DATE ));
	logPrintf( debugFileLog, "builder:ar6k2:Initialize() driver ver = %d.%d.%d.%d(b%d), date = %s \r\n",
		__VER_MAJOR_, __VER_MINOR_, __VER_PATCH_, __BUILD_NUMBER_, MMC_BUILD_NUM, MMC_BUILD_DATEs );
#else
	RETAILMSG(1, (TEXT("builder:ar6k2:MMC_PATCH_BLOCK_OTHER_MAC not defined!!!\r\n")));
#endif

    m_InitComplete = TRUE;


//builder: print configTargetParams

	//builder: print registry channel setting
	if( m_Config.channel_cnt > 0 )
		{
		int chno, freq, i;
		char ieee;
		RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() Setting channels =======> [%2d]\r\n"), m_Config.channel_cnt));
		logPrintf( debugFileLog, "builder:ar6k2:Initialize() Setting channels =======> [%2d]\r\n", m_Config.channel_cnt );
		for( i=0; i<m_Config.channel_cnt; i++ )
			{
			freq = m_Config.channelList[i];
			if( freq > 5000 )
			...
			RETAILMSG(1, (TEXT("builder:ar6k2:[%2d][11%c] %2d (%4dMhz)\r\n"), i+1, ieee, chno, freq ));
			logPrintf( debugFileLog, "builder:ar6k2:[%2d][11%c] %2d (%4dMhz)\r\n", i+1, ieee, chno, freq );
			}
		}
	else
		{
		RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() use default channel setting\r\n")));
		logPrintf( debugFileLog, "builder:ar6k2:Initialize() use default channel setting\r\n" );
		}

	//builder: print power save mode setting
	switch( m_80211_PowerMode )
		{
		case Ndis802_11PowerModeCAM:
			RETAILMSG( 1 , (TEXT("builder:ar6k2:Initialize() power save mode disabled\r\n")) );
			logPrintf( debugFileLog, "builder:ar6k2:Initialize() power save mode disabled\r\n" );
                break;
		case Ndis802_11PowerModeMAX_PSP:
			RETAILMSG( 1 , (TEXT("builder:ar6k2:Initialize() max power save mode\r\n")) );
			logPrintf( debugFileLog, "builder:ar6k2:Initialize() max power save mode\r\n" );
                break;
		case Ndis802_11PowerModeFast_PSP:
			RETAILMSG( 1 , (TEXT("builder:ar6k2:Initialize() normal power save mode\r\n")) );
			logPrintf( debugFileLog, "builder:ar6k2:Initialize() normal power save mode\r\n" );
                break;
		default:
                	// this will never occur.
	                RETAILMSG( 1 , (TEXT("builder:ar6k2:Initialize() wrong power save mode setting\r\n")) );
			logPrintf( debugFileLog, "builder:ar6k2:Initialize() wrong power save mode setting\r\n" );
                break;
            }

	//builder: print back ground scan setting
	if( m_Config.bkScanEnable )
		{
		RETAILMSG( 1 , (TEXT("builder:ar6k2:Initialize() background scan enabled, period = %dsecs\r\n"), (A_UINT16)m_Config.bkScanPeriod) );
		logPrintf( debugFileLog, "builder:ar6k2:Initialize() background scan enabled, period = %dsecs\r\n", (A_UINT16)m_Config.bkScanPeriod );
        	} 
	else 
		{
		RETAILMSG( 1 , (TEXT("builder:ar6k2:Initialize() background scan disabled \r\n")) );
		logPrintf( debugFileLog, "builder:ar6k2:Initialize() background scan disabled \r\n" );
        	}

	//builder: print roaming setting
	if( m_Config.RoamEnable && (m_Config.hostAssistedRoaming == 0))
		{
			WMI_SET_ROAM_CTRL_CMD cmd;
			int size = sizeof(WMI_SET_ROAM_CTRL_CMD);
			...
			RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() firmware roam enabled\r\n")));
			logPrintf( debugFileLog, "builder:ar6k2:Initialize() firmware roam enabled\r\n" );
				
			RETAILMSG(1, (TEXT("builder:ar6k2:Initialize(): roam scan period = %d, roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n"), 
					cmd.info.lrScanParams.lowrssi_scan_period, cmd.info.lrScanParams.lowrssi_scan_threshold,
					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor));

			logPrintf( debugFileLog, "builder:ar6k2:Initialize(): roam scan period = %d, roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n", 
					cmd.info.lrScanParams.lowrssi_scan_period, cmd.info.lrScanParams.lowrssi_scan_threshold,
					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor );
		}
	else if( ((m_Config.hostAssistedRoaming==1) || (m_Config.hostAssistedRoaming==2)) && (m_Config.RoamEnable == 0) )
		{
			...
			if( m_Config.hostAssistedRoaming == 1 )
				{
				RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() RoamFloorThre driver roam enabled\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:Initialize() RoamFloorThre driver roam enabled\r\n" );
				}
			else
				{
				RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() bestBSS driver roam enabled\r\n")));
				logPrintf( debugFileLog, "builder:ar6k2:Initialize() bestBSS driver roam enabled\r\n" );
				}

			RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n"), 
					cmd.info.lrScanParams.lowrssi_scan_threshold,
					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor));

			logPrintf( debugFileLog, "builder:ar6k2:Initialize() roam scan threshold = %d, roam threshold = %d, roam rssi floor = %d\r\n", 
					cmd.info.lrScanParams.lowrssi_scan_threshold,
					cmd.info.lrScanParams.lowrssi_roam_threshold, roamfloor );
		}
	else
		{
		RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() firmware, driver roam disabled\r\n")));
		logPrintf( debugFileLog, "builder:ar6k2:Initialize() firmware, driver roam disabled\r\n" );
		}

	if( m_Config.probeEnable == 1 )
		{
		RETAILMSG(1, (TEXT("builder:ar6k2: probe hidden SSID enabled probeSSID=%S, length=%d\r\n"), m_Config.probeSSID, strlen(m_Config.probeSSID)));
		RETAILMSG(1, (TEXT("builder:ar6k2: probeSSIDa=%S, length=%d\r\n"), m_Config.probeSSIDa, strlen(m_Config.probeSSIDa)));
		logPrintf( debugFileLog, "builder:ar6k2: probe hidden SSID enabled probeSSID=%s, length=%d\r\n", m_Config.probeSSID, strlen(m_Config.probeSSID));
		logPrintf( debugFileLog, "builder:ar6k2: probeSSIDa=%s, length=%d\r\n", m_Config.probeSSIDa, strlen(m_Config.probeSSIDa));
		}

	if( m_Config.lockEnable == 1 )
		{
		RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() lockSSID = %S\r\n"), m_Config.lockSSID));
		logPrintf( debugFileLog, "builder:ar6k2:Initialize() lockSSID = %s\r\n", m_Config.lockSSID);
		}
	
	RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() scan ch dwell time = %d\r\n"), m_Config.DwellTime));
	logPrintf( debugFileLog, "builder:ar6k2:Initialize() scan ch dwell time = %d\r\n", m_Config.DwellTime);


	if( m_Config.nodeAge > 0 )
		{
		ULONG a;
		a = m_Config.nodeAge;
		if( a < (m_Config.bkScanEnable*1000) )
			a = m_Config.bkScanEnable*1000 + 1000;
		RETAILMSG(1, (TEXT("builder:ar6k2:Initialize() registry nodeage = %d, current set = %d\r\n"), m_Config.nodeAge, a ));

		logPrintf( debugFileLog, "builder:ar6k2:Initialize() registry nodeage = %d, current set = %d\r\n", m_Config.nodeAge, a );
		}

	targetfailEvent = CreateEvent(NULL, FALSE, FALSE, _T("WLAN_RE_INSERT"));
	...
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:Initialize() completed - status = %x\r\n"), Status));
...
//
{
	...
	RETAIL_DEBUG_PRINTF(1, (TEXT("builder:ar6k2:++>Halt()\r\n")));
	logPrintf( (debugFileLog && debugErrorLog), "builder:ar6k2: Halt() start\r\n" );	

 m_Config.suspending = 1; //builder
