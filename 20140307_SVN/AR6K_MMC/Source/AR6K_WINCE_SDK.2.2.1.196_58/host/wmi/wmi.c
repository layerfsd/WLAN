...
static A_STATUS wmi_bssInfo_event_rx(struct wmi_t *wmip, A_UINT8 *datap,
                                     int len, A_UINT8 phy_mode, char* lockssid);	   /* Bug 82893 */ //builder: add ssid lock
...
A_STATUS
wmi_control_rx(struct wmi_t *wmip, void *osbuf, A_UINT8 phy_mode, char* lockssid) /* Bug 82893 */ //builder: add ssid lock
{
    WMI_CMD_HDR *cmd;
    A_UINT16 id;
    A_UINT8 *datap;
    ...
    case (WMI_BSSINFO_EVENTID):
        ...
        status = wmi_bssInfo_event_rx(wmip, datap, len, phy_mode, lockssid);	/* Bug 82893 */ //builder: add ssid lock
	if (status == A_OK)
...
//RETAILMSG(1, (TEXT("builderd:ar6k2: alloc = %x\r\n"), osbuf));
...
static A_STATUS
wmi_bssInfo_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len, A_UINT8 phy_mode, char* lockssid)	/* Bug 82893 */ //bulder: add ssid lock
{
    bss_t *bss = NULL;
...
//builder: remove hidden SSID
	buf = datap + sizeof(WMI_BSS_INFO_HDR);
	beacon_ssid_len = buf[SSID_IE_LEN_INDEX+1]; //builder: no legnth, 0 is hidden
	if( (beacon_ssid_len == 0) ||(buf[SSID_IE_LEN_INDEX] == 0))
		return A_OK;
	
	//logPrintf( (debugFileLog && debugBssInfo), "builder:ar6k2: SSID length = %d\r\n", beacon_ssid_len );
...
/* ATHENV V9 --- */
...
#if 0   //builder: remove ssid cache      
...
#if 1  //++builder: remove ssid cache
	...
	if( strlen(lockssid) != 0 )
		{
		if( strcmp(lockssid, a) != 0 )
			{
			 wlan_node_free(bss);
			 return A_OK;
			}
...
/*		RETAILMSG(1, (TEXT("builder:ar6k2: add bss info, SSID  = %S, RSSI = %d, CHANNEL = %d\r\n"), a, bss->ni_rssi, ch));
		RETAILMSG(1, (TEXT("builder:ar6k2: BSSID = %02x:%02x:%02x:%02x:%02x:%02x \r\n") ,
					bih->bssid[0], bih->bssid[1], bih->bssid[2],
					bih->bssid[3], bih->bssid[4], bih->bssid[5]));
*/
			
		RETAIL_DEBUG_PRINTF(debugBssInfo, (TEXT("builder:ar6k2: add bss info, SSID  = %S[%d], RSSI = %d, CHANNEL = %d\r\n"), a, length, bss->ni_rssi, ch));
		RETAIL_DEBUG_PRINTF(debugBssInfo, (TEXT("builder:ar6k2: BSSID = %02x:%02x:%02x:%02x:%02x:%02x \r\n") ,
					bih->bssid[0], bih->bssid[1], bih->bssid[2],
					bih->bssid[3], bih->bssid[4], bih->bssid[5]));
		logPrintf( (debugFileLog && debugBssInfo), "builder:ar6k2: add bss info, SSID  = %s[%d], RSSI = %d, CHANNEL = %d\r\n", a, length, bss->ni_rssi, ch );
