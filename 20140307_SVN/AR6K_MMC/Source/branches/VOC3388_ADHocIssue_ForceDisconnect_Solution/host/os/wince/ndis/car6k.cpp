...
//
{
	...
	static DWORD dwLastDataPacketTick = 0;
    ...
    {
   		dwLastDataPacketTick = GetTickCount();
	...
	// Added By psh
	if( pAdapter->m_bDisconnectIfNoDataPacket )
	{
		if( (pAdapter->m_Connected) && (dwLastDataPacketTick != 0) )
		{

			DWORD dwCheckInterval = pAdapter->m_DisconnectIfNoDataPacketChkIntv * 1000;
			
			if( (GetTickCount() - dwLastDataPacketTick) > dwCheckInterval )
			{
				NDIS_802_11_MAC_ADDRESS curMacAddr;

				RETAILMSG(1, (TEXT("[PSH] Disconnect!!!!!!!!! ???????????\r\n")));
				RETAILMSG(1, (TEXT("[PSH] m_bDisconnectIfNoDataPacket : %d \r\n"), pAdapter->m_bDisconnectIfNoDataPacket));
				RETAILMSG(1, (TEXT("[PSH] m_DisconnectIfNoDataPacketChkIntv : %d \r\n"), pAdapter->m_DisconnectIfNoDataPacketChkIntv));
				
				dwLastDataPacketTick = 0;
				if( pAdapter->Get802_11AssociatedAPBSSID(&curMacAddr) == NDIS_STATUS_SUCCESS)
				{
					pAdapter->WMIDisconnectIndication(NO_NETWORK_AVAIL);
				}
			}
		}
		else
...
CAR6KMini::configRegistryParameters()
{
	...
	// Added By psh
	if(m_Config.DisconnectIfNoDataPkt)
	{
		RETAILMSG(1, (TEXT("[PSH] DisconnectIfNoDataPkt Enable \r\n")));
		m_bDisconnectIfNoDataPacket = TRUE;
		
		if(m_Config.DisconnectIfNoDataPktCheckIntv < 5)
		{
			m_Config.DisconnectIfNoDataPktCheckIntv = 5;
		}
		else if(m_Config.DisconnectIfNoDataPktCheckIntv > 180)
		{
			m_Config.DisconnectIfNoDataPktCheckIntv = 180;
		}
		
		m_DisconnectIfNoDataPacketChkIntv = m_Config.DisconnectIfNoDataPktCheckIntv;
	}
	else
	{
		RETAILMSG(1, (TEXT("[PSH] DisconnectIfNoDataPkt Disable \r\n")));
		m_bDisconnectIfNoDataPacket = FALSE;
		m_DisconnectIfNoDataPacketChkIntv = 0;
	}
