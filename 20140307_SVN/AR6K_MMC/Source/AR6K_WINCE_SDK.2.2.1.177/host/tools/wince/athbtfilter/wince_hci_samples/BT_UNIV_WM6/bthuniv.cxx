...
//#undef BTE_CLASS_AVDTP

//typedef int (*HCI_ISSUE_AFH_CALLBACK)(int ChannelNumber);

int IssueAFHCommand(int CurrentWLANChannel);
...
static CRITICAL_SECTION       g_WritePacketLock;
#endif
...
BOOL WINAPI DllMain(HANDLE hinstDLL, DWORD dwReason, LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        InitializeCriticalSection(&g_WritePacketLock);
#endif
        break;
...
static DWORD WINAPI HciUnivNotifyThread(PVOID pv)
{
	...
	while(ERROR_SUCCESS == RegEnumKeyEx(hkTrans, cdwIClass, szIClass, &dwIClassSize, NULL, NULL, NULL, NULL))
	{
		...
		PREFAST_ASSERT(cdwIClass < MAXIMUM_WAIT_OBJECTS);
...
int HCI_WritePacket (HCI_TYPE eType, BD_BUFFER *pBuff) 
{
    ...
    if (fInit && pfn && (eType == COMMAND_PACKET)) {
    ...
    if (fInit && pfn && (eType == DATA_PACKET_ACL)) {
...
#ifdef ATHBT_FILTER
    EnterCriticalSection(&g_WritePacketLock);
#endif
...
#ifdef ATHBT_FILTER
    LeaveCriticalSection(&g_WritePacketLock);
#endif
...
#define HCI_PACKET_HEADER_PAD  64
#define HCI_PACKET_TRAILER_PAD 64
#define AFH_CHANNEL_MAP_BYTES  10

#define AFH_COMMAND_SIZE       (AFH_CHANNEL_MAP_BYTES + 3)

static UCHAR        g_AFHCommandBuffer[HCI_PACKET_HEADER_PAD + AFH_COMMAND_SIZE + HCI_PACKET_TRAILER_PAD];
static BD_BUFFER    g_AFHHCIPacket;
static LONG         g_AFHCommandPending = 0;

typedef struct _WLAN_CHANNEL_MAP {
    UCHAR  Map[AFH_CHANNEL_MAP_BYTES];
} WLAN_CHANNEL_MAP;

#define MAX_WLAN_CHANNELS 14

WLAN_CHANNEL_MAP g_ChannelMapTable[MAX_WLAN_CHANNELS] = {
    { {0x00,0x00,0xC0,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x7F}},
    { {0x0F,0x00,0x00,0xF8,0xFF,0xFF,0xFF,0xFF,0xFF,0x7F}},
    { {0xFF,0x01,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0x7F}},
    { {0xFF,0x3F,0x00,0x00,0xE0,0xFF,0xFF,0xFF,0xFF,0x7F}},
    { {0xFF,0xFF,0x07,0x00,0x00,0xFC,0xFF,0xFF,0xFF,0x7F}},
    { {0xFF,0xFF,0xFF,0x00,0x00,0x80,0xFF,0xFF,0xFF,0x7F}},
    { {0xFF,0xFF,0xFF,0x1F,0x00,0x00,0xF0,0xFF,0xFF,0x7F}},
    { {0xFF,0xFF,0xFF,0xFF,0x03,0x00,0x00,0xFE,0xFF,0x7F}},
    { {0xFF,0xFF,0xFF,0xFF,0x7F,0x00,0x00,0xC0,0xFF,0x7F}},
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0x0F,0x00,0x00,0xF8,0x7F}},
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x01,0x00,0x00,0x7F}},
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x3F,0x00,0x00,0x60}},
...
{
    UCHAR *pChannelMap;
    UCHAR *pHciBuffer = &g_AFHCommandBuffer[HCI_PACKET_HEADER_PAD];
    
    //DebugBreak();
    
    if ((CurrentWLANChannel > MAX_WLAN_CHANNELS) || (CurrentWLANChannel == 0)) {
        return TRUE;    
    }
   
    if (InterlockedIncrement(&g_AFHCommandPending) != 1) {
        InterlockedDecrement(&g_AFHCommandPending);
        return TRUE;    
    }   
    
    pChannelMap = &(g_ChannelMapTable[(CurrentWLANChannel - 1)].Map[0]);    
     
    pHciBuffer[0] = 0x3F;  /* set opcode */
    pHciBuffer[1] = 0x0C;  
    pHciBuffer[2] = 0x0A;  /* 10 bytes to follow command */
    
        /* fill 10 byte channel map */
    memcpy(&pHciBuffer[3], pChannelMap, AFH_CHANNEL_MAP_BYTES);
    
        /* setup HCI packet */
    memset(&g_AFHHCIPacket , 0, sizeof(g_AFHHCIPacket));   
    
    g_AFHHCIPacket.pBuffer = &g_AFHCommandBuffer[0];
    g_AFHHCIPacket.cStart = HCI_PACKET_HEADER_PAD;
    g_AFHHCIPacket.cEnd = g_AFHHCIPacket.cStart + AFH_COMMAND_SIZE;
    g_AFHHCIPacket.cSize = sizeof(g_AFHCommandBuffer);
    
        /* send packet, note: AFH command will be pending, we filter the 
         * command complete event and drop it */
    return HCI_WritePacket(COMMAND_PACKET, &g_AFHHCIPacket);
}
...
reReadPacket:
	...
	if (!nRet)
	{
        ...
        RETAILMSG(1,(TEXT(" [HCIUNIV] HCI_ReadPacket failed. \r\n")));
	}
    
#ifdef ATHBT_FILTER

    if (fInit && pfn && nRet && (*peType == EVENT_PACKET)) {
        AthBtFilterHciEvent(&g_AthBtFilterInstance, 
                            (pBuff->pBuffer + pBuff->cStart),
                            BufferTotal(pBuff));
        
        if (g_AFHCommandPending) {
            UCHAR *pEvt = (pBuff->pBuffer + pBuff->cStart);
                        
            if ((pEvt[0] == 0xE) && (pEvt[3] == 0x3F) &&
                (pEvt[4] == 0x0C)) {    
                /* got command complete for AFH */
                RETAILMSG(1,(TEXT(" Dropping AFH command complete \r\n")));
                InterlockedDecrement(&g_AFHCommandPending);
                goto reReadPacket;
            }
        }
    ...
    if (fInit && pfn && (*peType == DATA_PACKET_ACL)) {
            /* for CE5 we filter ACL packets to detect application traffic flows like A2DP */
