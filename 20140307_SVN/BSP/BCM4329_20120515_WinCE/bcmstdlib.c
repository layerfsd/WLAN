 ...
 *
 * $Id: bcmstdlib.c,v 1.42.4.1.2.10.6.5 2010/04/15 02:21:06 Exp $
 */
...
#include <typedefs.h>
#if defined(NDIS) || defined(__vxworks) || \
...
void *
BCMROMFN(memset)(void *dest, int c, size_t n)
{
	uint32 w, *dw;
	unsigned char *d;
...
void *
BCMROMFN(memcpy)(void *dest, const void *src, size_t n)
{
	uint32 *dw;
	const uint32 *sw;
	unsigned char *d;
...
#if !defined(printf)
int
BCMROMFN(printf)(const char *fmt, ...)
{
	va_list ap;
	int count, i;
