...
/* WL_BSS_FLAGS_RSSI_ONCHANNEL changed to 0x04 to match TOT */
...
typedef struct wl_assoc_params {
	...
	uint16 bssid_cnt;		/* 0: use chanspec_num, and the single bssid,
					 ...
					 */
	int32 chanspec_num;		/* 0: all available channels, OR as per bssid_cnt;
					 * otherwise count of chanspecs in chanspec_list
					 */
...
typedef struct {
	...
	uint32	rxmcs32_40M;  /* mpdu's recd at mc32 on 40MHz bw */
...
#define	WL_RXDIV_STATS_T_VERSION	1	/* Rx diversity counters */
typedef struct {
	uint16	version;	/* see definition of WL_CNT_T_VERSION */
	uint16	length;		/* length of entire structure */

	uint32 rxant[4];	/* num pkts rcvd on Ant[i] */
} wl_rxdiv_stats_t;
...
/* structure for identifying block-ack window sizes for tx/rx */
struct ampdu_ba_sizes {
	uint8 ba_tx_wsize;	/* ba for tx */
	uint8 ba_rx_wsize;	/* ba for rx */
};
...
typedef struct wl_rssi_event {
	/* Number of events posted to application will be limited to 1 per this rate
	 * limit (specified in milliseconds). Set to 0 to disable rate limit.
	 */
	uint32 rate_limit_msec;
	/* Number of RSSI levels specified in 'rssi_levels'. */
	uint8 num_rssi_levels;
	/* Variable number of RSSI levels. An event will be posted each time the
	 * RSSI of received beacons/packets crosses a level.
	 */
