...
#ifdef TCMD
static A_STATUS wmi_tcmd_cont_rx_report_rx(struct wmi_t *wmip, A_UINT8 *datap, int len);
#endif //TCMD
...
A_UINT8 wmi_implicit_create_pstream(struct wmi_t *wmip, void *osbuf, A_UINT8 dir)
{
    ...
    if (llcHdr->etherType == htons(ipType)) {
        ...
        trafficClass = ar6000_iptos_to_trafficclass(((A_UINT8 *)llcHdr)
                                                + sizeof(ATH_LLC_SNAP_HDR));
    } else {
    ...
    if (!((trafficClass == WMM_AC_BE) || (wmip->wmi_trafficClassMap[dir][trafficClass] != WMI_NOT_MAPPED))) {
        ...
        if (wmi_create_pstream_cmd(wmip, trafficClass, dir, reqMbox, 0, 0, 0,
                          WMI_IMPLICIT_PSTREAM_INACTIVITY_INT) != A_OK) {
...
wmi_control_rx(struct wmi_t *wmip, void *osbuf)
{
    ...
    case (WMI_EXTENSION_EVENTID):
        ...
        break;
#ifdef TCMD
    case (WMI_TCMD_CONT_RX_CMDID):
        status = wmi_tcmd_cont_rx_report_rx(wmip, datap, len);
        break;
#endif //TCMD
    default:
...
wmi_connect_cmd(struct wmi_t *wmip, NETWORK_TYPE netType, 
                ...
                AUTH_MODE authMode,
                CRYPTO_TYPE pairwiseCrypto, CRYPTO_TYPE groupCrypto,
...
A_STATUS
wmi_listeninterval_cmd(struct wmi_t *wmip, A_UINT16 listenInterval)
{
    void *osbuf;
...
A_STATUS
wmi_bmisstime_cmd(struct wmi_t *wmip, A_UINT16 bmissTime)
{
    void *osbuf;
...
A_STATUS
wmi_ibsspmcaps_cmd(struct wmi_t *wmip, A_UINT8 pmEnable, A_UINT32 atim_windows)
{
    void *osbuf;
...
A_STATUS
wmi_create_pstream_cmd(struct wmi_t *wmip,
                        A_UINT8  trafficClass,
                       ...
                       )
{
    ...
    if (trafficDirection == BIDIR_TRAFFIC) {
        if (wmip->wmi_pstreamCmdInProgress[UPLINK_TRAFFIC][trafficClass]
          || wmip->wmi_pstreamCmdInProgress[DNLINK_TRAFFIC][trafficClass]
          || wmip->wmi_cpstreamCmdInProgress) {
            WMI_DEBUG_PRINTF(ATH_LOG_ERR,"create %d too busy !\n",trafficClass);
            return A_EBUSY;
        }
        wmip->wmi_pstreamCmdInProgress[UPLINK_TRAFFIC][trafficClass] = TRUE;
        wmip->wmi_pstreamCmdInProgress[DNLINK_TRAFFIC][trafficClass] = TRUE;
    } else {
        if (wmip->wmi_pstreamCmdInProgress[trafficDirection][trafficClass]
         || wmip->wmi_cpstreamCmdInProgress) {
            WMI_DEBUG_PRINTF(ATH_LOG_ERR,"create %d too busy !\n",trafficClass);
            return A_EBUSY;
        }
        wmip->wmi_pstreamCmdInProgress[trafficDirection][trafficClass] = TRUE;
    }
    ...
    WMI_DEBUG_PRINTF(ATH_LOG_INF,"Sending create_pstream_cmd: ac=%d, rxQ=%d, dir=%d\n", 
                     trafficClass, rxQueueNum, trafficDirection);
    ...
    if (rxQueueNum == -1) {
        ...
        cmd->rxQueueNum = rxQueueNum;
    }
    cmd->voicePSCapability   = voicePSCapability;
    cmd->minServiceInt    = minServiceInt;
    cmd->maxServiceInt    = maxServiceInt;
    cmd->inactivityInt    = inactivyInt;
...
#ifdef TCMD
A_STATUS
wmi_tcmd_enable_cmd(struct wmi_t *wmip, A_UINT8 enable)
{
    void *osbuf;
    WMI_TCMD_ENABLE_CMD *cmd;
    ...
    cmd = (WMI_TCMD_ENABLE_CMD *)(a_netbuf_to_data(osbuf));
    A_MEMZERO(cmd, sizeof(*cmd));
    cmd->tcmdEnable = enable;

    return (wmi_cmd_send(wmip, osbuf, WMI_TCMD_ENABLE_CMDID,
            NO_SYNC_WMIFLAG));
}

A_STATUS
wmi_tcmd_cont_tx_cmd(struct wmi_t *wmip, A_UINT32 mode, A_UINT32 freq,
                      A_UINT32 dataRate, A_UINT32 txPwr, A_UINT32 antenna)
{
    void *osbuf;
    WMI_TCMD_CONT_TX_CMD *cmd;
    ...
    cmd = (WMI_TCMD_CONT_TX_CMD *)(a_netbuf_to_data(osbuf));
    A_MEMZERO(cmd, sizeof(*cmd));
    cmd->mode = mode;
    cmd->freq = freq;
    cmd->dataRate = dataRate;
    cmd->txPwr = txPwr;
    cmd->antenna = antenna;

    return (wmi_cmd_send(wmip, osbuf, WMI_TCMD_CONT_TX_CMDID,
            NO_SYNC_WMIFLAG));
}


A_STATUS
wmi_tcmd_cont_rx_cmd(struct wmi_t *wmip, A_UINT32 act, A_UINT32 freq, A_UINT32 antenna)
{
    void *osbuf;
    WMI_TCMD_CONT_RX_CMD *cmd;
    ...
    cmd = (WMI_TCMD_CONT_RX_CMD *)(a_netbuf_to_data(osbuf));
    A_MEMZERO(cmd, sizeof(*cmd));
    cmd->act = act;
    cmd->u.para.freq= freq;
    cmd->u.para.antenna = antenna;

    return (wmi_cmd_send(wmip, osbuf, WMI_TCMD_CONT_RX_CMDID,
            NO_SYNC_WMIFLAG));
}

A_STATUS
wmi_tcmd_setmac_cmd(struct wmi_t *wmip, A_UINT32 act, A_UCHAR *mac)
{
    void *osbuf;
    WMI_TCMD_CONT_RX_CMD *cmd;
    ...
    A_MEMZERO(cmd, sizeof(*cmd));
    cmd->act = act;
    A_MEMCPY(cmd->u.mac.addr, mac, ATH_MAC_LEN);
...
static A_STATUS
wmi_tcmd_cont_rx_report_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    WMI_TCMD_CONT_RX_CMD *rxCmd;

    if (len < sizeof(WMI_TCMD_CONT_RX_CMD)) {
        return A_EINVAL;
    }

    rxCmd = (WMI_TCMD_CONT_RX_CMD *)datap;

    if (rxCmd->act != WMI_TCMD_CONT_RX_REPORT) {
        return A_EINVAL;
    }

    ar6000_tcmd_rx_report_event(wmip->wmi_devt, rxCmd->u.report.totalPkt, rxCmd->u.report.rssi);

    return A_OK;
}

A_STATUS
wmi_tcmd_pm_cmd(struct wmi_t *wmip, A_UINT8 mode)
{
    void *osbuf;
    WMI_TCMD_PM_CMD *cmd;
    ...
    cmd = (WMI_TCMD_PM_CMD *)(a_netbuf_to_data(osbuf));
    A_MEMZERO(cmd, sizeof(*cmd));
    cmd->mode = mode;

    return (wmi_cmd_send(wmip, osbuf, WMI_TCMD_PM_CMDID,
            NO_SYNC_WMIFLAG));
}
