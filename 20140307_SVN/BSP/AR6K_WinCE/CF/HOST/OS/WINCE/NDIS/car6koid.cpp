...
static NDIS_802_11_AUTHENTICATION_ENCRYPTION g_WPAAuthEncryptPairSupported[] =
{
...
#ifdef OID_802_11_CAPABILITY // Using the presense of this OID to indicate WPA2 supported in this build
...
A_STATUS
CAR6KMini::ar6000_ioctl_tcmd_get_rx_report(PUCHAR rq)
{
    
	Lock();
    m_tcmdRxReport = 0;
    if (wmi_tcmd_cont_rx_cmd((wmi_t*)m_pWMI, WMI_TCMD_CONT_RX_REPORT, 0, 0) != A_OK)
	{
		Unlock();
        return A_ERROR;
    }
	...
	NdisWaitEvent(&m_rxReportEvent, 0);

	NdisResetEvent(&m_rxReportEvent);

    *((UINT *)rq) = m_tcmdRxTotalPkt;
    *((UINT *)rq + 1) = m_tcmdRxRssi;
...
#ifdef TCMD
	{ OID_CAR6K_TCMD_CONT_TX, sizeof(WMI_TCMD_CONT_TX_CMD) },
	{ OID_CAR6K_TCMD_CONT_RX, sizeof(WMI_TCMD_CONT_RX_CMD) },
	{ OID_CAR6K_TCMD_PM, sizeof(WMI_TCMD_PM_CMD) },
#endif //TCMD
	{ 0, 0 }
};
	...
	OUT PULONG   pcbNeeded)
{
	...
	switch (Oid)
	{
		...
		case OID_CAR6K_TCMD_CONT_TX:
			Status = SetCAr6kTCmdContTx((WMI_TCMD_CONT_TX_CMD *)Buffer);
			break;

		case OID_CAR6K_TCMD_CONT_RX:
			Status = SetCAr6kTCmdContRx((WMI_TCMD_CONT_RX_CMD *)Buffer);
			break;

		case OID_CAR6K_TCMD_PM:
			Status = SetCAr6kTCmdPm((WMI_TCMD_PM_CMD *)Buffer);
			break;
#endif //TCMD
...
{
	return "Atheros AR6001 802.11 SDIO Network Adapter";
}
...
{
	return ((USHORT)NDIS_DRIVER_MAJOR_VERSION << 8) | NDIS_DRIVER_MINOR_VERSION;
}
...
CAR6KMini::Get802_11AssociationInformation(
	...
	OUT ULONG                               *pcbNeeded)
{
	PBYTE  assocReqStart = NULL;
	PBYTE  assocRespStart = NULL;
	USHORT fixedRespLen = 0;
	...
	*pcbNeeded = m_AssocReqLen + m_AssocRespLen + sizeof(*pAssocInfo);
	...
	pAssocInfo->RequestFixedIEs.ListenInterval = 
		m_ListenInterval;
	pAssocInfo->RequestFixedIEs.Capabilities = *((A_UINT16*)m_pAssocInfo);//0x3104;
	memcpy(pAssocInfo->RequestFixedIEs.CurrentAPAddress,
		m_PeerBSSID, ETHERNET_MAC_ADDRESS_LENGTH);
	// Populate the Variable Request IEs
	pAssocInfo->RequestIELength = m_AssocReqLen - 4; //[subtract capIE and ListenInterval IE]
	pAssocInfo->OffsetRequestIEs = sizeof(*pAssocInfo);
	assocReqStart = (PBYTE)pAssocInfo + sizeof(*pAssocInfo);
	memcpy(assocReqStart, m_pAssocInfo+4, m_AssocReqLen);

	pAssocInfo->AvailableResponseFixedIEs = 
		NDIS_802_11_AI_RESFI_CAPABILITIES |
		NDIS_802_11_AI_RESFI_STATUSCODE   |
		NDIS_802_11_AI_RESFI_ASSOCIATIONID;
	assocRespStart = assocReqStart + m_AssocReqLen + 4;
	pAssocInfo->OffsetResponseIEs = sizeof(*pAssocInfo) + m_AssocReqLen + 4;

	/*
	 * Till we move to mainline firmware I have a patched beta
	 * firmware which has the complete associtation response
	 * it is available @ /etna_wince_ms/target/patch_19046/bin
	 */
	// move past the 802.11 header
	assocRespStart += sizeof(struct ieee80211_frame);
	memcpy(&(pAssocInfo->ResponseFixedIEs.Capabilities), 
		assocRespStart, sizeof(USHORT));
	assocRespStart += sizeof(USHORT);
	memcpy(&(pAssocInfo->ResponseFixedIEs.StatusCode),
		assocRespStart, sizeof(USHORT));
	assocRespStart += sizeof(USHORT);
	memcpy(&(pAssocInfo->ResponseFixedIEs.AssociationId),
		assocRespStart, sizeof(USHORT));
	assocRespStart += sizeof(USHORT);
	
	fixedRespLen = sizeof(struct ieee80211_frame) + (3*sizeof(USHORT));
	// Adjust to match the remaining length
	pAssocInfo->ResponseIELength = m_AssocRespLen - fixedRespLen;
	memcpy(assocRespStart, (m_pAssocInfo + m_AssocReqLen + fixedRespLen), 
		   pAssocInfo->ResponseIELength);

	// ?? why only structure length
						   ...
						   OUT ULONG                  *pcbNeeded)
{
    ULONG i = 0;
    ULONG lenNeeded;
    const ULONG NoOfAuthEncryptPairsSupported = _countof(g_WPAAuthEncryptPairSupported);

    lenNeeded = offsetof(NDIS_802_11_CAPABILITY, AuthenticationEncryptionSupported)
              + (NoOfAuthEncryptPairsSupported * sizeof(NDIS_802_11_AUTHENTICATION_ENCRYPTION));
    ...
    *pcbNeeded = lenNeeded;
    ...
    pCap->NoOfAuthEncryptPairsSupported = NoOfAuthEncryptPairsSupported; //excluding WEP shared and WPA-None
    pCap->Length = lenNeeded; 
    for (i = 0; i < NoOfAuthEncryptPairsSupported; i++) {
        pCap->AuthenticationEncryptionSupported[i].AuthModeSupported = 
            g_WPAAuthEncryptPairSupported[i].AuthModeSupported;
        pCap->AuthenticationEncryptionSupported[i].EncryptStatusSupported = 
            g_WPAAuthEncryptPairSupported[i].EncryptStatusSupported;
...
//
{
	...
	case Ndis802_11AuthModeAutoSwitch:
		wmiDot11AuthMode = SHARED_OPEN_AUTH;
		break;
	...
	wmiStatus = wmi_connect_cmd((wmi_t *)m_pWMI, wmiNetworkType, 
                                wmiDot11AuthMode, 
		                        wmiAuthMode, m_PairwiseCryptoType, m_GrpwiseCryptoType,
							    m_SSID.SsidLength, m_SSID.Ssid, pAPBSSID, channel);
	...
	IN NDIS_802_11_POWER_MODE PowerMode)
{
	...
	if ( powerMode != MAX_PERF_POWER ) {
		...
		Status = wmi_listeninterval_cmd((struct wmi_t *)m_pWMI, listenInterval);
		if (Status != A_OK)
			return NDIS_STATUS_FAILURE;
	}
	...
	if ( Status != A_OK ) {
		...
		if ( powerMode != MAX_PERF_POWER )
			wmi_listeninterval_cmd((struct wmi_t *)m_pWMI, m_ListenInterval);
...
#ifdef TCMD
NDIS_STATUS
CAR6KMini::SetCAr6kTCmdContTx(
	WMI_TCMD_CONT_TX_CMD *Buffer)
{

	if (m_tcmdPm == WMI_TCMD_PM_SLEEP) {
		NDIS_DEBUG_PRINTF(ATH_LOG_ERR, "AR6K: Can NOT send tx tcmd when target is asleep! \n");
		return NDIS_STATUS_FAILURE;
	}

	wmi_tcmd_cont_tx_cmd((wmi_t*)m_pWMI, Buffer->mode, Buffer->freq, Buffer->dataRate, Buffer->txPwr, Buffer->antenna);
	
	return NDIS_STATUS_SUCCESS;
}

NDIS_STATUS
CAR6KMini::SetCAr6kTCmdContRx(
	WMI_TCMD_CONT_RX_CMD *Buffer)
{

	if (m_tcmdPm == WMI_TCMD_PM_SLEEP) {
		NDIS_DEBUG_PRINTF(ATH_LOG_ERR, "AR6K: Can NOT send rx tcmd when target is asleep! \n");
		return NDIS_STATUS_FAILURE;
	}

	if (Buffer->act == WMI_TCMD_CONT_RX_PROMIS || Buffer->act == WMI_TCMD_CONT_RX_FILTER) {
		wmi_tcmd_cont_rx_cmd((wmi_t*)m_pWMI, Buffer->act, Buffer->u.para.freq, Buffer->u.para.antenna);
	} else if (Buffer->act == WMI_TCMD_CONT_RX_REPORT) {
		ar6000_ioctl_tcmd_get_rx_report((PUCHAR)Buffer);
	} else if (Buffer->act == WMI_TCMD_CONT_RX_SETMAC) {
		wmi_tcmd_setmac_cmd((wmi_t*)m_pWMI, Buffer->act, Buffer->u.mac.addr);
	}
...
NDIS_STATUS
CAR6KMini::SetCAr6kTCmdPm(
	WMI_TCMD_PM_CMD *Buffer)
{

	m_tcmdPm = Buffer->mode;
	wmi_tcmd_pm_cmd((wmi_t*)m_pWMI, Buffer->mode);
