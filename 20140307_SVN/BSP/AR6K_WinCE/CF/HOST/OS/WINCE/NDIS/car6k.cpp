...
//
{
...
#ifdef TCMD
    if (m_Config.byPassWmi == AR6000_TCMD_MODE)
		NdisInitializeEvent(&m_rxReportEvent);
#endif //TCMD
...
#else
	if ( m_Config.byPassWmi != AR6000_BYPASS_MODE ) {
#endif //TCMD
    ...
    if (m_Config.byPassWmi == AR6000_TCMD_MODE) {
        /*
         * Enable TCMD mode in target.           *
         * since we use ioctl in tcmd to talk with ar6000_drv, so
         * netif queue keep stopped since here.
         */
        wmi_tcmd_enable_cmd((wmi_t *)m_pWMI, 1);
    } else if (m_Config.byPassWmi ==  AR6000_WMI_MODE) {
        /*
         * Tell target go to normal WLAN app.
         */
        wmi_tcmd_enable_cmd((wmi_t *)m_pWMI, 0);
		NdisWaitEvent(&m_WMIReadyEvent, AR6K_DEFAULT_MS_TO_WAIT_FOR_WMI_READY);
		if (!m_WMIReady) {
			NDIS_DEBUG_PRINTF(ATH_LOG_ERR, "AR6K: ERROR - No WMI_READY event after %u ms, failing initialization\n", AR6K_DEFAULT_MS_TO_WAIT_FOR_WMI_READY);
			status = A_ERROR;
...
#ifdef TCMD
	if (m_Config.byPassWmi ==  AR6000_WMI_MODE) {
#else
	if (!m_Config.byPassWmi) {
#endif
		...
		if (m_Config.bkScanEnable) {
			wmiStatus = wmi_scanparams_cmd((wmi_t *)m_pWMI,0,0,(A_UINT16)m_Config.bkScanPeriod,
										0,0,WMI_SHORTSCANRATIO_DEFAULT);
		}
		...
		if ( wmiStatus == A_OK ) {
			if ( m_80211_PowerMode != Ndis802_11PowerModeCAM ) {
				wmiStatus = wmi_listeninterval_cmd((struct wmi_t *)m_pWMI, m_ListenInterval);
			}
		}
...
#define DRV_STATE_EVENT  (TEXT("SYSTEM\\netui-AR6K_CF1"))
#elif defined SDIO
...
CAR6KMini::ResetPowerWorkItem()
{
	...
	if (!m_Config.byPassWmi) {
		...
		if ((!powerState) && (m_Halting == false)) {    
	       ...
	       driverStateEvent = CreateEvent(NULL, FALSE, FALSE, TEXT("SYSTEM\\netui-AR6K1"));
	       if (driverStateEvent != NULL) {
			...
			// with the power manager
			while (m_Halting == false) {
				status= GetDevicePower((PVOID)AR6K_DEVICE_POWER_NAME, POWER_NAME, &cePowerState);
				if (status != ERROR_FILE_NOT_FOUND) {
					break;
				}
                NdisMSleep(1000);
			...
			if ((status == ERROR_SUCCESS) && (m_Halting == false)) {
				cePowerState = D4;
				status = DevicePowerNotify((PVOID)AR6K_DEVICE_POWER_NAME,cePowerState, POWER_NAME);
                if (status == ERROR_SUCCESS) {
                    athStatus = A_OK;
                }
			}
