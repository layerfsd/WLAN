...
wmi_ready_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
	...
	printk("len = %d, sizeof(WMI_READY_EVENT) = %d \r\n", len, sizeof(WMI_READY_EVENT));
...
wmi_connect_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
	...
	printk("%s() -> freq %d bssid %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
        __func__, ev->channel,
        ev->bssid[0], ev->bssid[1], ev->bssid[2],
        ev->bssid[3], ev->bssid[4], ev->bssid[5]);
...
wmi_bssInfo_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len, A_UINT8 phy_mode)	/* Bug 82893 */
{
    ...
    A_DPRINTF(DBG_WMI2, (DBGFMT "bssInfo event - ch %u, rssi %02x, "
              ...
              bih->bssid[5]));
#if 0
	printk("%s: bssInfo event - ch %u, rssi %02x, bssid \"%02x:%02x:%02x:%02x:%02x:%02x\"\n", 
				__func__,bih->channel, (unsigned char) bih->rssi, bih->bssid[0],
			  	bih->bssid[1], bih->bssid[2], bih->bssid[3], bih->bssid[4],  bih->bssid[5]);
#endif
                  ...
                  A_INT8 numChan, A_UINT16 *channelList)
{
	...
	printk(" %s() --> forceFgScan(%d), forceScanInterval(%d), numChan(%d) \r\n",
				__func__,forceFgScan, forceScanInterval, numChan);
                   ...
                   A_UINT32 max_dfsch_act_time, A_UINT16 maxact_scan_per_ssid)
{
	...
	printk(" %s() --> fg_start_sec(%d), fg_end_sec(%d)\r\n",__func__,fg_start_sec,fg_end_sec);
