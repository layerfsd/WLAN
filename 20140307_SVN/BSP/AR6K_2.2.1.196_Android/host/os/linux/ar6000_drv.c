 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/os/linux/ar6000_drv.c#94 $";
...
#include "regulatory/reg_dbvalues.h"	/* for bug 82603 channel tests */
...
#define plat_setup_power plat_setup_power_stub
#else
#define plat_setup_power(_a, _on, _det) /* define as your version */
#endif
...
int regCode = 0x0; 
#else
/* world wide reg code is 0x60 for android SCAN-CHANNELS command */
//int regCode = 0x60;
...
int debugdriver = 0;
#ifdef TCMD
unsigned int debughtc = 0x00;	//ATH_DEBUG_WMI
//	unsigned int debughtc = 0x01 + 0x02;
//unsigned int debughtc = 0x120 | 0x80;
#else
unsigned int debughtc = (0x80 + 0x01);
#endif
unsigned int debugbmi = 0;	//org 1
unsigned int debughif = 0;	//org 2
...
static void rx_priority_timer_handler(unsigned long ptr);
...
static void deliver_frames_to_nw_stack(AR_SOFTC_T *ar, struct sk_buff *skb);
...
ar6000_set_host_app_area(AR_SOFTC_T *ar)
{
	...
	while(count--)
	{
		...
		msleep(100);
		printk("retry read reg  count = %d \r\n", count);
	}

	if(!count) {
		printk(" ReadRegDiag Error !! \r\n");
		return A_ERROR;
	}
...
ar6000_dbglog_get_debug_logs(AR_SOFTC_T *ar)
{
    ...
    if(debugdriver) printk("debug_hdr_ptr: 0x%x\n", debug_hdr_ptr);
...
void plat_setup_power_stub(AR_SOFTC_T *ar, int on, int detect)
{
    ...
    if (gpio_request(PLAT_WLAN_CHIP_PWD_PIN, "wlan_chip_pwd_l")!=0) {
      AR_DEBUG_PRINTF("Cannot request CHIP_PWD GPIO");
    } else {
...
ar6000_wow_irq(int irq, void *dev_id)
{
    ...
    if(debugdriver) printk("%s: wakeup !!!\n", __func__);
#ifdef ANDROID_ENV
    wake_lock_timeout(&ar6k_wow_wake_lock, 3*HZ);
#endif
    return IRQ_HANDLED;
...
static int ar6000_pm_probe(struct platform_device *pdev)
{
    ...
    if (wow_irq) {
        ...
        if(debugdriver) printk("wow_irq %d, ret %d\n", wow_irq, ret);
        if (!ret) {
            ret = enable_irq_wake(wow_irq);
            if (ret < 0) {
                if(debugdriver) printk("Couldn't enable WoW IRQ as wakeup interrupt");
            }
        }
    }
#endif /* PLAT_WOW_GPIO_PIN */
...
static int ar6000_pm_remove(struct platform_device *pdev)
{
    ...
    if (wow_irq) {
        if (disable_irq_wake(wow_irq)) {
            if(debugdriver) printk("Couldn't disable hostwake IRQ wakeup mode\n");
		 }

        if(debugdriver) printk("Free irq %d\n", wow_irq);
...
/* ATHENV */
    ...
    if (platform_driver_register(&ar6000_pm_device)) {
        if(debugdriver) printk("ar6000: fail to register the power control driver.\n");
	}
#endif
/* ATHENV */
...
/* ATHENV */
#ifdef ANDROID_ENV
            if(debugdriver) printk("AR6K: ar6000_cleanup_module ar6000_destroy [%d]\n", i);
#endif
/* ATHENV */
...
static void firmware_transfer(HIF_DEVICE *device, char* filename, A_UINT32 address, A_BOOL isCompressed)
{
    ...
    if ( (ret=ar6000_readwrite_file(filename, NULL, NULL, 0)) < 0) {
        if(debugdriver) printk("%s: Cannot read %s size\n", __FUNCTION__, filename);
        return;
    } else {
        length = ret;
    }    
    ...
    if (fw_buf == NULL) {
        if(debugdriver) printk("%s: kernel memory alloc error\n", __FUNCTION__);
        return;
    }
    ...
    if ( isCompressed) {
        ...
        if (ret != A_OK) {
            if(debugdriver) printk("%s: BMILZStreamStart failed, ret=%d\n", __FUNCTION__, ret);
            goto Transfer_DONE;
        }
    }
    ...
    while (remaining>0) {
        ...
        if (isCompressed) {
            ...
            if (ret != A_OK) {
                if(debugdriver) printk("%s: BMILZData failed, ret=%d\n", __FUNCTION__, ret);
                goto Transfer_DONE;
            }
        } else {
            ret = BMIWriteMemory(device, address, src, length1);
            if (ret != A_OK) {
                if(debugdriver) printk("%s: BMIWriteMemory failed, ret=%d\n", __FUNCTION__, ret);
                goto Transfer_DONE;
            }
        }
...
{   
    if ((length=ar6000_readwrite_file(reg_file, val_ptr, NULL, 2)) != 2)) {
        if(debugdriver) printk("%s: file read error, length=[0x%x]\n", __func__, length);
        return;
    }
}
...
/* ATHENV */
    ...
    {   /*
        ...
        if(debugdriver) printk("call usermode fw_loader ret = %d err %d\n", ret, ((ret & 0xff00) >> 8));
    ...
    if (tgt_fw != NULL) {
        ...
        if (value == 0) {
           if(debugdriver) printk("use internal clock\n");
            value = 0x100000;
        ...
        value = 1;
        if(debugdriver) printk("AR6000: BMIExecute\n");
        ...
        /* WLAN patch DataSets */
        firmware_transfer(ar->arHifDevice, tgt_patch, 0x52d6c4, FALSE);
        ...
        if (work_mode == 2) {
            ...
            ret = ar6000_init(dev);
            if (ret!= A_OK) {
                if(debugdriver) printk("%s: ar6000_init failed, ret=%d\n", __FUNCTION__, ret);
                goto err_exit;
            }
            ...
            if ((work_mode == 0) && (ret == A_OK)) {
                ...
                if ((chan_num == 11) || (chan_num == 13) || (chan_num == 14)) {
                    ...
                    if (wmi_set_channelParams_cmd(ar->arWmi, 1, WMI_11G_MODE, chan_num, chan_list) != A_OK) {
                        if(debugdriver) printk("Fialed to configure channel number\n");
					}
                }
/* ATHENV V10.1 +++ */
#ifdef ANDROID_ENV
                if (txpwr > 0 ) {
                    if ( wmi_set_txPwr_cmd(ar->arWmi, txpwr) != A_OK )  {
                        if(debugdriver) printk("Failed to set txpwr : %d\n", txpwr);
                    } else {
                        if(debugdriver) printk("AR6K: set txpwr : %d successfully\n", txpwr);
                    }
                }
...
ar6000_avail_ev(void *context, void *hif_handle)
{
    ...
    BMIInit();
    {
        ...
        if (BMIGetTargetInfo(ar->arHifDevice, &targ_info) != A_OK) {
			AR_DEBUG_PRINTF("BMIGetTargetInfo Fail !!\n");
            return A_ERROR;
        }
    ...
    if (ar->arHtcTarget == NULL) {
		AR_DEBUG_PRINTF("ar->arHtcTarget fail !!\n");
        return A_ERROR;
    }
    ...
    if (ar6000_download_image(dev)!=A_OK) {
		printk("ar6000_download_image fail & Skip ar6000_destroy !!! \r\n");
        //ar6000_destroy(dev, 0);
        return A_ERROR;
    }
...
{
    ar->arWmiEnabled         = FALSE;
    ar6000_init_profile_info(ar);	//ar->arNetworkType = INFRA_NETWORK;
...
int ar6000_init(struct net_device *dev)
{
    ...
    if (!bypasswmi) {
        ...
        if(!timeleft || signal_pending(current))
        {
		 ...
		 printk("SKIP writing wmi protocal version !!!\n");
#if 0
	...
	if( wmi_scanparams_cmd(ar->arWmi, 0, 0, 5, 100, 110, 0, 1, DEFAULT_SCAN_CTRL_FLAGS, 0, 0) != A_OK )
		printk("AR6K2:ar6000_init(): Scan parameters setting failed\r\n");
	
	if( wmi_disctimeout_cmd(ar->arWmi, 5) != A_OK )
		printk( "AR6K2:ar6000_init(): Disconnect timeout setting failed\r\n" );
		
	if( wmi_bmisstime_cmd(ar->arWmi, 0, 10) != A_OK )
		printk( "AR6K2:ar6000_init(): Beacon miss timeout setting failed\r\n" );
			
	if( wmi_powermode_cmd(ar->arWmi, MAX_PERF_POWER) != A_OK )
		printk( "AR6K2:ar6000_init(): Power mode setting failed\r\n" );
	
	if( wmi_pmparams_cmd(ar->arWmi, 100, 0, 2) != A_OK )
		printk( "AR6K2:ar6000_init(): Power management setting failed\r\n" );
	
	if( wmi_listeninterval_cmd(ar->arWmi, 0, MIN_LISTEN_BEACONS) != A_OK )
		printk( "AR6K2:ar6000_init(): Listen interval setting failed\r\n" );
#endif
    return(0);
}
...
#define AC_NOT_MAPPED   99
    ...
    if (ar->arWowState) {
        A_NETBUF_FREE(skb);
        if(debugdriver) printk("Wow enabled. Not allow to send tx data\n");
        return 0;
    }
#endif /* CONFIG_PM */
...
static int checkforDHCPPacket(struct sk_buff *skb)
{
   ...
   if(ntohs(dhcpPacket->ether2Hdr.etherType) != 0x0800)
   {
       return(ATH_DHCP_INVALID_MSG);
   }

   /* Check for UDP */
   if(dhcpPacket->ipHdr.protocol != 17)
   {
       return(ATH_DHCP_INVALID_MSG);
   }

   /* Check for DHCP ports (67 and 68) */
   if(ntohs(dhcpPacket->udpHdr.destPort) != 67 &&
       ntohs(dhcpPacket->udpHdr.destPort) != 68)
   {
       return(ATH_DHCP_INVALID_MSG);
   }

   /* Some DHCP sanity checks */
   if((dhcpPacket->dhcpOptions[0] != ATH_DHCP_OPCODE_MSG_TYPE) ||
       (dhcpPacket->dhcpOptions[1] != ATH_DHCP_MSG_TYPE_LEN))
   {
       return(ATH_DHCP_INVALID_MSG);
   }

   /* Return the type */
   return(dhcpPacket->dhcpOptions[2]);
}
#else
static int checkforDHCPPacket(struct sk_buff *skb) 
{
...
static void
deliver_frames_to_nw_stack(AR_SOFTC_T *ar, struct sk_buff *skb)
{
    A_UINT8                 rxPriority;
    if(skb) {
...
ar6000_ready_event(void *devt, A_UINT8 *datap, A_UINT8 phyCap, A_UINT32 vers)
{
    ...
    ar->arDefPhyCapability = phyCap;	/* 82603 track the HW capabilities */
                     ...
                     A_UINT8 *assocInfo)
{
    ...
    if (beaconIeLen && (sizeof(buf) > (9 + beaconIeLen * 2))) {
        ...
        if (wrqu.data.length <= IW_CUSTOM_MAX) {
            ...
            AR_DEBUG_PRINTF("Fail to send BeaconIEs to upper layer\n");
        }
    }
...
#else
            AR_DEBUG_PRINTF("Fail to send Association Response to upper layer\n");
#endif
        }
    }
...
#else
            AR_DEBUG_PRINTF("Fail to send Association Request to upper layer\n");
#endif
        }
    }
    ...
    /* 82603 Update the PHY with the ASSOC Resp of the assocInfo
     * using the supported rates fields
     * assocInfo contains: beacon + assocReq + assocResp
     *  but in the case of creating an AdHoc network, 
     *  they may not be in the assocInfo buffer
     */
    if (assocRespLen)	/* if resp is in the buffer ... */
    {
      A_UINT8 *ptr = (A_UINT8 *)assocInfo;
      A_UINT8 basic_rate_len=0, ext_rate_len, x;
      A_UINT8 mode_11b=1;
      
      /* skip past beacon, assocreq, and top parts of assocresp (capinfo, status code, associd) */
      ptr += beaconIeLen + assocReqLen +
                            sizeof(A_UINT16)  +  /* capinfo*/
                            sizeof(A_UINT16)  +  /* status Code */
                            sizeof(A_UINT16)  ;  /* associd */
      /* now points to header of the rates IE (0x01) */
      if (*ptr == 0x01)
      {
	basic_rate_len = ptr[1];
	for (x=0;x<basic_rate_len;x++)
	{
	    if (ptr[2+x] == 12)
		mode_11b=0;
	}
      ...
      /* skip past the basic rate IE */
      ptr += 2+basic_rate_len;

      /* Now points to the header of the extended rates IE */
      if (mode_11b && *ptr == 50)
      {
	  ext_rate_len = ptr[1];
	  for (x=0;x<ext_rate_len;x++)
	  {
	      if (ptr[2+x] == 12)
		 mode_11b=0;
	  }
      ...
      /* Bug 82603 */
      if (IS_CHANNEL_11A(channel))
          ar->arPhyCapability = WMI_11A_CAPABILITY;
      else if (!mode_11b) 
	  ar->arPhyCapability = WMI_11G_CAPABILITY;    
      else
	  ar->arPhyCapability = WMI_11B_CAPABILITY;
    }
    reconnect_flag = 0;
                        ...
                        A_UINT8 assocRespLen, A_UINT8 *assocInfo, A_UINT16 protocolReasonStatus)
{
    ...
    /* Reset the phy cap back to the original value */
    ar->arPhyCapability = ar->arDefPhyCapability;	/*82603 */
    ...
    /* Update connect & link status atomically */
    if (!ar->arWlanDisableCmd)	/* 82509 don't call netif_carrier_off here - its called in ar6000_set_wlan_state */
    {
	ar->arWlanDisableCmd = FALSE;
	spin_lock_irqsave(&ar->arLock, flags);
	ar->arConnected = FALSE;

	netif_carrier_off(ar->arNetDev);  /* This call can lock out the SDIO stack if disconnect initiated from user space */
	spin_unlock_irqrestore(&ar->arLock, flags);
    }
...
ar6000_control_tx(void *devt, void *osbuf, HTC_ENDPOINT_ID eid)
{
    ...
    do {
        ...
        //printk("ar_contrstatus = ol_tx: skb=0x%x, len=0x%x eid =%d\n",
        ...
        //if(logWmiRawMsgs) 
		{
            printk("WMI cmd send, msgNo %d :", wmiSendCmdNum);
            for(i = 0; i < a_netbuf_to_len(osbuf); i++)
                printk("%x ", ((A_UINT8 *)a_netbuf_to_data(osbuf))[i]);
            printk("\n");
        }
...
ar6000_set_wlan_state(struct ar6_softc *ar, AR6000_WLAN_STATE state)
{
    ...
    do {
	    ...
	    ar->arWlanDisableCmd = TRUE;	/* Bug 82509 */
            AR6000_SPIN_LOCK(&ar->arLock, 0);
            if (ar->arConnected == TRUE || ar->arConnectPending == TRUE) {
                AR6000_SPIN_UNLOCK(&ar->arLock, 0);
                wmi_disconnect_cmd(ar->arWmi);
            ...
            A_MDELAY(250);
            HIFMaskInterrupt((HIF_DEVICE*)HTCGetHifDevice(ar->arHtcTarget));
			/* 82509 Following disconnect command, have carrier_off called here 
			 * instead of the rx_disconnect_event cleanup path, which causes a
			 * momentary race condition inolving the kernel General work queue.
			 */
			{
		    unsigned long flags;
		    spin_lock_irqsave(&ar->arLock, flags);
		    ...
		    spin_unlock_irqrestore(&ar->arLock, flags);
			ar->arWlanDisableCmd = FALSE;			/* Its now safe to allow netif_carrier_off to be called in ar6000_disconnect_event path */
			}
        }
    } while (0);
    if (status!=A_OK) {
...
#ifdef ANDROID_ENV
    if (status==A_OK && !(ar->arNetworkType & AP_NETWORK)) {
        /* Send wireless event which is needed by android supplicant */
        union iwreq_data wrqu;
