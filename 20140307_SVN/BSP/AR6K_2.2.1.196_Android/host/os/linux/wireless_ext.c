...
static A_BOOL specific_ssid_flag = FALSE;
            ...
            struct iw_point *data, char *extra)
{
    ...
    if (ar->arWlanState == WLAN_DISABLED) {
		printk("%s() ->end, return -EIO, by ar->arWlanState == WLAN_DISABLED !!\r\n",__func__);
        return -EIO;
    }

    if (ar->arWmiReady == FALSE) {
		printk("%s() ->end, return -EIO, by ar->arWmiReady == FALSE !!\r\n",__func__);
        return -EIO;
    }
    ...
    if (param.bytes_needed > (param.current_ev - extra))
    {
		...
		printk("%s() ->end, return -E2BIG !!\r\n",__func__);
        return -E2BIG;
    }

	printk("%s() ->end, return 0 !!\r\n",__func__);
    return 0;
}
...
static int ar6000_ioctl_giwpower(struct net_device *dev,
                 struct iw_request_info *info,
                 struct iw_param *rrq, char *extra) /* Bug 82814 */
{
#if defined(WIRELESS_EXT) && WIRELESS_EXT > 20
    AR_SOFTC_T *ar = (AR_SOFTC_T *)netdev_priv(dev);

	A_UINT8 power_mode;	/* Bug 82814 */
	...
	/* 82814 set ->power = 0 to mean Power Management is on, which corresponds to driver value 1 */
	rrq->disabled = 0;
	power_mode = wmi_get_power_mode_cmd(ar->arWmi);
	/* Bug 82814 REC_POWER is IEEE power save mode */
	if (power_mode == REC_POWER)
	{
		rrq->flags = IW_POWER_ON;
		rrq->value = power_mode;
	}
	else
	{
		rrq->disabled = 1;
              ...
              struct iw_param *data, char *extra)
{
	...
	printk("%s() ->start !! cmd = %s by reo  \r\n",__func__,(A_UCHAR *)data->value);
    ...
    else if (strcasecmp((A_UCHAR *)data->value, "RSSI") == 0)
    {
		...
		printk(" Nothing to do this command !!\r\n");
	}
...
#define SCAN_INT                HOME_TXDRAIN_TIME + ACT_DWELLTIME_DEFAULT
    ...
    if (data->pointer && (data->length == sizeof(struct iw_scan_req)))
    {
        if ((data->flags & IW_SCAN_THIS_ESSID) == IW_SCAN_THIS_ESSID)
        {
            ...
            specific_ssid_flag = TRUE;
        }
        else
        {
            if (specific_ssid_flag) {
