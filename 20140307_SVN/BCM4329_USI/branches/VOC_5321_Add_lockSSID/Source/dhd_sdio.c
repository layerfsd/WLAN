...
dhdsdio_sdclk(dhd_bus_t *bus, bool on)
{
	...
	if (on) {
		if (bus->idleclock == DHD_IDLE_STOP) {
			...
			if (err) {
				DHD_ERROR(("%s: error enabling sd_clock: %d\n",
				           __FUNCTION__, err));
				printf("%s: error enabling sd_clock: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
			...
			if (err) {
				DHD_ERROR(("%s: error changing sd_mode: %d\n",
				           __FUNCTION__, err));
				printf("%s: error changing sd_mode: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
			...
			if (err) {
				DHD_ERROR(("%s: error restoring sd_divisor: %d\n",
				           __FUNCTION__, err));
				printf("%s: error restoring sd_divisor: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		}
		...
		if ((bus->sd_divisor == -1) || (bus->sd_mode == -1)) {
			...
			printf("%s: can't idle clock, divisor %d mode %d\n",
			           __FUNCTION__, bus->sd_divisor, bus->sd_mode);
			return BCME_ERROR;
		}
		if (bus->idleclock == DHD_IDLE_STOP) {
			if (sd1idle) {
				...
				if (err) {
					DHD_ERROR(("%s: error changing sd_clock: %d\n",
					           __FUNCTION__, err));
					printf("%s: error changing sd_clock: %d\n",
					           __FUNCTION__, err);
					return BCME_ERROR;
				}
			}
			...
			if (err) {
				DHD_ERROR(("%s: error disabling sd_clock: %d\n",
				           __FUNCTION__, err));
				printf("%s: error disabling sd_clock: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		} else if (bus->idleclock != DHD_IDLE_ACTIVE) {
			...
			if (err) {
				DHD_ERROR(("%s: error changing sd_divisor: %d\n",
				           __FUNCTION__, err));
				printf("%s: error changing sd_divisor: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		}
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_SVAL(IOV_DEVRESET):
		DHD_TRACE(("%s: Called set IOV_DEVRESET=%d dongle_reset=%d busstate=%d\n",
		           ...
		           bus->dhd->busstate));
		printf("%s: Called set IOV_DEVRESET=%d dongle_reset=%d busstate=%d\n",
		           __FUNCTION__, bool_val, bus->dhd->dongle_reset,
		           bus->dhd->busstate);
		ASSERT(bus->dhd->osh);
		/* ASSERT(bus->cl_devid); */
...
dhdsdio_write_vars(dhd_bus_t *bus)
{
	...
	printf("Physical memory size: %d, usable memory size: %d\n",
		bus->orig_ramsize, bus->ramsize);
	printf("Vars are at %d, orig varsize is %d\n",
		varaddr, varsize);
	...
	printf("New varsize is %d, length token=0x%08x\n", varsize, varsizew);
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if (err) {
		DHD_ERROR(("%s: Failed to force clock for F2: err %d\n", __FUNCTION__, err));
		printf("%s: Failed to force clock for F2: err %d\n", __FUNCTION__, err);
		goto exit;
	}
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	{
		DHD_ERROR(("%s: dongle image download done.\n", __FUNCTION__));
	}
		...
		if (bus->dhd->dongle_reset) {
			...
			if (dhdsdio_probe_attach(bus, bus->dhd->osh, bus->sdh,
				...
				if (dhdsdio_probe_init(bus, bus->dhd->osh, bus->sdh) &&
					...
					sdnow_init_Interrupt(bus->sdh); // Chris.Jeong sleep -> wakeup issue. - Koine
