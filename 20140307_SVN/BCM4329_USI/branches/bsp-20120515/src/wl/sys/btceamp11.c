...
#include <wlc_cfg.h>
#endif /* !(BCMDONGLEHOST && !EXT_STA) || DHD_NDIS_OID || WLBTAMP_HIGH_ONLY */
#include <typedefs.h>
#include <bcmdefs.h>
#include <osl.h>
#include <bcmutils.h>
...
//        KdPrint ("BTCEAMP11_SubmitWrite with %u bytes", length);
        dhd_bta_tx_hcidata(dhd_bus_pub(wl->dhd), buffer, (uint)length);
//	logf(" BT->WLAN: data len %u", length);
        return TRUE;
        }
...
//        KdPrint ("BTCEAMP11_SubmitCmd with %u bytes", length);
        dhd_bta_docmd(dhd_bus_pub(wl->dhd), buffer, (uint)length);
        return TRUE;
        }
