 ...
 *
 * $Id: wlu_client_shared.c,v 1.1.6.3.2.5 2010/03/19 21:42:17 Exp $
 */
#ifdef WIN32
	...
	*/
		...
		case REMOTE_WIFI:
			...
			for (;;) {
				...
				/* Clip output if too much */
				if (retry > 3)
					DPRINT_INFO(OUTPUT, "Shell CMD retry %d\n", retry);
...
rwl_queryinformation_fe(void *wl, int cmd, void* input_buf, unsigned long *input_len, int debug)
{
	...
	switch (remote_type) {
		...
		case REMOTE_SERIAL:
			...
			if (remote_CDC_tx(wl, cmd, input_buf, *input_len,
			                  tx_len, REMOTE_GET_IOCTL, debug) < 0) {
				DPRINT_ERR(ERR, "query_info_fe: Send command failed\n");
				return FAIL;
			}
			...
			*/
			...
			for (retry = 0; retry < RWL_WIFI_RETRY; retry++) {
				...
				if ((error = remote_CDC_wifi_tx(wl, cmd, input_buf,
					*input_len, tx_len, REMOTE_GET_IOCTL)) < 0) {
...
rwl_setinformation_fe(void *wl, int cmd, void* buf, unsigned long *len, int debug)
{
	...
	switch (remote_type) {
		case REMOTE_SOCKET:
			error = rwl_information_socket(wl, cmd, buf, len, len, REMOTE_SET_IOCTL);
			break;
#ifdef RWL_SERIAL
		case REMOTE_SERIAL:
			if (*len > (1024 *10))
			...
			if (remote_CDC_tx(wl, cmd, buf, *len, *len, REMOTE_SET_IOCTL, debug) < 0) {
		...
		case REMOTE_DONGLE:
			 ...
			 */
			tx_len = MIN(*len, 512);
			error = rwl_information_dongle(wl, cmd, buf, len, tx_len, REMOTE_SET_IOCTL);
			break;
...
rwl_usage(int remote_type)
{
	switch (remote_type) {
		case REMOTE_SERIAL:
			fprintf(stderr, " Usage: wl [--linuxdut/linux] [--debug] [--serial port no]\
			...
			fprintf(stderr, "\t--serial enables with serial port number \n");
			fprintf(stderr, "\t--ReadTimeout is applied only when WL is used in remote \
				mode, to specify the ReadTotalTimeoutConstant\n");
			...
			break;
		case REMOTE_DONGLE:
			fprintf(stderr, " Usage: wl --dongle <Device Name>  <command>\n");
			fprintf(stderr, "\t<Device Name> COM1/COM2 (WinXP) or /dev/ttyS0"
			" or /dev/ttyS1 (Linux)\n");
			fprintf(stderr, "\t<command> - wl or shell command\n");
			fprintf(stderr, "\t\t shell command usage: sh <command>\n");
			break;
		case REMOTE_SOCKET:
			fprintf(stderr, " Usage: wl --socket <IP ADDRESS> <PORT>\n");
			...
			break;
		case REMOTE_WIFI:
			fprintf(stderr, " Usage: wl --wifi <MAC Address> <command>\n");
