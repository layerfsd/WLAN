...
#endif
#ifdef __BBSOFT__
	{ "hostAssistedRoaming", NdisParameterInteger, 0, 0, NULL },
...
dhd_open(dhd_info_t *dhd, wl_info_t *wl)
{
	...
	static BOOL _isFirmwareDown = FALSE;
#endif
	...
	*/
	...
	if ((param = wl_readparam(dhd_configs, "hostAssistedRoaming", confighandle, wl, NULL))) {
		dhd_roam = param->ParameterData.IntegerData ? 0 : 1;
	}
#endif //__BBSOFT__
	...
	uint8 *mac_addr, uint32 flags, uint8 bssidx)
{
	...
	printf("%s: idx %d, handle->%p, name = %s\n", __FUNCTION__, ifidx, handle, name);
