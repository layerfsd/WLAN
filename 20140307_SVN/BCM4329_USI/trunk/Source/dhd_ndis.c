...
#if 0 //def CUSTOMER_HW_PT	-->  dhcp 모드에서 특정 packet을 filter 하기 위한 것.
...
#endif
#ifdef __BBSOFT__
	{ "roam_off", NdisParameterInteger, 0, 0, NULL },

	{ "dbgSetOid", NdisParameterInteger, 0, 0, NULL },
	{ "dbgLogLevel", NdisParameterInteger, 0, 0, NULL },
	{ "dbgErrorLog", NdisParameterInteger, 0, 0, NULL },
	{ "dbgFileLog", NdisParameterInteger, 0, 0, NULL },
	{ "dbgLogDir", NdisParameterString, 0, 0, NULL },
#endif //__BBSOFT__
	{ NULL, 0, 0, 0, NULL }
};
...
dhd_open(dhd_info_t *dhd, wl_info_t *wl)
{
...
#ifdef __BBSOFT__
	if ((param = wl_readparam(dhd_configs, "dbgSetOid", confighandle, wl, NULL))) {
		if (param->ParameterData.IntegerData) {
			wl_msg_level |= WL_OID_VAL;
		} else {
			wl_msg_level &= (~WL_OID_VAL);
		}
	}
	
	if ((param = wl_readparam(dhd_configs, "dbgErrorLog", confighandle, wl, NULL))) {
		if (param->ParameterData.IntegerData) {
	...
	if ((param = wl_readparam(dhd_configs, "dbgLogLevel", confighandle, wl, NULL))) {
		switch (param->ParameterData.IntegerData) {
		case 4 :
			dhd_msg_level |= (DHD_DATA_VAL | DHD_BYTES_VAL);
		case 3 :
			dhd_msg_level |= (DHD_CTL_VAL | DHD_HDRS_VAL);
		case 2 :
			dhd_msg_level |= DHD_TRACE_VAL;
			wl_msg_level |= WL_TRACE_VAL;
		case 1 :
			dhd_msg_level |= (DHD_INFO_VAL | DHD_EVENT_VAL);
	...
	*/
#if 0 //def CUSTOMER_HW_PT
	dhd_pkt_filter_enable = 1;
	dhd_master_mode = 1;
	dhd->pub.pktfilter[0] = "100 0 0 0 0x01 0x00";
	dhd->pub.pktfilter_count = 1;
	dhd_pkt_filter_enable = 1;
#else
	...
	*/
...
#ifdef __BBSOFT__
	if ((param = wl_readparam(dhd_configs, "dbgLogDir", confighandle, wl, NULL))) {
		SYSTEMTIME st;
		GetLocalTime(&st);

		wsprintf(g_oslLogFile, _T("%s\\wlan_%04d%02d%02d.txt"),
				param->ParameterData.StringData.Buffer, st.wYear,st.wMonth,st.wDay);
	}

	if ((param = wl_readparam(dhd_configs, "dbgFileLog", confighandle, wl, NULL))) {
		if (param->ParameterData.IntegerData < DBG_OUTPUT_MAX) {
			g_dbgOutputIndex = param->ParameterData.IntegerData;

			if (g_dbgOutputIndex == DBG_OUTPUT_LOG && !g_oslLogFile[0])
				g_dbgOutputIndex = DBG_OUTPUT_KITL;
		}
	}

	printf("Entered the Broadcom 802.11 DCF-only driver version %s " \
	       "(Compiled at %s on %s)\n",
	       EPI_VERSION_STR, __TIME__, __DATE__);


	if ((param = wl_readparam(dhd_configs, "roam_off", confighandle, wl, NULL))) {
		dhd_roam = param->ParameterData.IntegerData ? 1 : 0;
	...
	{
		printf("dhd_bus_init() error...failed in bring up the bus\n");
		return ret;
	}

	/* If bus is not ready, can't come up */
	if (dhd->pub.busstate != DHD_BUS_DATA) {
		printf("Failed in bring up the bus\n");
		return -1;
	}
	...
	clrbit(dhd->pub.eventmask, WLC_E_UNICAST_DECODE_ERROR);
#endif /* DHD_NDIS_OID */
#endif /* EXT_STA */
#ifdef WLBTAMP_HIGH_ONLY
	...
	uint8 *mac_addr, uint32 flags, uint8 bssidx)
{
	...
	DHD_TRACE(("%s: idx %d, handle->%p, name = %s\n", __FUNCTION__, ifidx, handle, name));
...
wl_pnp_set_power(wl_info_t *wl, int devicestate)
{
	...
	case NdisDeviceStateD3:
	{
		...
		printf(" %s : Wake Up Cnt = %d !! \r\n", __FUNCTION__, ++WakeUpCnt);
	}			
		DHD_ERROR(("***** BCMSDDHD on==>off ****\n"));
#ifdef WLBTAMP
#ifdef WLBTCEAMP11
		...
		{ // DeepSleep
			...
			bcm_mkiovar("deepsleep", (void *)&deepSleep, 1, buf, sizeof(buf));
			ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, buf, sizeof(buf));
			if (ret < 0) {
			           DHD_ERROR(("%s: can't set DeepSleep mode. \n", __FUNCTION__, ret));
			}
		}
