...
)
{
	...
	WL_INFORM(("wl%d: wl_mqueryinformation:  oid 0x%x, str = %s\n", wl->sh.unit, Oid, wl_oid2str(Oid)));
	...
	*BytesWritten = 0;
	...
	default:
		...
		if(Oid == OID_802_11_RSSI)
		{
		    printf("%s : Get WLAN RSSI Value \r\n", __FUNCTION__);
		}
...
)
{
	...
	WL_INFORM(("wl%d: wl_msetinformation: oid 0x%x, str = %s \r\n", wl->sh.unit, Oid,wl_oid2str(Oid)));
...
wl_ind_connect_complete(wl_info_t *wl, uint status)
{
	...
	WL_ERROR(("wl%d: EXT STA indication: CONNECTION COMPLETION\n", wl->sh.unit));
...
#ifdef __BBSOFT__	
	to->Configuration.DSConfig = wl_channel2freq(CHSPEC_CHANNEL(bi->ctl_ch? bi->ctl_ch : bi->chanspec)) * 1000;
#else
...
wl_iscan_set_scan(wl_info_t *wl)
{
	...
	if (wl_iscan_request(wl, WL_SCAN_ACTION_START) == 0) {
		...
		printf(" ===> wl_iscan_set_scan: failed. Retry pending...\n");
...
)
{
	...
	WL_ERROR(("Total number of AP's found  = %d\n", max_ap));
...
)
{
	...
	/* Check for scan in progress */
	if (iscan->iscan_state != ISCAN_STATE_IDLE) {
		WL_ERROR(("%s: SIOCGIWSCAN GET still scanning, state = %d\n",
			__FUNCTION__, iscan->iscan_state));
		return 0;
	}
	...
	while (p_buf != iscan->list_cur) {
		...
		if (((wl_info_t *)wl)->lockEnable)
		{
			if( strlen(wl->lockSSID) > 0 )
			{
				if( memcmp(wl->lockSSID, bi->SSID, bi->SSID_len) != 0 )
				{
					continue;
				}
			}
		...
		/*
			if( Bssid->Configuration.DSConfig != 2437000 )
			{
