...
ndis_config_t ndis_configs[] = {
	...
	{ "lockEnable", NdisParameterInteger, 0, 0, NULL },
	{ "lockSSID", NdisParameterString, 0, 0, NULL },
...
)
{
	...
	*/
	...
	((wl_info_t *)wl)->lockEnable = 0;
	memset( ((wl_info_t *)wl)->lockSSID, 0x00, sizeof( ((wl_info_t *)wl)->lockSSID ) );
	if ((param = wl_readparam(ndis_configs, "lockEnable", confighandle, wl, NULL)))
	{
		
		((wl_info_t *)wl)->lockEnable = (int)param->ParameterData.IntegerData;
	}

	if (((wl_info_t *)wl)->lockEnable)
	{
		if ((param = wl_readparam(ndis_configs, "lockSSID", confighandle, wl, NULL))) {
			wchar2ascii(((wl_info_t *)wl)->lockSSID, (ushort *) (param->ParameterData.StringData.
				Buffer), param->ParameterData.StringData.Length,
				sizeof(((wl_info_t *)wl)->lockSSID));
		}
	}
