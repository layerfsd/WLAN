...
#define REG_BCMSDDHD1_PARMS_PATH		L"Comm\\BCMSDDHD1\\Parms"
...
extern "C" codeINSTALL_INIT Install_Init(HWND hwndParent, BOOL fFirstCall, BOOL fPreviouslyInstalled, LPCTSTR pszInstallDir)
{
	...
	// Delete Prev DLL
	DeleteFile(_T("Windows\\bcmsddhd.dll_Back"));
	MoveFile(_T("Windows\\bcmsddhd.dll"), _T("Windows\\bcmsddhd.dll_Back"));

	// Delete All Registry Value
	if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_BCMSDDHD1_PARMS_PATH, 0, 0, &hKey))
	{
		DeleteValues(hKey);
		RegCloseKey(hKey);
	}
