                    ...
                    uint16 devid)
{
	...
	if (DHD_INFO_ON()) {
		...
		for (fn = 0; fn <= numfn; fn++) {
			...
			if (fn == 0) { // terence: check mac address
				if ((err = memcmp((cis[fn]+71), mac_addr, sizeof(mac_addr)))) {
							DHD_ERROR(("MAC ERROR: %02X:%02X:%02X:%02X:%02X:%02X\n",
							*(cis[fn]+71), *(cis[fn]+72), *(cis[fn]+73),
							*(cis[fn]+74), *(cis[fn]+75), *(cis[fn]+76)));
					break;
				}
#ifndef DHD_DEBUG
				break;
