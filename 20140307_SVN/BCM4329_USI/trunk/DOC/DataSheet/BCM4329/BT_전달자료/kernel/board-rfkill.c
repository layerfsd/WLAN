...
static int __init recro_rfkill_probe(struct platform_device *pdev)
{
	int irq;
	int ret;
...
static struct platform_driver recro_device_rfkill = {
	.probe = recro_rfkill_probe,
	.driver = {
		.name = "bt_rfkill",
		.owner = THIS_MODULE,
	},
...
static int __init recro_rfkill_init(void)
{
	int rc = 0;
	rc = platform_driver_register(&recro_device_rfkill);

	return rc;
}

module_init(recro_rfkill_init);
MODULE_DESCRIPTION("recro rfkill");
MODULE_LICENSE("GPL");
