...
wmic_ether_aton(const char *orig, size_t len, A_UINT8 *eth)
{
  ...
  for(bufp = orig; bufp!=orig+len && *bufp; ++bufp) {
    ...
    if (c >= '0' && c <= '9') val = c - '0';
        ...
        NDIS_DEBUG_PRINTF(DBG_TRACE, "%s: MAC value is invalid\n", __FUNCTION__);
        break;
    }
...
update_regCode(unsigned char* eeprom, int size, int regcode)
{
    ...
    /* Change Region Code */
    NDIS_DEBUG_PRINTF(DBG_TRACE, "AR6K: Original Region Code= 0x%02x%02x\n", *(eeprom+9), *(eeprom+8));
...
request_4byte_write(int offset, A_UINT32 data)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "%s: request_4byte_write (0x%x) to offset 0x%x \r\n", __FUNCTION__, data, offset);
...
request_in_progress(void)
{
	...
	{
		NDIS_DEBUG_PRINTF(DBG_TRACE, "%s: EEPROM signaled ERROR (0x%x) \r\n", __FUNCTION__, regval);
    }
...
static void eeprom_type_detect(void)
{
   ...
   /* Wait for DONE_INT in SI_CS */
    do
	{
        ...
        if (regval & SI_CS_DONE_ERR_MASK) 
		{
            NDIS_DEBUG_PRINTF(DBG_TRACE, "%s: ERROR : address type was wrongly set\n", __FUNCTION__);
            break;
        }
		
        if (i++ == EEPROM_WAIT_LIMIT) 
		{
            NDIS_DEBUG_PRINTF(DBG_TRACE, "%s: EEPROM not responding\n", __FUNCTION__);
        }
...
static int read_eeprom_from_file(char *fake_file, A_UCHAR *eeprom_data)
{
        ...
        if (IS_ERR(filp)) 
		{
            NDIS_DEBUG_PRINTF(DBG_ERR, "%s: file %s filp_open error\n", __FUNCTION__, fake_file));
            set_fs(oldfs);
        	return -1;
        }
        ...
        if (!filp->f_op) 
		{
            NDIS_DEBUG_PRINTF(DBG_ERR, "%s: File Operation Method Error\n", __FUNCTION__);
        ...
        if (!inode) {
            NDIS_DEBUG_PRINTF(DBG_TRACE "%s: Get inode from filp failed\n", __FUNCTION__);
            filp_close(filp, NULL);
            set_fs(oldfs);
        return -1;
        }
...
void eeprom_ar6000_transfer(HIF_DEVICE *device, char *fake_file, char *p_mac, int regcode, char *c_mac)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "%s: Enter, fake_file = %s \n", __func__, fake_file);
    ...
    if (fake_file) 
	{
        ...
        else 
		{
			...
			printk(" eeprom_ar6000_transfer fail !!! \r\n");
            return;
        }
#endif //bluebird		
	}
	...
	if (!is_read_eeprom_from_file) 
	{
		...
		*/
		...
		{
			NDIS_DEBUG_PRINTF(DBG_TRACE, "Did not find EEPROM with valid Board Data.\n");
		}
	...
	{
#if 0 //bluebird	
		NDIS_DEBUG_PRINTF(DBG_TRACE, "eeprom_ar6000 : p_mac \r\n");
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "eeprom_ar6000 : BMI_read_mem \r\n");
	
	BMI_read_mem( HOST_INTEREST_ITEM_ADDRESS(hi_board_data), &board_data_addr);
	
	if (board_data_addr == 0) 
	{
		NDIS_DEBUG_PRINTF(DBG_TRACE, "hi_board_data is zero\n");
	}

	if (regcode != 0) 
	{
		NDIS_DEBUG_PRINTF(DBG_TRACE, "update_regCode : 0x%x\n", regcode);
	...
	disable_SI();
	NDIS_DEBUG_PRINTF(DBG_TRACE, " eeprom_ar6000 : Exit !! \r\n");
}
