...
#if 0 //bluebird  exsit at ar6k_drv.h
                       ...
                       HTC_EVENT_ID evId, HTC_EVENT_INFO *evInfo, void *arg)
{
    ...
    if ( WAIT_FAILED == WaitForSingleObject(raw_htc_read_sem[endPointId], INFINITE) ) {
		...
		NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to wait on the Semaphore \r\n");
	}
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "Waking up the endpoint(%d) read process \r\n", endPointId);
                        ...
                        HTC_EVENT_ID evId, HTC_EVENT_INFO *evInfo, void *arg)
{
    ...
    if ( WAIT_FAILED == WaitForSingleObject(raw_htc_write_sem[endPointId], INFINITE) ) {
		...
		NDIS_DEBUG_PRINTF(DBG_TRACE, "Unable to wait on the Semaphore \r\n");
	}
    ...
    /* Signal the waiting process */
	NDIS_DEBUG_PRINTF(DBG_TRACE, "Waking up the endpoint(%d) write process \r\n", endPointId);
                         ...
                         HTC_EVENT_ID evId, HTC_EVENT_INFO *evInfo, void *arg)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "Not Implemented \r\n");
}
                     ...
                     char *buffer, int length)
{
    ...
    while (!write_buffer_available[endPointId]) {
		...
		NDIS_DEBUG_PRINTF(DBG_TRACE, "Sleeping endpoint(%d) write process \r\n", endPointId);
...
CAR6KMini::ar6000_get_target_stats()
{
	...
	if( !NdisWaitEvent(&m_tgtStatsEvent, AR6K_DEFAULT_MS_TO_WAIT_FOR_TGT_GETSTATS) ) {
		...
		NDIS_DEBUG_PRINTF(DBG_ERR, "AR6K: ERROR - No TGT_GETSTATS event after %u ms \r\n", AR6K_DEFAULT_MS_TO_WAIT_FOR_TGT_GETSTATS);
		status = A_ERROR;
	}

	NDIS_DEBUG_PRINTF(DBG_TRACE, " %s() Occur !! \r\n", __FUNCTION__);
...
//
{
	...
	//NDIS_DEBUG_PRINTF(DBG_QUERY_OID,"[CAR6K] %s() + \r\n",__FUNCTION__);
	...
	OUT PULONG   pcbNeeded)
{
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() + \r\n",__FUNCTION__);
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE," SetInformation() -> Oid = 0x%08x \r\n",Oid);
...
//
{
	PNDIS_OID   p_Car6kOidListStart=0;
...
{
	NDIS_DEBUG_PRINTF(DBG_QUERY_OID," %s() + \r\n",__FUNCTION__);
...
// The current hardware status of the underlying NIC 
{
	...
	NDIS_DEBUG_PRINTF(DBG_QUERY_OID," %s() + m_WMIReady = %d \r\n",__FUNCTION__,m_WMIReady);
...
// Whether the media is currently connected and able to Tx/Rx packets.
{
	...
	NDIS_DEBUG_PRINTF(DBG_QUERY_OID," %s() + m_Connected = %d \r\n",__FUNCTION__,m_Connected);
...
void CAR6KMini::ChangeCurrentPowerState (DWORD data)
{
    ...
    {
		NDIS_DEBUG_PRINTF (DBG_ERR, "ChangeCurrentPowerState Failed\n");
    }
         ...
         )
{
	...
	DWORD       sysPwrState;
	ULONG       pwrMgmtMode = POWER_MANAGEMENT_MODE_CUT_PWR;
	DWORD       result;

	static BOOL	m_bIsSuspend;
	static BOOL m_bIsWiFiOnOff;

	NDIS_DEBUG_PRINTF(1," %s() + *pDevPowerState = %d\r\n",__FUNCTION__,*pDevPowerState);
	...
	if (*pDevPowerState == NdisDeviceStateD3) 
	{
		if (!m_WMIReady) 
		{
			...
			NDIS_DEBUG_PRINTF(1, " %s() : NDIS_STATUS_ADAPTER_NOT_READY !! \r\n", __FUNCTION__);
		}
		else 
		{
			...
			pwrMgmtMode = POWER_MANAGEMENT_MODE_CUT_PWR;

			if((m_bIsSuspend == TRUE)&&(pwrMgmtMode == POWER_MANAGEMENT_MODE_CUT_PWR))
			{
	...
	else if (*pDevPowerState == NdisDeviceStateD0) 
	{
		...
		if (StartEndpoints() != A_OK)
		{
				...
				NDIS_DEBUG_PRINTF (DBG_ERR, "WiFi ReEnabled and StartEndpont failed\n");
		}		
 	...
 	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() +  3\r\n", __FUNCTION__);
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() - Exit\r\n", __FUNCTION__);
	return Status;
}
	...
	OUT NDIS_802_11_SSID *pSSID)
{
	...
	NDIS_DEBUG_PRINTF(DBG_QUERY_OID, " %s() ,m_Connected = %s  \r\n",__FUNCTION__, (m_Connected)? "TRUE":"FALSE");
	...
	OUT NDIS_802_11_RSSI *pRSSI)
{
	...
	if (!m_WMIReady) {
		NDIS_DEBUG_PRINTF(DBG_QUERY_OID, " %s() + m_WMIReady = FALSE \r\n", __FUNCTION__);
	...
	*pRSSI = m_RSSI; //RSSI_TO_NDIS(m_RSSI);
...
//
{
	...
	if (*m_pcbBSSIDListNeeded + spaceNeeded < m_cbBSSIDList)
	{
		...
		if (pIE)
		{
			...
			/* check specific AP, we show only registed AP*/
			if(m_Config.apEnable)
			{
				int	i,ret;
				UCHAR *pTempSSID;

				/* max specific AP is 2 */
				for(i=0; i<2; i++)
				{

					pTempSSID = (i==0)? m_Config.apSSID : m_Config.apSSIDa;
					
					ret = memcmp(pTempSSID,&pIE[2],pIE[1]);
					if((ret==0)&&(	strlen((const char*)pTempSSID) == pIE[1]))
					{
							break;
					}
				}
		...
		if((this->m_Connected) && (memcmp(m_PeerBSSID,pBSSID->MacAddress,ETHERNET_MAC_ADDRESS_LENGTH)==0))
		{
				if(m_RSSI != 0)
					pBSSID->Rssi = m_RSSI;
				else
					pBSSID->Rssi = pWmiBss->ni_rssi;
				NDIS_DEBUG_PRINTF(DBG_BSS_LIST,"Update Current Connected AP's RSSI value, AP = %s !! \r\n",pBSSID->Ssid.Ssid);
		}
		else
			pBSSID->Rssi = pWmiBss->ni_rssi;//RSSI_TO_NDIS(pWmiBss->ni_rssi);


		NDIS_DEBUG_PRINTF(DBG_BSS_LIST,"#BSS_LIST :: pBSSID->SSID = %s,RSSI = %d, ch = %d \r\n", pBSSID->Ssid.Ssid,pBSSID->Rssi,pWmiBss->ni_cie.ie_chan);
...
//
{
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() +, wmiNetworkType = %d \r\n",__FUNCTION__,wmiNetworkType);
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID," wmiDot11AuthMode = %d, wmiAuthMode = %d \r\n", wmiDot11AuthMode,wmiAuthMode);
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID, " m_PairwiseCryptoType = %d,m_GrpwiseCryptoType = %d \r\n", m_PairwiseCryptoType,m_GrpwiseCryptoType);

	pAPBSSID = NULL;
	if (!IsBroadcastAddress(m_DesiredAPBSSID))
		pAPBSSID = m_DesiredAPBSSID;

	NDIS_DEBUG_PRINTF(DBG_TRACE, "Connect command from Supplicant \r\n");
	...
	Lock();
	wmiStatus = wmi_listeninterval_cmd((struct wmi_t *)m_pWMI, MAX_LISTEN_INTERVAL, 0 );
	Unlock();

	if (wmiStatus != A_OK) {
	    NDIS_DEBUG_PRINTF(DBG_TRACE,"AR6K: ERROR - wmi_listeninterval_cmd failed = %u \r\n", wmiStatus);
		return NDIS_STATUS_FAILURE;
	}
	...
	wmiStatus = wmi_connect_cmd((wmi_t *)m_pWMI, wmiNetworkType, 
				...
				pAPBSSID, channel,CONNECT_IGNORE_WPAx_GROUP_CIPHER);
	Unlock();
	
	if (wmiStatus != A_OK) 
	{
	    NDIS_DEBUG_PRINTF(DBG_TRACE, "AR6K: ERROR - wmi_connect_cmd failed = %u \r\n", wmiStatus);
		Status = NDIS_STATUS_FAILURE;
    }
	else 
	{
...
//
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "Disconnect from supplicant \r\n");

	if (wmiStatus != A_OK) 
	{
	    NDIS_DEBUG_PRINTF(DBG_TRACE, "AR6K: ERROR - wmi_disconnect_cmd failed = %u \r\n", wmiStatus);
		Status = NDIS_STATUS_FAILURE;
    }
...
//
{
	...
	if (m_RadioDisabled) 
	{
		...
		if (wmiStatus != A_OK)
			NDIS_DEBUG_PRINTF(DBG_SET_OID,"AR6K: ERROR - wmi_startscan_cmd failed = %u \r\n", wmiStatus);
	}
	...
	//NDIS_DEBUG_PRINTF(1, "%s() -> ssidLength = %d \r\n",__FUNCTION__, SSID->SsidLength);
	...
	/* if the probeEnable is setted then we are searching a hidden ap */
	if((m_Config.probeEnable == 1)&&(m_WantToBeConnected == false))
	{
		int len = 0;
		char hiddenSSID[32];	// SSID max length is 32

		if(m_SelectHiddenSsid == 0) {// select hidden AP1
			memcpy(hiddenSSID,m_Config.probeSSID,32);
			m_SelectHiddenSsid = 1;
		} else {
			memcpy(hiddenSSID,m_Config.probeSSIDa,32);
			m_SelectHiddenSsid = 0;
		}

		NDIS_DEBUG_PRINTF(DBG_SET_OID,"### force to set [%s] \r\n", hiddenSSID);
		len = strlen(hiddenSSID);

		if(len)
		{
			SSID->SsidLength = len;
			memcpy(SSID->Ssid, m_Config.probeSSID,len+1);
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE,"[AR6k] %s() ->length = %d, SSID = %s \r\n",__FUNCTION__,SSID->SsidLength,SSID->Ssid);
	NDIS_DEBUG_PRINTF(DBG_TRACE, " m_WantToBeConnected = %d \r\n", m_WantToBeConnected);
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID, "%s() - Exit, status = %d \r\n",__FUNCTION__, Status);
 	return Status;
}
	...
	IN NDIS_802_11_NETWORK_INFRASTRUCTURE Mode)
{
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() + Mode = %d \r\n", __FUNCTION__, Mode);
...
#else
	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() is not ready~~~ !! \r\n", __FUNCTION__);
#endif	//bluebird
...
//
{
	...
	if (m_ConnectInProgress || m_Connected)
	{
		/* Add for update RSSI value at connected status 120430*/
		if(m_Connected) {
			Lock();
			wmi_get_stats_cmd((wmi_t*)m_pWMI);	
	...
	Lock();
	wmiStatus = wmi_startscan_cmd((struct wmi_t *)m_pWMI, WMI_LONG_SCAN, FALSE, FALSE, m_Config.dwellTime, 0,0, NULL);
	Unlock();
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID," %s() -> Status = %d \r\n", __FUNCTION__, Status);
	return Status;
}
	...
	IN NDIS_802_11_KEY_INDEX KeyIndex)
{
	...
	NDIS_DEBUG_PRINTF(DBG_SET_OID, "AR6K: RemoveWEP from Supplicant \r\n");
	...
	if (wmiStatus != A_OK) {
	    NDIS_DEBUG_PRINTF(DBG_SET_OID, "AR6K: ERROR - wmi_deleteKey_cmd failed = %u \r\n", wmiStatus);
		return NDIS_STATUS_FAILURE;
	}
	return NDIS_STATUS_SUCCESS;
}
	...
	IN NDIS_802_11_KEY *pKey)
{
...
#if 0
	{
		...
		NDIS_DEBUG_PRINTF(DBG_SET_OID, "m_key[] = ");
		for(j=0; j<pKey->KeyLength; j++) 
		{
			NDIS_DEBUG_PRINTF(DBG_SET_OID, "[%02x] ",m_Key[j]);
		}
		NDIS_DEBUG_PRINTF(DBG_SET_OID, " \r\n");
	}
#endif
	...
	if (NONE_CRYPT != m_PairwiseCryptoType) 
	{
		...
		if (wmiStatus != A_OK) 
		{
		    NDIS_DEBUG_PRINTF(DBG_SET_OID, "AR6K: ERROR - wmi_addKey_cmd failed = %u \r\n", wmiStatus);
			return NDIS_STATUS_FAILURE;
		}
	}
...
{
	A_STATUS status = A_OK;
	HTC_TARGET *target = GET_HTC_TARGET_FROM_HANDLE(m_hHtcTarget);
	...
	if (!rawIfInit) {
		...
		if ( status == A_OK ) {
			status = (A_STATUS) ar6000_htc_raw_open(target);
		}
	...
	void)
{
	...
	if (rawIfInit) {
		status = (A_STATUS) ar6000_htc_raw_close(target);
		rawIfInit = false;
	} else {
		return NDIS_STATUS_FAILURE;
	}
	...
	HTC_RAW_READ_PARAM *Buffer)
{
	...
	if (rawIfInit) {
		if ((bufLen = (unsigned int) ar6000_htc_raw_read(target, (HTC_ENDPOINT_ID)Buffer->endPointId, \
	...
	HTC_RAW_WRITE_PARAM *Buffer)
{
	...
	if (rawIfInit) {
		if ( ar6000_htc_raw_write(target, (HTC_ENDPOINT_ID)Buffer->endPointId, \
			(&(Buffer->buffer)), Buffer->length) != -1) {
			status = A_OK;
		} else {
			status = A_ERROR;
						  ...
						  IN  ULONG              cbBuffer)
{
	...
	if(tempPMKID == NULL) 
	{
		NDIS_DEBUG_PRINTF(DBG_SET_OID, "%s() malloc failed\r\n", __FUNCTION__);
		Sleep(1000);
	}
