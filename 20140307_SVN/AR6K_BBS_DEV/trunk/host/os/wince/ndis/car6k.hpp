...
typedef struct WlanConfig		// Station Configuration Structure for Reading values from Registry.
{
	...
	ULONG	nodeAge;				// millisecs, BSS list aging 
	ULONG	tcmd;					// RF test mode, enable=1/disable=0

	/* about hidden AP */
	ULONG	probeEnable;			// enable hidden SSID probe
	UCHAR	probeSSID[32];				// hidden SSID for probe
	UCHAR	probeSSIDa[32];				// hidden SSID for probe
	...
	/* Channel List */
	UCHAR	channelList[96];

	/* about Roaming */
	ULONG	RoamEnable;				// rssi based roaming, enable=1/disable=0
	ULONG	hostAssistedRoaming;	// driver roam
	ULONG	RoamFloorThre;			
	ULONG	RoamThreshold;
	ULONG	ScanThreshold;
	
	ULONG	tp;						// thread priority 0:disable, 100-150:enable
	ULONG	dwellTime;				// scan ch dwell time (msecs)
...
extern "C" void eeprom_ar6000_transfer(HIF_DEVICE *device, char *fake_file, char *p_mac, int regcode, char *c_mac);
...
#define DEFAULT_MAX_TRANSMIT_BUFFERS 20
...
//	HTC_TARGET                         *m_pHTCTarget;
	...
	A_UINT32							m_enableuartprint;
	A_UINT32 							m_enabletimerwar;
	A_UINT32 							m_fwmode;
	A_UINT32							m_mbox_yield_limit;
	A_UINT32							m_processDot11Hdr;
	HTC_HANDLE							m_hHtcTarget;
	boolean								m_bIsDestroyProgress;
	A_UINT32							m_RefClock; 
	boolean								m_bDebugdriver;
	A_UINT32							m_WorkMode;
	A_UINT32							m_ChanNum;	
    BYTE								m_NumDataEndPts;

    HTC_ENDPOINT_ID						m_Ac2EpMapping[WMM_NUM_AC];
    boolean								m_AcStreamActive[WMM_NUM_AC];
    BYTE								m_AcStreamPriMap[WMM_NUM_AC];
	BYTE								m_Ep2AcMapping[ENDPOINT_MAX];
	COMMON_CREDIT_STATE_INFO			m_CreditStateInfo;

	struct ar_cookie					m_Cookie_mem[MAX_COOKIE_NUM];
    struct ar_cookie					*m_CookieList;

	BYTE								m_PhyCapability;
    BYTE								m_HiAcStreamActivePri;
    BYTE								m_HiRXAcStreamActivePri;
	
	DWORD								m_MgmtFilter;

	AR6000_WLAN_STATE					m_WlanState;
	
public:
	boolean								m_WMIEnabled;
	
	INT									m_RxBuffers[ENDPOINT_MAX];			
	HTC_ENDPOINT_ID						m_ControlEp;
	boolean								m_DbglogInitDone;
	
	INT 								m_SelectHiddenSsid;

	A_UINT8								m_ScanCtrlFlag;
...
// Parse Registry
	...
	void				SetWmiHandle(struct wmi_t *wmi);
	A_STATUS			Ar6000SendPkt(HTC_ENDPOINT_ID endPointId, PVOID osbuf, WORD PktType);
	void				IndicateTxActivity(A_UINT8 TrafficClass, A_BOOL Active);

	struct ar_cookie *		AllocCookie(void);
	NDIS_STATUS				TargetInitialize(void);
...
private:
	...
	A_STATUS				AR6KConfigureTarget(HIF_DEVICE *device);
#if 0	
	A_STATUS				DownloadImage(void);
	void					firmware_transfer(HIF_DEVICE *device, int file_id, A_UINT32 address, A_BOOL isCompressed);
#endif

	A_STATUS				DeviceInit(void);
	A_STATUS				AR6KConnectSvc(HTC_SERVICE_CONNECT_REQ  *pConnect, char *pDesc);

	void					SetAc2EndpointIDMap(BYTE ac, HTC_ENDPOINT_ID ep);
	void					CookieInit(void);
	void					CookieCleanUp(void);
	void					FreeCookie(struct ar_cookie * cookie);
	A_STATUS				SetAR6KPowerStatus(AR6000_WLAN_STATE state);
	void					TxDataCleanup(void);	
public:
	void                  	IndicateReceivePackets();	
	void					AR6KRxRefill(HTC_ENDPOINT_ID Endpoint);
	HTC_ENDPOINT_ID			Ac2EndpointID(BYTE ac);
	void					BssInfoEventRx(A_UINT8 *datap, int len);
...
public:
	...
	void      ReceiveWMIControlPacket(HTC_PACKET  *evInfo);
	void      ReceiveWMIDataPacket(HTC_PACKET  *evInfo);
	...
	#if 1
	void	WMIConnectIndication( IN USHORT	Channel, IN PBYTE PeerBSSID, IN USHORT listenInterval, 
										IN USHORT	beaconInterval,NETWORK_TYPE	networkType, IN BYTE assocReqLen, 
