    ...
    do {
        if ((request & HIF_ASYNCHRONOUS) || (request & HIF_SYNCHRONOUS)){
            ...
            if (request & HIF_SYNCHRONOUS) 
			{
                ...
                /* wait for completion */
                up(&device->sem_async);
                if (down_interruptible(&busrequest->sem_req) != 0) /* wait for complete async_task's working */
				{
                    /* interrupted, exit */
                    return A_ERROR;
                } 
...
/* ATHENV */
    ...
    if ((osdrvCallbacks.deviceInsertedHandler(osdrvCallbacks.context,device)) != A_OK) {
		...
		printk("deviceInsertedHandler error !! \r\n");
    }
/* ATHENV */
#ifdef ANDROID_ENV
...
static int hifDeviceInserted(struct sdio_func *func, const struct sdio_device_id *id)
{
...
#endif
	/*enable device,  ready to use mmc device */
    ...
    sdio_release_host(func);	/* release sdio bus */
    if (ret) {
