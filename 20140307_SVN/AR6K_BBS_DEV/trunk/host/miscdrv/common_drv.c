...
A_STATUS ar6000_SetAddressWindowRegister(HIF_DEVICE *hifDevice, A_UINT32 RegisterAddr, A_UINT32 Address)
{
    ...
    if (status != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_TRACE, "Cannot write initial bytes of 0x%x to window reg: 0x%X \n", RegisterAddr, Address);
        return status;
    }
    ...
    if (status != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_TRACE, "Cannot write 0x%x to window reg: 0x%X \n", RegisterAddr, Address);
        return status;
    }
...
A_STATUS ar6000_SetAddressWindowRegister(HIF_DEVICE *hifDevice, A_UINT32 RegisterAddr, A_UINT32 Address)
{
    ...
    if (status != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "Cannot write initial bytes of 0x%x to window reg: 0x%X \n", RegisterAddr, Address);
        return status;
    }
    ...
    if (status != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "Cannot write 0x%x to window reg: 0x%X \n", RegisterAddr, Address);
        return status;
    }
...
ar6000_ReadRegDiag(HIF_DEVICE *hifDevice, A_UINT32 *address, A_UINT32 *data)
{
    ...
    status = HIFReadWrite(hifDevice,
                          ...
                          NULL);
    if (status != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "Cannot read from WINDOW_DATA_ADDRESS\n");
		return status;
    }
...
ar6000_WriteRegDiag(HIF_DEVICE *hifDevice, A_UINT32 *address, A_UINT32 *data)
{
    ...
    status = HIFReadWrite(hifDevice,
                          ...
                          NULL);
    if (status != A_OK) {
		NDIS_DEBUG_PRINTF(DBG_ERR, "Cannot write 0x%x to WINDOW_DATA_ADDRESS\n", *data);
        return status;
    }
...
A_STATUS ar6000_reset_device(HIF_DEVICE *hifDevice, A_UINT32 TargetType, A_BOOL waitForCompletion)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, " %s() + Enter, Type(%d) \r\n", __FUNCTION__, TargetType);
    ...
    do {
        ...
        NDIS_DEBUG_PRINTF(DBG_TRACE, "Reset Cause readback: 0x%X \n",data);
        data &= RESET_CAUSE_LAST_MASK;
        if (data != 2) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to cold reset the target \n");
        }

    } while (FALSE);

    if (A_FAILED(status)) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "Failed to reset target \n");
    }
...
void ar6000_dump_target_assert_info(HIF_DEVICE *hifDevice, A_UINT32 TargetType)
{
    ...
    do {
        ...
        if (A_FAILED(status)) {
            NDIS_DEBUG_PRINTF(DBG_ERR,"AR6K: Failed to get ptr to register dump area \n");
            break;
        }

        NDIS_DEBUG_PRINTF(DBG_TRACE,"AR6K: Location of register dump data: 0x%X \n",regDumpArea);
        ...
        if (A_FAILED(status)) {
            NDIS_DEBUG_PRINTF(DBG_ERR,"AR6K: Failed to get register dump \n");
            break;
        }
        NDIS_DEBUG_PRINTF(DBG_TRACE,"AR6K: Register Dump: \n");

        for (i = 0; i < regDumpCount; i++) {
            NDIS_DEBUG_PRINTF(DBG_TRACE," %d :  0x%8.8X \n",i, regDumpValues[i]);
        }
                               ...
                               A_UINT8     HtcControlBuffers)
{
	...
	do 
	{
		...
		if (A_FAILED(status)) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR,"Failed to get block size info from HIF layer...\n");
			break;
		}
		...
		* size on mailbox 0 is artificially set to 1 */
		...
		AR_DEBUG_ASSERT((blocksizes[1] & (blocksizes[1] - 1)) == 0);
		...
		if (A_FAILED(status)) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR,"BMIWriteMemory for IO block size failed \n");
			break;
		}

		NDIS_DEBUG_PRINTF(DBG_TRACE,"Block Size Set: %d (target address:0x%X)\r\n",
		...
		if (MboxIsrYieldValue != 0) 
		{
			...
			if (A_FAILED(status)) 
			{
				NDIS_DEBUG_PRINTF(DBG_ERR,"BMIWriteMemory for yield limit failed \n");
				break;
			}
		}
...
static A_STATUS prepare_ar6002(HIF_DEVICE *hifDevice, A_UINT32 TargetVersion)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "prepare_ar6002() \r\n");
    /* placeholder */
...
static A_STATUS prepare_ar6003(HIF_DEVICE *hifDevice, A_UINT32 TargetVersion)
{
    ...
    /* placeholder */
	NDIS_DEBUG_PRINTF(DBG_TRACE, "prepare_ar6003() \r\n");
...
{
	NDIS_DEBUG_PRINTF(DBG_TRACE, "ar6000_prepare_target() -> type = %d, Ver = 0x%08x \r\n", TargetType,TargetVersion);
...
ar6002_REV1_reset_force_host (HIF_DEVICE *hifDevice)
{
    ...
    NDIS_DEBUG_PRINTF (DBG_TRACE,"Force Target to recognize Host....\n");
    for (i = 0; i < szForceROM; i++)
    {
        ...
        {
            NDIS_DEBUG_PRINTF (DBG_TRACE, "Cannot force Target to recognize Host!\n");
            return A_ERROR;
        }
    }
