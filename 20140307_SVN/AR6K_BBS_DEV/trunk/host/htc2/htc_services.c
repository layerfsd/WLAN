...
void HTCControlRecv(void *Context, HTC_PACKET *pPacket)
{
	 ...
	 * never get here */
	NDIS_DEBUG_PRINTF(DBG_ERR, "HTCControlRecv, got message with length:%d \n", pPacket->ActualLength + HTC_HDR_LENGTH);
                           ...
                           HTC_SERVICE_CONNECT_RESP *pConnectResp)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "+HTCConnectService, target:0x%X SvcID:0x%X \r\n",
    ...
    do {
		...
		else 
		{
            ...
            if (pResponseMsg->Status != HTC_SERVICE_SUCCESS) {
				NDIS_DEBUG_PRINTF(DBG_ERR, " Target failed service 0x%X connect request (status:%d)\n",
		...
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " %s() EndpointId =%d, MaxTxQueueDepth = %d \r\n",__FUNCTION__,
								assignedEndpoint,pConnectReq->MaxSendQueueDepth);
