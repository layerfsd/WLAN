...
static __inline void DO_RCV_COMPLETION(HTC_TARGET *t ,HTC_PACKET *p ,HTC_ENDPOINT *e)            \
{                                           \
    if ((p)->ActualLength > 0) {            \
        (e)->EpCallBacks.EpRecv((e)->EpCallBacks.pContext,      \
                                (p));                           \
    } else {                                                    \
        NDIS_DEBUG_PRINTF(DBG_TRACE, " recycling empty packet \n");  \
...
static __inline A_STATUS HTCProcessTrailer(HTC_TARGET *target,
                                         A_UINT8    *pBuffer,
                                         int         Length,
                                         ...
                                         HTC_ENDPOINT_ID FromEndpoint)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "+HTCProcessTrailer (length:%d) \n", Length);
    ...
    while (Length > 0) {
        ...
        if (pRecord->Length > Length) {
            ...
            NDIS_DEBUG_PRINTF(DBG_ERR, " invalid record length: %d (id:%d) buffer has: %d bytes left \r\n",
        ...
        switch (pRecord->RecordID) {
            ...
            case HTC_RECORD_LOOKAHEAD:
                ...
                if ((pLookAhead->PreValid == ((~pLookAhead->PostValid) & 0xFF)) &&
                    ...
                    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC," LookAhead Report Found (pre valid:0x%X, post valid:0x%X) \r\n",
                ...
                break;
            default:
                NDIS_DEBUG_PRINTF(DBG_ERR, "unhandled record: id:%d length:%d \n", pRecord->RecordID, pRecord->Length);
                break;
        }
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "-HTCProcessTrailer \r\n");
    return status;
...
static A_STATUS HTCProcessRecvHeader(HTC_TARGET *target, HTC_PACKET *pPacket, A_UINT32 *pNextLookAhead)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "+HTCProcessRecvHeader \n");
    ...
    do {
        ...
        if (lookAhead != pPacket->HTCReserved) {
             ...
             * reflect the actual header in the pending message */
             NDIS_DEBUG_PRINTF(DBG_ERR, "HTCProcessRecvHeader, lookahead mismatch! \r\n");
...
void HTCRecvCompleteHandler(void *Context, HTC_PACKET *pPacket)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "+HTCRecvCompleteHandler (status:%d, ep:%d) \r\n",
    ...
    do {
        if (A_FAILED(status)) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "HTCRecvCompleteHandler: request failed (status:%d, ep:%d) \r\n",
                pPacket->Status, pPacket->Endpoint);
            break;
        }
        ...
        if (nextLookAhead != 0) {
            A_STATUS nextStatus;
            NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "HTCRecvCompleteHandler - next look ahead was non-zero : 0x%X \r\n", nextLookAhead);
            ...
            if (A_EPROTO == nextStatus) {
                NDIS_DEBUG_PRINTF(DBG_ERR,"Next look ahead from recv header was INVALID\n");
                DebugDumpBytes((A_UINT8 *)&nextLookAhead, 4, "BAD lookahead from lookahead report");
            }
        } else {
             NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "HTCRecvCompleteHandler - rechecking for more messages... \r\n");
    ...
    if (A_FAILED(status)) {
         NDIS_DEBUG_PRINTF(DBG_ERR, "HTCRecvCompleteHandler , message fetch failed (status = %d) \r\n", status);
            /* recyle this packet */
         HTC_RECYCLE_RX_PKT(target, pPacket, pEndpoint);
    }

    NDIS_DEBUG_PRINTF(DBG_TRACE, "-HTCRecvCompleteHandler \r\n");
}
...
A_STATUS HTCWaitforControlMessage(HTC_TARGET *target, HTC_PACKET **ppControlPacket)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "+HTCWaitforControlMessage \n");
	...
	do  
	{
		...
		*ppControlPacket = NULL;
		...
		status = DevPollMboxMsgRecv(&target->Device, &lookAhead, HTC_TARGET_RESPONSE_TIMEOUT);
		...
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "HTCWaitforControlMessage : lookAhead : 0x%X \r\n", lookAhead);
		...
		if (A_FAILED(status)) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "HTCWaitforControlMessage, HTCProcessRecvHeader failed (status = %d) \r\n", status);
			break;
		}
...
A_STATUS HTCRecvMessagePendingHandler(void *Context, A_UINT32 *LookAhead, A_BOOL *pAsyncProc)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE,"+%s() -> LookAhead:0x%X \n",__FUNCTION__, *LookAhead);
	...
	while (TRUE) 
	{
		...
		{
			NDIS_DEBUG_PRINTF(DBG_ERR,"Invalid Endpoint in look-ahead: %d \n",pHdr->EndpointID);
			/* invalid endpoint */
			status = A_EPROTO;
			break;
		}
		...
		{
			NDIS_DEBUG_PRINTF(DBG_ERR,"Payload length %d exceeds max HTC : %d !\n",
		...
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "Endpoint %d is not connected !\n",pHdr->EndpointID);
		...
		*LookAhead = 0;
		...
		/* call ar6000_rx() */
		NDIS_DEBUG_PRINTF(DBG_TRACE, "%s() call-> ar6000_rx() \r\n", __FUNCTION__);
		DO_RCV_COMPLETION(target,pPacket,pEndpoint);
	...
	{
		NDIS_DEBUG_PRINTF(DBG_ERR, " Endpoint :%d has no buffers, blocking receiver to prevent overrun.. \n",
	...
	else if (A_FAILED(status)) 
	{
		...
		if (HTC_STOPPING(target)) 
		{
			NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " HTC stopping... blocking receiver to finish the stop \r\n");
			DevStopRecv(&target->Device, asyncProc ? DEV_STOP_RECV_ASYNC : DEV_STOP_RECV_SYNC);    
		}
	}

	NDIS_DEBUG_PRINTF(DBG_TRACE,"- %s() Exit, status = %d \r\n",__FUNCTION__, status);
...
A_STATUS HTCAddReceivePkt(HTC_HANDLE HTCHandle, HTC_PACKET *pPacket)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "+- %s(): endPointId: %d, buffer: 0x%X, length: %d \r\n",__FUNCTION__,
	...
	do 
	{
		...
		if (target->HTCStateFlags & HTC_STATE_WAIT_BUFFERS) 
		{
			...
			{
				NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " receiver was blocked on ep:%d, unblocking.. \r\n", target->EpWaitingForBuffers);
				target->HTCStateFlags &= ~HTC_STATE_WAIT_BUFFERS;
...
void HTCUnblockRecv(HTC_HANDLE HTCHandle)
{
    ...
    LOCK_HTC_RX(target);
        ...
        /* check if we are blocked waiting for a new buffer */
    if (target->HTCStateFlags & HTC_STATE_WAIT_BUFFERS) {
        AR_DEBUG_PRINTF(0,("HTCUnblockRx : receiver was blocked on ep:%d, unblocking.. \n",
            target->EpWaitingForBuffers));
        target->HTCStateFlags &= ~HTC_STATE_WAIT_BUFFERS;
        target->EpWaitingForBuffers = ENDPOINT_MAX;
        unblockRecv = TRUE;
...
static void HTCFlushEndpointRX(HTC_TARGET *target, HTC_ENDPOINT *pEndpoint)
{
    ...
    while (1) {
        ...
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "Flushing RX packet:0x%X, length:%d, ep:%d \r\n",
