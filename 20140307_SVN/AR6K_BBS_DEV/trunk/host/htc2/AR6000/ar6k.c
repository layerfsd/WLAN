...
{
	A_UINT32 mailboxaddrs[AR6K_MAILBOXES];	//#define AR6K_MAILBOXES 4
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "DevSetup + Enter \r\n");
	...
	do 
	{
		...
		NDIS_DEBUG_PRINTF(DBG_TRACE,"BlockSize: %d, MailboxAddress:0x%X \r\n",
        ...
        switch (pDev->HifIRQProcessingMode) 
		{
			case HIF_DEVICE_IRQ_SYNC_ONLY:
				NDIS_DEBUG_PRINTF(DBG_TRACE, "HIF Interrupt processing is SYNC ONLY \r\n");
				break;
			case HIF_DEVICE_IRQ_ASYNC_SYNC:
				NDIS_DEBUG_PRINTF(DBG_TRACE, "HIF Interrupt processing is ASYNC and SYNC \r\n");
				break;
			default:
				AR_DEBUG_ASSERT(FALSE);
		}
		...
		NDIS_DEBUG_PRINTF(DBG_TRACE,"HIF special overrides : 0x%X , 0x%X\n",
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "DevSetup - Exit -> status (%d) !! \r\n", status);
	return status;
...
/* ATHENV */
    ...
    if (status != A_OK) {
        ...
        NDIS_DEBUG_PRINTF(DBG_TRACE, "Failed to update interrupt control registers err: %d\n", status);
...
static void DevDoEnableDisableRecvAsyncHandler(void *Context, HTC_PACKET *pPacket)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "+DevDoEnableDisableRecvAsyncHandler: (dev: 0x%X)\n", (A_UINT32)pDev);

    if (A_FAILED(pPacket->Status)) {
        NDIS_DEBUG_PRINTF(DBG_TRACE, " Failed to disable receiver, status:%d \n", pPacket->Status);
    }
        /* free this IO packet */
    AR6KFreeIOPacket(pDev,pPacket);
    NDIS_DEBUG_PRINTF(0, "-DevDoEnableDisableRecvAsyncHandler \n");
}
...
static A_STATUS DevDoEnableDisableRecvOverride(AR6K_DEVICE *pDev, A_BOOL EnableRecv, A_BOOL AsyncMode)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "%s(): Enable:%d Mode:%d\n",__FUNCTION__,   EnableRecv,AsyncMode);
                      ...
                      AR6K_IRQ_ENABLE_REGISTERS *pIrqEnableRegs)
{
    ...
    if (pIrqProcRegs != NULL) {
        NDIS_DEBUG_PRINTF(0, "Int Status:               0x%x\n",pIrqProcRegs->host_int_status);
        NDIS_DEBUG_PRINTF(0, "CPU Int Status:            0x%x\n",pIrqProcRegs->cpu_int_status);
        NDIS_DEBUG_PRINTF(0, "Error Int Status:          0x%x\n",pIrqProcRegs->error_int_status);
        NDIS_DEBUG_PRINTF(0, "Counter Int Status:        0x%x\n",pIrqProcRegs->counter_int_status);
        NDIS_DEBUG_PRINTF(0, "Mbox Frame:                0x%x\n",pIrqProcRegs->mbox_frame);
        NDIS_DEBUG_PRINTF(0, "Rx Lookahead Valid:        0x%x\n",pIrqProcRegs->rx_lookahead_valid);
        NDIS_DEBUG_PRINTF(0, "Rx Lookahead 0:            0x%x\n",pIrqProcRegs->rx_lookahead[0]);
        NDIS_DEBUG_PRINTF(0, "Rx Lookahead 1:            0x%x\n",pIrqProcRegs->rx_lookahead[1]);
    }

    if (pIrqEnableRegs != NULL) {
        NDIS_DEBUG_PRINTF(0, "Int Status Enable:         0x%x\n",pIrqEnableRegs->int_status_enable);
        NDIS_DEBUG_PRINTF(0, "Counter Int Status Enable: 0x%x\n",pIrqEnableRegs->counter_int_status_enable);
        NDIS_DEBUG_PRINTF(0, "<------------------------------->\n");
    }
}
...
static A_BOOL CheckOneBuffer(A_UINT16 *pBuffer16, int Length)
{
    ...
    for (i = 0; i < (Length / 2) ; i++,startCount++) {
        ...
        if ((A_UINT16)pBuffer16[i] != (A_UINT16)~startCount) {
            success = FALSE;
            NDIS_DEBUG_PRINTF(DBG_ERR, "Invalid Data Got:0x%X, Expecting:0x%X (offset:%d, total:%d) \n",
                        	pBuffer16[i], ((A_UINT16)~startCount), i, Length);
             NDIS_DEBUG_PRINTF(DBG_ERR, "0x%X 0x%X 0x%X 0x%X \n",
...
static A_BOOL CheckBuffers(void)
{
    ...
    for (i = 0; i < BUFFER_PROC_LIST_DEPTH ; i++) {
        ...
        if (!success) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Buffer : 0x%X, Length:%d failed verify \n",
...
static A_STATUS SendBuffers(AR6K_DEVICE *pDev, int mbox)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "Sending buffers on mailbox : %d \n",mbox);
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "Sent %d bytes (%d padded bytes) to mailbox : %d \n",totalBytes,totalwPadding,mbox);
...
static A_STATUS GetCredits(AR6K_DEVICE *pDev, int mbox, int *pCredits)
{
    ...
    while (TRUE) 
	{
        ...
        if (status != A_OK) {
			NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to decrement the command credit count register (mbox=%d)\n",mbox);
            status = A_ERROR;
            break;
        }
        ...
        if (timeout <= 0) {
			NDIS_DEBUG_PRINTF(DBG_ERR, " Timeout reading credit registers (mbox=%d, address:0x%X) \n",mbox,address);
            status = A_ERROR;
            break;
        }
...
static A_STATUS RecvBuffers(AR6K_DEVICE *pDev, int mbox)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "Waiting for buffers on mailbox : %d \n",mbox);
    ...
    while (curBuffer < BUFFER_PROC_LIST_DEPTH) {
        ...
        NDIS_DEBUG_PRINTF(DBG_TRACE, "Got %d messages on mailbox : %d \n",credits, mbox);
        ...
        for (i = 0; i < credits; i++) 
		{
			...
			* order... with padding applied by the target */
            ...
            if (status != A_OK) 
			{
                NDIS_DEBUG_PRINTF(DBG_ERR, "Failed to read %d bytes on mailbox:%d : address:0x%X \n",
    ...
    if (totalBytes != TEST_BYTES) {
        ...
        NDIS_DEBUG_PRINTF(DBG_ERR, "Got all buffers on mbox:%d total recv :%d (w/Padding : %d) \n",
...
static A_STATUS DoOneMboxHWTest(AR6K_DEVICE *pDev, int mbox)
{
    ...
    do {
        ...
        if (status != A_OK) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Sending buffers Failed : %d mbox:%d\n",status,mbox);
            break;
        }
        ...
        if (status != A_OK) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Recv buffers Failed : %d mbox:%d\n",status,mbox);
            break;
        }

            /* check the returned data patterns */
        if (!CheckBuffers()) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Buffer Verify Failed : mbox:%d\n",mbox);
            status = A_ERROR;
            break;
        }

        NDIS_DEBUG_PRINTF(DBG_TRACE, " Send/Recv success! mailbox : %d \n",mbox);
...
A_STATUS DoMboxHWTest(AR6K_DEVICE *pDev)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE,  " DoMboxHWTest START -  \n");
    ...
    do {
		...
		NDIS_DEBUG_PRINTF(DBG_TRACE, "Block Size to use: %d \n",g_BlockSizes[0]);

        if (g_BlockSizes[1] > BUFFER_BLOCK_PAD) {
			NDIS_DEBUG_PRINTF(DBG_TRACE, "%d Block size is too large for buffer pad %d\n",
                g_BlockSizes[1], BUFFER_BLOCK_PAD);
            break;
        }

		NDIS_DEBUG_PRINTF(DBG_TRACE, "Waiting for target.... \n");
        ...
        if (status != A_OK) {
            NDIS_DEBUG_PRINTF(DBG_TRACE, "Failed to wait for target ready \n");
            break;
        }

        NDIS_DEBUG_PRINTF(DBG_TRACE, "Target is ready ...\n");
        ...
        if (status != A_OK) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Failed to wait get parameters \n");
            break;
        }
        ...
        if ((numBufs * bufferSize) < TOTAL_BYTES) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Not Enough buffer space to run test! need:%d, got:%d \n",
        ...
        if (status != A_OK) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Failed to write end marker \n");
            break;
        }

        NDIS_DEBUG_PRINTF(DBG_TRACE, "End Marker: 0x%X \n",temp);
        ...
        if (status != A_OK) {
            NDIS_DEBUG_PRINTF(DBG_ERR, "Failed to write block mask \n");
            break;
        }

        NDIS_DEBUG_PRINTF(DBG_TRACE, "Set Block Mask: 0x%X \n",temp);
    ...
    if (status == A_OK) {
        NDIS_DEBUG_PRINTF(DBG_TRACE, " DoMboxHWTest DONE - SUCCESS! -  \n");
    } else {
        NDIS_DEBUG_PRINTF(DBG_ERR, " DoMboxHWTest DONE - FAILED! -  \n");
    }
