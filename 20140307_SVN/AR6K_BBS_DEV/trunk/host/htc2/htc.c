...
HTC_HANDLE HTCCreate(void *hif_handle, HTC_INIT_INFO *pInfo)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "HTCCreate + Enter \r\n");
	...
	do 
	{
		...
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to allocate memory. \r\n");
			status = A_ERROR;
			break;
		}
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "HTCCreate - Exit, status(%d) \r\n", status);
...
void  HTCDestroy(HTC_HANDLE HTCHandle)
{
    ...
    AR_DEBUG_PRINTF(0, ("+HTCDestroy ..  Destroying :0x%X \n",(A_UINT32)target));
    HTCCleanup(target);
    AR_DEBUG_PRINTF(0, ("-HTCDestroy \n"));
}
...
A_STATUS HTCWaitTarget(HTC_HANDLE HTCHandle)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "HTCWaitTarget - Enter (target:0x%X) \n", (A_UINT32)target);
	...
	do 
	{
		...
		if (A_FAILED(status)) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR," Target Not Available!!\n");
			break;
		}
		...
		if ((pRdyMsg->MessageID != HTC_MSG_READY_ID) || (pPacket->ActualLength < sizeof(HTC_READY_MSG))) 
		{
		...
		NDIS_DEBUG_PRINTF(DBG_TRACE," Target Ready: credits: %d credit size: %d, max_endpoint = %d\n",
							target->TargetCredits, target->TargetCreditSize,pRdyMsg->MaxEndpoints);			
		...
		status = HTCConnectService((HTC_HANDLE)target, &connect, &resp);
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE,"HTCWaitTarget - Exit\n");
...
A_STATUS HTCStart(HTC_HANDLE HTCHandle)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "HTCStart Enter\n");
	...
	* HTC control endpoint */
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "HTCStart Exit, status = %d \r\n", status);
    return status;
}
...
static void HTCReportFailure(void *Context, AR6K_TARGET_FAILURE_TYPE Type)
{
	...
	NDIS_DEBUG_PRINTF(DBG_ERR, " %s(type(%d) ) +Enter  \r\n", __FUNCTION__,Type);
...
void DebugDumpBytes(A_UCHAR *buffer, A_UINT16 length, char *pDescription)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "<---------Dumping %d Bytes : %s ------>\n", length, pDescription);
    ...
    for(i = 0; i < length; i++) {
        ...
        if(count == 16) {
            count = 0;
            offset = 0;
			NDIS_DEBUG_PRINTF(DBG_TRACE, "[H]: %s\n", stream);
            A_MEMZERO(stream, 60);
        }
    }
...
//bluebird modified 
__inline void HTC_RECYCLE_RX_PKT(HTC_TARGET *target, HTC_PACKET *p, HTC_ENDPOINT *e)
{
    if ((e)->EpCallBacks.EpRecvAlloc != NULL) {                   
         HTC_PACKET_RESET_RX(p);                           
         p->Status = A_ECANCELED;                          
