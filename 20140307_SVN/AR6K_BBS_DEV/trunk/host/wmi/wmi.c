...
static A_STATUS wmi_bssInfo_event_rx(struct wmi_t *wmip, A_UINT8 *datap,
                                     int len, A_UINT8 phy_mode);	   /* Bug 82893 */
...
#else
static unsigned int processDot11Hdr = 1;
#endif
#else
...
wmi_init(void *devt)
{
	...
	NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() + Enter \r\n",__FUNCTION__);
...
{
	NDIS_DEBUG_PRINTF(DBG_TRACE, "%s() + Enter \r\n",__FUNCTION__);
    AR_DEBUG_ASSERT( eid != ENDPOINT_UNUSED);
...
wmi_dix_2_dot3(struct wmi_t *wmip, void *osbuf)
{
    ...
    if (!IS_ETHERTYPE(A_BE2CPU16(typeorlen))) {
         ...
         */
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "packet already 802.3\n");
        return (A_OK);
    }
...
A_UINT8 wmi_get_traffic_class(void *osbuf, A_UINT32 layer2Priority, A_BOOL wmmEnabled)
{
    A_UINT8             *datap;
    ...
    else
    {
            ...
            userPriority = wmi_determine_userPriority (datap,layer2Priority);
    }
...
wmi_control_rx_xtnd(struct wmi_t *wmip, void *osbuf)
{
    ...
    if (A_NETBUF_LEN(osbuf) < sizeof(WMIX_CMD_HDR)) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "bad packet 1\n");
    ...
    if (A_NETBUF_PULL(osbuf, sizeof(WMIX_CMD_HDR)) != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "bad packet 2 \r\n");
        wmip->wmi_stats.cmd_len_err++;
        A_NETBUF_FREE(osbuf);
        return A_ERROR;
    }
	...
	NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : id = 0x%x \r\n", __FUNCTION__,id);
    switch (id) {
...
#endif /* CONFIG_TARGET_PROFILE_SUPPORT */
    default:
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " %s() : Unknown id(0x%x)\n",__FUNCTION__,id);
...
A_STATUS
wmi_control_rx(struct wmi_t *wmip, void *osbuf, A_UINT8 phy_mode) /* Bug 82893 */
{
	WMI_CMD_HDR *cmd;
	A_UINT16 id;
	A_UINT8 *datap;
	...
	if(loggingReq) 
	{
		NDIS_DEBUG_PRINTF(DBG_TRACE, "WMI %d \n",id);
		NDIS_DEBUG_PRINTF(DBG_TRACE, "WMI recv, MsgNo %d : ", cmdRecvNum);
		
		for(i = 0; i < len; i++)
			NDIS_DEBUG_PRINTF(DBG_TRACE, "%x ", datap[i]);
	...
	if(id != WMI_BSSINFO_EVENTID)
		NDIS_DEBUG_PRINTF(1, " %s() id -> 0x%04x \r\n", __FUNCTION__, id);

    switch (id) 
	{
	case (WMI_GET_BITRATE_CMDID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_GET_BITRATE_CMDID\n");
		status = wmi_bitrate_reply_rx(wmip, datap, len);
		break;
	case (WMI_GET_CHANNEL_LIST_CMDID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_GET_CHANNEL_LIST_CMDID\n");
		status = wmi_channelList_reply_rx(wmip, datap, len);
		break;
	case (WMI_GET_TX_PWR_CMDID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_GET_TX_PWR_CMDID\n");
		status = wmi_txPwr_reply_rx(wmip, datap, len);
		break;
	case (WMI_READY_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_READY_EVENTID\n");
		status = wmi_ready_event_rx(wmip, datap, len);
		A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
		A_WMI_DBGLOG_INIT_DONE(wmip->wmi_devt);
		break;
	case (WMI_CONNECT_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_CONNECT_EVENTID\n");
		status = wmi_connect_event_rx(wmip, datap, len);
		A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
		break;
	case (WMI_DISCONNECT_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_DISCONNECT_EVENTID \r\n");
		status = wmi_disconnect_event_rx(wmip, datap, len);
		A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
		break;
	case (WMI_PEER_NODE_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_PEER_NODE_EVENTID\n");
		status = wmi_peer_node_event_rx(wmip, datap, len);
		A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
		break;
	case (WMI_TKIP_MICERR_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_TKIP_MICERR_EVENTID\n");
		status = wmi_tkip_micerr_event_rx(wmip, datap, len);
		break;
	case (WMI_BSSINFO_EVENTID):
		NDIS_DEBUG_PRINTF(0, "WMI_BSSINFO_EVENTID\n");
		status = wmi_bssInfo_event_rx(wmip, datap, len, phy_mode);	/* Bug 82893 */
	if (status == A_OK)
		A_WMI_SEND_GENERIC_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
		break;
	case (WMI_REGDOMAIN_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_REGDOMAIN_EVENTID\n");
		status = wmi_regDomain_event_rx(wmip, datap, len);
		break;
	case (WMI_PSTREAM_TIMEOUT_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_PSTREAM_TIMEOUT_EVENTID \r\n");
	...
	case (WMI_NEIGHBOR_REPORT_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_NEIGHBOR_REPORT_EVENTID \r\n");
		status = wmi_neighborReport_event_rx(wmip, datap, len);
		break;
	case (WMI_SCAN_COMPLETE_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_SCAN_COMPLETE_EVENTID \r\n");
		status = wmi_scanComplete_rx(wmip, datap, len);
		A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
		break;
	case (WMI_CMDERROR_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_CMDERROR_EVENTID \r\n");
		status = wmi_errorEvent_rx(wmip, datap, len);
		break;
	case (WMI_REPORT_STATISTICS_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_REPORT_STATISTICS_EVENTID \r\n");
		status = wmi_statsEvent_rx(wmip, datap, len);
		break;
	case (WMI_RSSI_THRESHOLD_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_RSSI_THRESHOLD_EVENTID \r\n");
		status = wmi_rssiThresholdEvent_rx(wmip, datap, len);
		break;
	case (WMI_ERROR_REPORT_EVENTID):
		NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_ERROR_REPORT_EVENTID \r\n");
        status = wmi_reportErrorEvent_rx(wmip, datap, len);
        A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
        break;
    case (WMI_OPT_RX_FRAME_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_OPT_RX_FRAME_EVENTID\n");
        status = wmi_opt_frame_event_rx(wmip, datap, len);
        break;
    case (WMI_REPORT_ROAM_TBL_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_REPORT_ROAM_TBL_EVENTID\n");
        status = wmi_roam_tbl_event_rx(wmip, datap, len);
        break;
    case (WMI_EXTENSION_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_EXTENSION_EVENTID\n");
        status = wmi_control_rx_xtnd(wmip, osbuf);
        break;
    case (WMI_CAC_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_CAC_EVENTID\n");
        status = wmi_cac_event_rx(wmip, datap, len);
        break;
    case (WMI_CHANNEL_CHANGE_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_CHANNEL_CHANGE_EVENTID\n");
        status = wmi_channel_change_event_rx(wmip, datap, len);
        break;
    case (WMI_REPORT_ROAM_DATA_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_REPORT_ROAM_DATA_EVENTID\n");
        status = wmi_roam_data_event_rx(wmip, datap, len);
        break;
#ifdef CONFIG_HOST_TCMD_SUPPORT
    case (WMI_TEST_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_TEST_EVENTID\n");
        status = wmi_tcmd_test_report_rx(wmip, datap, len);
        break;
#endif
    case (WMI_GET_FIXRATES_CMDID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_GET_FIXRATES_CMDID\n");
        status = wmi_ratemask_reply_rx(wmip, datap, len);
        break;
    case (WMI_TX_RETRY_ERR_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_TX_RETRY_ERR_EVENTID\n");
        status = wmi_txRetryErrEvent_rx(wmip, datap, len);
        A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
        break;
    case (WMI_SNR_THRESHOLD_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_SNR_THRESHOLD_EVENTID\n");
        status = wmi_snrThresholdEvent_rx(wmip, datap, len);
        break;
    case (WMI_LQ_THRESHOLD_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_LQ_THRESHOLD_EVENTID\n");
        status = wmi_lqThresholdEvent_rx(wmip, datap, len);
        A_WMI_SEND_EVENT_TO_APP(wmip->wmi_devt, id, datap, len);
        break;
    case (WMI_APLIST_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "Received APLIST Event\n");
        status = wmi_aplistEvent_rx(wmip, datap, len);
        break;
    case (WMI_GET_KEEPALIVE_CMDID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_GET_KEEPALIVE_CMDID\n");
    ...
    case (WMI_GET_PMKID_LIST_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_GET_PMKID_LIST Event\n");
        status = wmi_get_pmkid_list_event_rx(wmip, datap, len);
        break;
    case (WMI_PSPOLL_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_PSPOLL_EVENT\n");
        status = wmi_pspoll_event_rx(wmip, datap, len);
        break;
    case (WMI_DTIMEXPIRY_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_DTIMEXPIRY_EVENT\n");
        status = wmi_dtimexpiry_event_rx(wmip, datap, len);
        break;
    case (WMI_SET_PARAMS_REPLY_EVENTID):
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "WMI_SET_PARAMS_REPLY Event\n");
    ...
    case (WMI_ACM_REJECT_EVENTID):
        ...
        break;
    default:
        NDIS_DEBUG_PRINTF(DBG_ERR, "Unknown id 0x%x \r\n", id);
        wmip->wmi_stats.cmd_id_err++;
        status = A_ERROR;
        break;
    }
	...
	NDIS_DEBUG_PRINTF(0, "%s() - Exit, status = %d \r\n", __FUNCTION__, status);
    return status;
}
...
{
	const int ready_data_size = 11;
	
    WMI_READY_EVENT *ev = (WMI_READY_EVENT *)datap;

	 NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : + Enter , sizeof(WMI_READY_EVENT) = %d\r\n", __FUNCTION__, sizeof(WMI_READY_EVENT));

    if (len < ready_data_size) {
		 NDIS_DEBUG_PRINTF(DBG_ERR, "%s() :len(%d) \n", __FUNCTION__,len);
        return A_EINVAL;
    }
...
wmi_connect_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : freq %d bssid %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
							__FUNCTION__, ev->channel,
...
wmi_peer_node_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    if (ev->eventCode == PEER_NODE_JOIN_EVENT) {
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : Joined node with Macaddr: ", __FUNCTION__);
    } else if(ev->eventCode == PEER_NODE_LEAVE_EVENT) {
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : left node with Macaddr: ", __FUNCTION__);
    }
...
static A_STATUS
wmi_bssInfo_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len, A_UINT8 phy_mode)	/* Bug 82893 */
{
    bss_t *bss = NULL;
    ...
    len -= sizeof(WMI_BSS_INFO_HDR);
   	...
   	NDIS_DEBUG_PRINTF(DBG_BSS_EVENT, "#BSS_EVENT - ch %u, rssi %d, bssid %02x:%02x:%02x:%02x:%02x:%02x \r\n", 
					bih->channel, (unsigned char) bih->rssi, bih->bssid[0],bih->bssid[1], 
					bih->bssid[2], bih->bssid[3], bih->bssid[4],    bih->bssid[5]);
...
/* ATHENV V9 --- */
    ...
    /* Bug 82896 - make sure connected to hidden AP before populate SSID */
    if (IEEE80211_ADDR_EQ(wmip->wmi_bssid, bih->bssid) &&		   
         (BEACON_FTYPE == bih->frameType) &&
         (0 != cached_ssid_len) &&
    ...
    if (wlan_parse_beacon(bss->ni_buf, len, &bss->ni_cie, phy_mode) != A_OK) {	/* Bug 82893 */
        wlan_node_free(bss);
        return A_EINVAL;
    }
...
wmi_opt_frame_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " %s() : opt frame event %2.2x:%2.2x\n", __FUNCTION__, bih->bssid[4], bih->bssid[5]);
...
wmi_dset_open_req_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() +Enter - dset_id=0x%x\n", __FUNCTION__, dsetopenreq->dset_id);
...
wmi_errorEvent_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_ERR_LOG, "Programming Error: cmd=%d ", ev->commandId);
    switch (ev->errorCode) {
    case (INVALID_PARAM):
        NDIS_DEBUG_PRINTF(DBG_ERR_LOG, "Illegal Parameter \r\n");
        break;
    case (ILLEGAL_STATE):
        NDIS_DEBUG_PRINTF(DBG_ERR_LOG, "Illegal State \r\n");
        break;
    case (INTERNAL_ERROR):
        NDIS_DEBUG_PRINTF(DBG_ERR_LOG, "Internal Error \r\n");
        break;
    }
...
wmi_rssiThresholdEvent_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    SQ_THRESHOLD_PARAMS *sq_thresh = &wmip->wmi_SqThresholdParams[SIGNAL_QUALITY_METRICS_RSSI];
    ...
    if (newThreshold) {
        ...
        if (rssi < sq_thresh->upper_threshold[0]) {
            NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : Spurious upper RSSI threshold event: %d\n", __FUNCTION__, rssi);
        ...
        if (rssi > sq_thresh->lower_threshold[0]) {
            NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : Spurious lower RSSI threshold event: %d %d\n", __FUNCTION__, rssi, sq_thresh->lower_threshold[0]);
    ...
    if (wmi_send_rssi_threshold_params(wmip, &cmd) != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "%s() : Unable to configure the RSSI thresholds\n",
                  __FUNCTION__);
    }
...
wmi_reportErrorEvent_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_ERR_LOG,"%s() +Enter\n", __FUNCTION__);
...
wmi_snrThresholdEvent_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    if (newThreshold) {
        ...
        if (snr < sq_thresh->upper_threshold[0]) {
            NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : Spurious upper SNR threshold event: %d\n", __FUNCTION__, snr);
        ...
        if (snr > sq_thresh->lower_threshold[0]) {
            NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : Spurious lower SNR threshold event: %d %d\n", __FUNCTION__, snr, sq_thresh->lower_threshold[0]);
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s()-> snr: %d, threshold: %d, lower: %d, upper: %d\n" ,__FUNCTION__, snr, newThreshold, lower_snr_threshold,
              upper_snr_threshold);

    snr_event_value = snr;

    if (wmi_send_snr_threshold_params(wmip, &cmd) != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "%s()-> Unable to configure the SNR thresholds\n", __FUNCTION__);
    }
...
wmi_aplistEvent_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() : Number of APs in APLIST Event is %d\n",__FUNCTION__, ev->numAP);
    ...
    for (i = 0; i < ev->numAP; i++) {
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "AP#%d BSSID %2.2x %2.2x %2.2x %2.2x %2.2x %2.2x, Channel %d\n", i,
...
wmi_dbglog_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
	...
	NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " %s() +Enter, len = %d, dropped = %02x%02x%02x%02x \r\n", __FUNCTION__, len,datap[0],datap[1],datap[2],datap[3]);
	
	dropped = (datap[0]<<24) + (datap[1]<<16) + (datap[2]<<8) + datap[3];
...
wmi_gpio_intr_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC,
        "%s() Enter - intrmask=0x%x input=0x%x.\n", __FUNCTION__,
...
#define IS_OPT_TX_CMD(cmdId) ((cmdId == WMI_OPT_TX_FRAME_CMDID))
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "%s() + Enter , CMD_ID = %x\r\n",__FUNCTION__,cmdId);
    ...
    if (A_NETBUF_PUSH(osbuf, sizeof(WMI_CMD_HDR)) != A_OK) {
		NDIS_DEBUG_PRINTF(DBG_ERR, " %S() : No Memory !! \r\n", __FUNCTION__);
        return A_NO_MEMORY;
    }
	...
	{
		NDIS_DEBUG_PRINTF(DBG_TRACE," -- IS_OPT_TX_CMD -- \r\n");
                  ...
                  A_INT8 numChan, A_UINT16 *channelList)
{
	...
	NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " %s() --> forceFgScan(%d), forceScanInterval(%d), numChan(%d) \r\n",
				__FUNCTION__,forceFgScan, forceScanInterval, numChan);
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, " %s() -> Start CMD send !! \r\n",__FUNCTION__);
                   ...
                   A_UINT32 max_dfsch_act_time, A_UINT16 maxact_scan_per_ssid)
{
	...
	NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " %s() --> fg_start_sec(%d), fg_end_sec(%d)\r\n",__FUNCTION__,fg_start_sec,fg_end_sec);
...
wmi_dataSync_send(struct wmi_t *wmip, void *osbuf, HTC_ENDPOINT_ID eid)
{
	...
	AR_DEBUG_ASSERT( eid != wmip->wmi_endpoint_id);
	AR_DEBUG_ASSERT(osbuf != NULL);
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, "%s(): +Enter - eid %d\n",__FUNCTION__, eid);
...
wmi_sync_point(struct wmi_t *wmip)
{
	...
	do 
	{
		...
		*/
		...
		for(i=0; i < numPriStreams; i++) 
		{
			AR_DEBUG_ASSERT(dataSyncBufs[i].osbuf != NULL);
...
wmi_create_pstream_cmd(struct wmi_t *wmip, WMI_CREATE_PSTREAM_CMD *params)
{
    ...
    if (params->nominalPHY >= minimalPHY)
    {
        ...
        NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "TSRS IE Enabled::MinPhy %x->NominalPhy ===> %x \r\n", minimalPHY, nominalPHY);
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "Sending create_pstream_cmd: ac=%d tsid:%d\n", params->trafficClass, params->tsid);
...
wmi_delete_pstream_cmd(struct wmi_t *wmip, A_UINT8 trafficClass, A_UINT8 tsid)
{
    ...
    if (trafficClass > 3) {
		NDIS_DEBUG_PRINTF(DBG_ERR, "Invalid trafficClass: %d \r\n",trafficClass);
        return A_EINVAL;
    }
    ...
    if (!(activeTsids & (1<<tsid))) {
        ...
        NDIS_DEBUG_PRINTF(DBG_ERR, "TSID %d does'nt exist for trafficClass: %d\n",tsid, trafficClass);
            /* TODO: return a more appropriate err code */
        return A_ERROR;
    }

    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "Sending delete_pstream_cmd: trafficClass: %d tsid=%d\n", trafficClass, tsid);
...
wmi_set_framerate_cmd(struct wmi_t *wmip, A_UINT8 bEnable, A_UINT8 type, A_UINT8 subType, A_UINT16 rateMask)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, " type %02X, subType %02X, rateMask %04x \r\n",type, subType, rateMask);
    ...
    for(rateIndex = 0; rateIndex < MAX_NUMBER_OF_SUPPORT_RATES; rateIndex++) {
       if((1 << rateIndex) & (A_UINT32)fixRatesMask) {
            if(wmi_is_bitrate_index_valid(wmip, rateIndex) != TRUE) {
                NDIS_DEBUG_PRINTF(DBG_ERR, "Set Fix Rates command failed: Given rate is illegal in current PHY mode\n");
                return A_EINVAL;
            }
       }
    }
                      ...
                      A_UINT8 size)
{
	...
	NDIS_DEBUG_PRINTF(DBG_TRACE, " %s(): + Enter , size = %d\r\n", __FUNCTION__, size);
...
#define TSPEC_SBA_ATHEROS_DEF                 0x2000  /* factor is 1 */
    ...
    if(tspecCompliance == ATHEROS_COMPLIANCE) {
        if ((pCmd->suspensionInt != TSPEC_SUSPENSION_INTERVAL_ATHEROS_DEF) ||
            ...
            NDIS_DEBUG_PRINTF(0, "Invalid TSPEC params\n");
...
wmi_tcmd_test_report_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
   ...
   NDIS_DEBUG_PRINTF(DBG_WMI, "Enter \r\n");
...
wmi_set_bt_status_cmd(struct wmi_t *wmip, A_UINT8 streamType, A_UINT8 status)
{
    ...
    NDIS_DEBUG_PRINTF(0,"Enter - streamType=%d, status=%d\n", streamType, status);
...
wmi_set_bt_params_cmd(struct wmi_t *wmip, WMI_SET_BT_PARAMS_CMD* cmd)
{
    ...
    NDIS_DEBUG_PRINTF(0,"cmd params is %d\n", cmd->paramType);

    if (cmd->paramType == BT_PARAM_SCO) {
      NDIS_DEBUG_PRINTF(0,"sco params %d %d %d %d %d %d %d %d %d %d %d %d\n",
      ...
      NDIS_DEBUG_PRINTF(0,"A2DP params %d %d %d %d %d %d %d %d %d\n",
                    ...
                    A_UINT32 targ_reply_arg)
{
    ...
    A_DPRINTF(0, (DBGFMT "Enter - wmip=0x%x\n", DBGARG, (int)wmip));
...
wmi_scan_indication (struct wmi_t *wmip)
{
    ...
    if (0 == numbss)
    {
/*        NDIS_DEBUG_PRINTF(1, "AR6K: scan indication: 0 bss\n"); */
...
/*    NDIS_DEBUG_PRINTF(1, "AR6K: scan indication: %u bss\n", numbss); */
...
wmi_ap_set_hidden_ssid(struct wmi_t *wmip, A_UINT8 hidden_ssid)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "%s() :  AR6000_XIOCTL_AP_HIDDEN_SSID %d\n", __FUNCTION__,hidden_ssid);
...
wmi_pspoll_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    A_WMI_PSPOLL_EVENT(wmip->wmi_devt, (A_UINT8)ev->aid);
    return A_OK;
}
...
#if 1 //bluebird
/*
* It isn't exist at linxe. we need for build . 
*/
HTC_ENDPOINT_ID
wmi_get_endpoint(struct wmi_t *wmip, A_UINT8 trafficClass)
{
    NDIS_DEBUG_PRINTF(DBG_LEVEL_WMI_HTC, "1 pkt of traffic class(%d) to mbox(%d) \r\n",
