...
BMIDone(HIF_DEVICE *device)
{
	...
	if (bmiDone) 
	{
		NDIS_DEBUG_PRINTF(1, "BMIDone skipped\n");
		return A_OK;
	}

	NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI Done: Enter (device: 0x%p) \r\n", device);
	bmiDone = TRUE;
	cid = BMI_DONE;
	...
	if (status != A_OK) 
	{
		NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to write to the device\n");
		return A_ERROR;
	}
	NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI Done: Exit \r\n");
...
BMIGetTargetInfo(HIF_DEVICE *device, struct bmi_target_info *targ_info)
{
	...
	if (bmiDone) {
		NDIS_DEBUG_PRINTF(DBG_ERR, "Command disallowed \r\n");
		return A_ERROR;
	}

	NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI Get Target Info: Enter (device: 0x%p) \r\n", device);
	...
	if (status != A_OK) 
	{
		NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to read Target Version from the device \r\n");
		return A_ERROR;
	}

	NDIS_DEBUG_PRINTF(DBG_TRACE, "AR6000 : target_infot -> target_ver = 0x%08x \r\n", targ_info->target_ver);
	...
	if (targ_info->target_ver == TARGET_VERSION_SENTINAL) 
	{
		...
		if (status != A_OK) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to read Target Info Byte Count from the device\n");
			return A_ERROR;
		}
		...
		*/
		AR_DEBUG_ASSERT(targ_info->target_info_byte_count == sizeof(*targ_info));
		...
		if (status != A_OK) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to read Target Info (%d bytes) from the device\n",
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI Get Target Info: Exit (ver: 0x%x type: 0x%x)\n",
              ...
              A_UINT32 length)
{
    ...
    while (remaining)
    {
        ...
        if (status != A_OK) {
            AR_DEBUG_PRINTF(0, ("Unable to write to the device\n"));
            return A_ERROR;
        }
        status = bmiBufferReceive(device, data, rxlen, TRUE);
        if (status != A_OK) {
            AR_DEBUG_PRINTF(0, ("Unable to read from the device\n"));
            return A_ERROR;
        }
    ...
    AR_DEBUG_PRINTF(0, ("BMI Read Memory: Exit\n"));
    return A_OK;
}
               ...
               A_UINT32 length)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI Write Memory: Exit\n");
           ...
           A_UINT32 *param)
{
    ...
    if (bmiDone) {
        AR_DEBUG_PRINTF(1, ("Command disallowed\n"));
        return A_ERROR;
    }
    ...
    status = bmiBufferSend(device, data, offset);
    if (status != A_OK) {
        AR_DEBUG_PRINTF(1, ("Unable to write to the device\n"));
        return A_ERROR;
    }

    status = bmiBufferReceive(device, data, sizeof(*param), FALSE);
    if (status != A_OK) {
        AR_DEBUG_PRINTF(1, ("Unable to read from the device\n"));
        return A_ERROR;
    }

    A_MEMCPY(param, data, sizeof(*param));

    AR_DEBUG_PRINTF(1, ("BMI Execute: Exit (param: %d)\n", *param));
    return A_OK;
}
               ...
               A_UINT32 address)
{
    ...
    AR_DEBUG_PRINTF(0, ("BMI Set App Start: Exit\n"));
    return A_OK;
}
                   ...
                   A_UINT32 *rompatch_id)
{
    ...
    AR_DEBUG_PRINTF(0, ("BMI rompatch Install: (rompatch_id=%d)\n", *rompatch_id));
    return A_OK;
}
                     ...
                     A_UINT32 rompatch_id)
{
    ...
    AR_DEBUG_PRINTF(0, ("BMI rompatch UNinstall: (rompatch_id=0x%x)\n", rompatch_id));
    return A_OK;
}
                             ...
                             A_UINT32 do_activate)
{
    ...
    AR_DEBUG_PRINTF(0, ("BMI Change rompatch Activation: Exit\n"));
          ...
          A_UINT32 length)
{
    ...
    NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI LZ Data: Exit\n");
                 ...
                 A_UINT32 address)
{
    ...
    status = bmiBufferSend(device, data, offset);
    if (status != A_OK) {
        NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to Start LZ Stream to the device\n");
        return A_ERROR;
    }

    NDIS_DEBUG_PRINTF(DBG_TRACE, "BMI LZ Stream Start: Exit\n");
              ...
              A_UINT32 length)
{
	...
	while(timeout-- && !cmdCredits) 
	{
		...
		* make all HIF accesses 4-byte aligned */
        ...
        if (status != A_OK) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to decrement the command credit count register\n");
			return A_ERROR;
		}
	...
	if (cmdCredits) 
	{
		...
		status = HIFReadWrite(device, address, buffer, length, HIF_WR_SYNC_BYTE_INC, NULL);
		
        if (status != A_OK) 
		{
			NDIS_DEBUG_PRINTF(DBG_ERR, "Unable to send the BMI data to the device\n");
			return A_ERROR;
		}
	...
	{
		NDIS_DEBUG_PRINTF(DBG_ERR, "BMI Communication timeout - bmiBufferSend\n");
		return A_ERROR;
	}
                 ...
                 A_BOOL want_timeout)
{
	...
	HIFConfigureDevice(device, HIF_DEVICE_GET_MBOX_ADDR, &mboxAddress[0], sizeof(mboxAddress));
		...
		*/
		...
		while((!want_timeout || timeout--) && !word_available) 
		{
			...
			if (getPendingEventsFunc != NULL) 
			{
				...
				if (status != A_OK) 
				{
					AR_DEBUG_PRINTF(1,("BMI: Failed to get pending events \n"));
					break;
				}
			...
			if (status != A_OK) 
			{
				AR_DEBUG_PRINTF(1, ("Unable to read RX_LOOKAHEAD_VALID register\n"));
				return A_ERROR;
			}
		...
		if (!word_available) 
		{
			AR_DEBUG_PRINTF(0, ("BMI Communication timeout - bmiBufferReceive FIFO empty\n"));
			return A_ERROR;
		}
	}
...
#define CONSERVATIVE_BMI_READ 0
	...
	*/
		...
		*/
		...
		while((!want_timeout || timeout--) && !cmdCredits) 
		{
			...
			* The rationale here is to make all HIF accesses a multiple of 4 bytes */
			...
			if (status != A_OK) 
			{
				AR_DEBUG_PRINTF(1, ("Unable to read the command credit count register\n"));
				return A_ERROR;
			}
		...
		if (!cmdCredits) 
		{
			AR_DEBUG_PRINTF(0, ("BMI Communication timeout- bmiBufferReceive no credit\n"));
			return A_ERROR;
		}
	...
	if (status != A_OK) 
	{
		AR_DEBUG_PRINTF(1, ("Unable to read the BMI data from the device\n"));
		return A_ERROR;
	}
