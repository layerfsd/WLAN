                   ...
                   void *arg)
{
    ...
    /*NDIS_DEBUG_PRINTF (1, "numNodes available ==> %d\n",  numNodes); */
}
...
wlan_node_table_init(void *wmip, struct ieee80211_node_table *nt)
{
    ...
    NDIS_DEBUG_PRINTF(0, "node table = 0x%x\n", (A_UINT32)nt);
...
{
	NDIS_DEBUG_PRINTF(0, " %s() -> ############# nodeAge = %d \r\n", __func__, nodeAge);
...
wlan_refresh_inactive_nodes (struct ieee80211_node_table *nt)
{
    ...
    A_UINT32 timeoutValue = 0;
#if 1	//bluebird
	A_UINT32 now = A_MS_TICKGET() + WLAN_NODE_INACT_TIMEOUT_MSEC;
#else
